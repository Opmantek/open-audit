...
function fieldEnter(campo,evt,idfld) {
    ...
    if (evt.keyCode == 13) {
        ...
        remotos = new datosServidor;
         ...
         nt = remotos.enviar(urlBase+fieldName+"/"+fieldValue,"");
         //remove glow
        noLight(elem);
        elem.innerHTML = nt;
        changing = false;
...
function editBox(actual) {
    ...
    if(!changing){
        ...
        if(height < 40){
            ...
            actual.innerHTML = "<textarea name=\"textarea\" id=\""+ actual.id +"_field\" style=\"width: "+width+"px; height: "+height+"px; font-family: Verdana;\" onfocus=\"highLight(this);\" onblur=\"noLight(this); return fieldBlur(this,'" + actual.id + "');\">" + actual.innerHTML + "</textarea>";
        }
        changing = true;
    }
