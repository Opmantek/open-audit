...
class Test extends MY_Controller {
	...
	function test_snmp() {
		if (!isset($_POST['submit'])) {
			...
			foreach ($i as $line) {
				$j = explode (" ", $line);
				if (substr($j[0], 0, 1) == ".") { $j[0] = substr($j[0], 1, strlen($j[0])); }
				$result[$j[0]] = @$j[3];
			}
			echo "<pre>\n";
			#print_r($result);
		...
		$details->device_type = '';
				...
				if (substr($details->oid, 0, 1) == ".") { $details->oid = substr($details->oid, 1, strlen($details->oid)); }
				...
				if ($details->type == '') {
					$details->type = str_replace("OID: HOST-RESOURCES-TYPES::hrDevice", "", 
									 str_replace("STRING: ", "", @$result["1.3.6.1.2.1.25.3.2.1.2.1"]));
