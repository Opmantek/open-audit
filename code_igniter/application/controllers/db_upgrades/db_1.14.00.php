...
$this->log_db('Upgrade database to 1.14 commenced');
...
if ($this->db->table_exists('oa_location')) {
  $location = 'oa_location';
} else {
  $location = 'locations';
}
$sql = "SELECT COUNT(*) AS `count` FROM `" . $location . "` WHERE id = 0";
$query = $this->db->query($sql);
$result = $query->result();
$this->log_db($this->db->last_query());
if ($result[0]->count > 0) {
    // OK, we have a default Org at id = 0, check to see if we have an Org at id = 1
    $sql = "SELECT COUNT(*) AS `count` FROM `" . $location . "` WHERE id = 1";
    $query = $this->db->query($sql);
    $result = $query->result();
    $this->log_db($this->db->last_query());
    if ($result[0]->count > 0) {
        // Move this Location
        if ($this->db->field_exists('org_id', $location)) {
            $sql = "INSERT INTO `" . $location . "` (SELECT NULL, `name`,`org_id`,`type`,`room`,`suite`,`level`,`address`,`suburb`,`city`,`district`,`region`,`area`,`state`,`postcode`,`country`,`tags`,`phone`,`picture`,`latitude`,`longitude`,`geo`,`comments`,`icon`,'system',NOW() FROM `" . $location . "` WHERE id = 1)";
        } else {
            $sql = "INSERT INTO `" . $location . "` (SELECT NULL, `name`,`type`,`room`,`suite`,`level`,`address`,`suburb`,`city`,`district`,`region`,`area`,`state`,`postcode`,`country`,`tags`,`phone`,`picture`,`latitude`,`longitude`,`geo`,`comments`,`icon`, `group_id` FROM `" . $location . "` WHERE id = 1)";
        }
        $this->db->query($sql);
        $this->log_db($this->db->last_query());
    ...
    $sql = "UPDATE `" . $location . "` SET id = 1 WHERE id = 0";
    $this->db->query($sql);
    $this->log_db($this->db->last_query());
} else {
    // Nothing at id = 0, check something is at id = 1
    ...
    if ($result[0]->count == 0) {
        // Insert the default Org
        $sql = "INSERT INTO `" . $location . "` VALUES (1,'Default Location',1,'Office','','','','','','Gold Coast','','','','Queensland','','Australia','','','',-28.017260,153.425705,'','Default Location','office','system','2000-01-01 00:00:00')";
        $this->db->query($sql);
        $this->log_db($this->db->last_query());
    }
}
...
$this->alter_table('additional_field', 'edited_date', "ADD `edited_date` datetime NOT NULL DEFAULT '2000-01-01 00:00:00' AFTER `edited_by`", 'add');
...
$sql = "DELETE FROM `chart` WHERE `org_id` NOT IN (SELECT `org_id` FROM `oa_org`)";
$this->db->query($sql);
$this->log_db($this->db->last_query());
...
# cluster
$sql = "SHOW COLUMNS FROM `cluster` WHERE Field = 'org_id'";
$query = $this->db->query($sql);
$result = $query->result();
if (count($result) === 0) {
...
# configuration
if ($this->db->table_exists('oa_config') and !$this->db->table_exists('configuration')) {
...
# credentials
$sql = "SHOW COLUMNS FROM `credentials` WHERE Field = 'org_id'";
$query = $this->db->query($sql);
$result = $query->result();
if (count($result) === 0) {
...
# files
$sql = "SHOW COLUMNS FROM `files` WHERE Field = 'org_id'";
$query = $this->db->query($sql);
$result = $query->result();
if (count($result) === 0) {
...
# graph
$sql = "SHOW COLUMNS FROM `graph` WHERE Field = 'org_id'";
$query = $this->db->query($sql);
$result = $query->result();
if (count($result) === 0) {
...
# invoice
$sql = "SHOW COLUMNS FROM `invoice` WHERE Field = 'org_id'";
$query = $this->db->query($sql);
$result = $query->result();
if (count($result) === 0) {
...
# networks
$sql = "SHOW COLUMNS FROM `networks` WHERE Field = 'org_id'";
$query = $this->db->query($sql);
$result = $query->result();
if (count($result) === 0) {
...
if ($this->db->table_exists('oa_connection')) {
    $sql = "SHOW COLUMNS FROM `oa_connection` WHERE Field = 'org_id'";
    $query = $this->db->query($sql);
    $result = $query->result();
    if (count($result) === 0) {
...
if ($this->db->table_exists('oa_group')) {
    $sql = "SHOW COLUMNS FROM `oa_group` WHERE Field = 'org_id'";
    $query = $this->db->query($sql);
    $result = $query->result();
    if (count($result) === 0) {
...
# oa_location
if ($this->db->table_exists('oa_location')) {
    $sql = "SHOW COLUMNS FROM `oa_location` WHERE Field = 'org_id'";
    $query = $this->db->query($sql);
    $result = $query->result();
    if (count($result) === 0) {
    ...
    $sql = "SHOW COLUMNS FROM `oa_location` WHERE Field = 'group_id'";
    $query = $this->db->query($sql);
    $result = $query->result();
    if (count($result) === 0) {
...
if ($this->db->table_exists('oa_report')) {
    $sql = "SHOW COLUMNS FROM `oa_report` WHERE Field = 'org_id'";
    $query = $this->db->query($sql);
    $result = $query->result();
    if (count($result) === 0) {
...
# oa_temp
if ($this->db->table_exists('oa_temp')) {
...
if ($this->db->table_exists('oa_user')) {
    $sql = "SHOW COLUMNS FROM `oa_user` WHERE Field = 'org_id'";
    $query = $this->db->query($sql);
    $result = $query->result();
    if (count($result) === 0) {
...
$sql = "INSERT INTO queries VALUES (NULL, 1, \"Billing Report\", \"Name, last seen on and by, type, class, manufacturer, model, serial, user, location.\", \"SELECT system.id AS `system.id`, system.icon AS `system.icon`, system.type AS `system.type`, system.name AS `system.name`, system.domain AS `system.domain`, system.ip AS `system.ip`, oa_org.name AS `oa_org.name`, system.last_seen AS `system.last_seen`, system.last_seen_by AS `system.last_seen_by`, system.manufacturer AS `system.manufacturer`, system.model AS `system.model`, system.serial AS `system.serial`, system.class AS `system.class`, windows.user_name AS `windows.user_name`, oa_location.name AS `oa_location.name` FROM system LEFT JOIN oa_location ON (system.location_id = oa_location.id) LEFT JOIN windows ON (system.id = windows.system_id AND windows.current = 'y') LEFT JOIN oa_org ON (system.org_id = oa_org.id) WHERE @filter\", \"\", \"y\", \"system\", '2000-01-01 00:00:00')";
$this->db->query($sql);
$this->log_db($this->db->last_query());
...
$sql = "INSERT INTO queries VALUES (NULL, 1, \"Failed Audits\", \"Name, ip address, audit time, audit stage.\", \"SELECT system.id AS `system.id`, system.icon AS `system.icon`, system.type AS `system.type`, system.name AS `system.name`, system.domain AS `system.domain`, system.ip AS `system.ip`, audit_log.timestamp AS `audit_log.timestamp`, audit_log.debug AS `audit_log.debug` FROM system LEFT JOIN audit_log ON (system.id = audit_log.system_id) WHERE @filter AND audit_log.debug > ''\", \"\", \"y\", \"system\", '2000-01-01 00:00:00')";
$this->db->query($sql);
$this->log_db($this->db->last_query());
...
$sql = "INSERT INTO roles VALUES (NULL, 'admin', 'This role can change global options.', '{\"baselines\":\"crud\",\"configuration\":\"crud\",\"database\":\"crud\",\"errors\":\"r\",\"groups\":\"crud\",\"ldap_servers\":\"crud\",\"logs\":\"crud\",\"nmis\":\"crud\",\"queries\":\"crud\",\"roles\":\"crud\",\"search\":\"crud\",\"sessions\":\"crud\",\"summaries\":\"crud\"}', 'open-audit_roles_admin', 'system', '2000-01-01 00:00:00')";
$this->db->query($sql);
$this->log_db($this->db->last_query());

$sql = "INSERT INTO roles VALUES (NULL, 'org_admin', 'This role is used for administration of endpoints that contain an org_id.', '{\"baselines\":\"crud\",\"charts\":\"crud\",\"connections\":\"crud\",\"credentials\":\"crud\",\"summaries\":\"crud\",\"devices\":\"crud\",\"discoveries\":\"crud\",\"errors\":\"r\",\"fields\":\"crud\",\"files\":\"crud\",\"graph\":\"crud\",\"groups\":\"crud\",\"invoice\":\"crud\",\"licenses\":\"crud\",\"locations\":\"crud\",\"networks\":\"crud\",\"orgs\":\"crud\",\"queries\":\"crud\",\"scripts\":\"crud\",\"search\":\"crud\",\"sessions\":\"crud\",\"users\":\"crud\"}', 'open-audit_roles_org_admin', 'system', '2000-01-01 00:00:00')";
$this->db->query($sql);
$this->log_db($this->db->last_query());

$sql = "INSERT INTO roles VALUES (NULL, 'reporter', 'The role used for reading endpoints and creating reports above to the user role.', '{\"baselines\":\"crud\",\"charts\":\"r\",\"connections\":\"r\",\"credentials\":\"r\",\"summaries\":\"r\",\"devices\":\"r\",\"errors\":\"r\",\"fields\":\"r\",\"files\":\"r\",\"graph\":\"r\",\"invoice\":\"r\",\"licenses\":\"crud\",\"locations\":\"r\",\"networks\":\"r\",\"orgs\":\"r\",\"queries\":\"crud\",\"search\":\"crud\",\"sessions\":\"crud\"}', 'open-audit_roles_reporter', 'system', '2000-01-01 00:00:00')";
$this->db->query($sql);
$this->log_db($this->db->last_query());
...
# scripts
$sql = "SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '" . $this->db->database . "' AND `TABLE_NAME` = 'scripts'";
$query = $this->db->query($sql);
$result = $query->result();
if (count($result) === 1) {
    $sql = "SHOW COLUMNS FROM `scripts` WHERE Field = 'org_id'";
    $query = $this->db->query($sql);
    $result = $query->result();
    if (count($result) === 0) {
...
$sql = "INSERT INTO summaries VALUES (NULL, 'Server Types', 1, 'server', 'type', '', 'system', '2000-01-01 00:00:00')";
$this->db->query($sql);
$this->log_db($this->db->last_query());
...
# system
$sql = "SHOW COLUMNS FROM `system` WHERE Field = 'org_id'";
$query = $this->db->query($sql);
$result = $query->result();
if (count($result) === 0) {
...
$sql = "SHOW COLUMNS FROM `system` WHERE Field = 'location_id'";
$query = $this->db->query($sql);
$result = $query->result();
if (count($result) === 0) {
...
$this->log_db('Upgrade database to 1.14 completed');
