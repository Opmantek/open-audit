...
if (!empty($log->type) and $log->type == 'discovery') {
    ...
    if (strpos($log->command_status, ' of ') !== false) {
        ...
        $sql = "UPDATE `discoveries` SET `discovered` = ?, `last_log` = (SELECT `timestamp` FROM discovery_log WHERE `id` = ?) WHERE id = ?";
        $data = array($progress, $log_id, $log->discovery_id);
        $query = $this->db->query($sql, $data);
    }
    if (strpos($log->message, 'Completed discovery') !== false) {
        # STATUS
        $sql = "UPDATE `discoveries` SET `status` = 'complete' WHERE id = ?";
    ...
    $sql = "UPDATE discoveries SET `duration` = TIMEDIFF(last_log, last_run) WHERE id = ?";
    $data = array($log->discovery_id);
    $query = $this->db->query($sql, $data);

    $sql = "UPDATE `discoveries` SET `status` = 'zombie' WHERE `last_log` < (NOW() - INTERVAL 20 MINUTE) AND `last_log` != '2000-01-01 00:00:00' AND `status` != 'complete'";
