                ...
                if (count($result) > 0) {
                    ...
                    if (php_uname('s') !== 'Windows NT') {
                        if ($this->config->config['oae_product'] === 'Open-AudIT Cloud') {
                            $command = 'nohup ' . $this->config->config['base_path'] . '/other/execute.sh url=http://localhost/' . $this->db->database. '/open-audit/index.php/input/queue/scans/' . $id . ' method=post > /dev/null 2>&1 &';
                            @exec($command);
                        } else {
                            $command = 'nohup php ' . $this->config->config['base_path'] . '/www/open-audit/index.php input queue scans ' . $id . ' > /dev/null 2>&1 &';
                            @exec($command);
                        }
                    } else {
                        $command = "%comspec% /c start /b c:\\xampp\\php\\php.exe c:\\xampp\\htdocs\\open-audit\\index.php input queue scans " . $id;
