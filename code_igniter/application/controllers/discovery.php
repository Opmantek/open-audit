...
class Discovery extends CI_Controller {
				...
				if (isset($details->complete) and $details->complete == 'y') {
					...
					if ((isset($details->snmp_oid)) and ($details->snmp_oid > '')){
						...
						if (isset($details->system_id) and $details->system_id != '') {
							...
							if (((isset($loggedin)) OR ($this->session->userdata('logged_in') == TRUE))) { echo "DEBUG - SNMP update for " . $details->system_id . "\n"; }
							$log_details = "C:discovery F:process_subnet SNMP update for $details->man_ip_address (System ID $details->system_id)"; $this->log_event($log_details);
						} else {
					...
					if ($details->ssh_status == "true") {
						...
						if ($details->ssh_username > '' and $details->ssh_password > '') {
							#$log_details = "C:discovery F:process_subnet SSH is open on $details->man_ip_address (System ID $details->system_id)"; $this->log_event($log_details);
						} else {
							$log_details = "C:discovery F:process_subnet No credentials supplied for SSH audit for $details->man_ip_address (System ID $details->system_id)"; $this->log_event($log_details);
						}
						...
						if ($details->ssh_username > '' and $details->ssh_password > '') {
							...
							if (php_uname('s') == 'Linux') {
								...
								if ($remote_os == "Linux") {
									...
									if ($error == "" ) {
										if ($sudo > "") {
											$command_string = "sshpass -p \"" . $details->ssh_password . "\" ssh -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null " . $details->ssh_username . "@" . $details->man_ip_address . " \"echo " . $details->ssh_password . " | $sudo -S /tmp/audit_linux.sh submit_online=y create_file=n url=" . $url . "index.php/system/add_system debugging=1 \"";
										} else {
							...
							if (php_uname('s') == 'Windows NT') {
								...
								if ($remote_os == "Linux") {
									$error = "";
									$log_details = "Attempting SSH audit for discovery on $details->man_ip_address (linux)"; $this->log_event($log_details);
									...
									if ($error == "" ) {
										if ($sudo > "") {
											$command_string = "$filepath\\plink.exe -pw " . $details->ssh_password . " " . $details->ssh_username . "@" . $details->man_ip_address . " \"echo " . $details->ssh_password . " | $sudo -S /tmp/audit_linux.sh submit_online=y create_file=n url=" . $url . "index.php/system/add_system debugging=1 \"";
										} else {
											$command_string = "$filepath\\plink.exe -pw " . $details->ssh_password . " " . $details->ssh_username . "@" . $details->man_ip_address . " \"/tmp/audit_linux.sh submit_online=y create_file=n url=" . $url . "index.php/system/add_system debugging=1 \"";
										}
										exec($command_string, $output, $return_var);
										if ($return_var != '0') { $error = "SSH audit command for linux audit script on $details->man_ip_address failed"; $this->log_event($error); }
										if (((isset($loggedin)) OR ($this->session->userdata('logged_in') == TRUE))) {
	...
	function log_event($log_details, $display='y') {
		# setup the log file
		if ((php_uname('s') == 'Linux') or (php_uname('s') == 'Darwin')) {
