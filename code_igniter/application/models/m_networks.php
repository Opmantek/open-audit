...
class M_networks extends MY_Model
{
    ...
    public function upsert($network = null)
    {
        ...
        if (empty($result) OR $result === false OR count($result) == 0) {
            ...
            $sql = "INSERT INTO `networks` (id, name, network, org_id, type, description, location_id, edited_by, edited_date) VALUES (NULL, ?, ?, ?, 'Local Area Network', ?, ?, ?, NOW())";
            $data = array((string)$network->name, (string)$network->network, intval($network->org_id), (string)$description, intval($network->location_id), 'auto-generated by '.@$model.'::'.@$function);
            $result = $this->run_sql($sql, $data);
            return true;
        } else {
            return false;
