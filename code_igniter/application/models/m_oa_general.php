...
class M_oa_general extends MY_Model
{
    ...
    public function get_system_document_api_new($table = '', $system_id = '')
    {
        ...
        #    $sql = 'SELECT software_name, software_url, software_email, software_version, software_publisher, date(software_installed_on) as software_installed_on, software_installed_by FROM sys_sw_software LEFT JOIN system ON system.system_id = sys_sw_software.system_id AND system.timestamp = sys_sw_software.timestamp WHERE system.system_id = ? ';
            ...
            $sql = 'SELECT software.id, software.name, software.version, software.first_seen, software.last_seen, IF(software.first_seen = system.first_seen, "y", "n") as original_install, IF(software.last_seen = system.last_seen, "y", "n") as current_install FROM software LEFT JOIN system ON (software.system_id = system.system_id) WHERE system.system_id = ?';
        } elseif ($table == 'sys_sw_software_history_full') {
            $sql = 'SELECT software.id, software.name, software.version, software.first_seen, software.last_seen, IF(software.first_seen = system.first_seen, "y", "n") as original_install, IF(software.last_seen = system.last_seen, "y", "n") as current_install FROM software LEFT JOIN system ON (software.system_id = system.system_id) WHERE system.system_id = ? AND (system.first_seen = software.first_seen OR system.last_seen = software.last_seen)';
