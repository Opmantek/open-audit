...
class M_group extends MY_Model {
	...
	function get_system_group($system_id) {
		$sql = "SELECT 
				sys_sw_group.group_id, 
				sys_sw_group.group_name, 
				sys_sw_group.group_description,
				sys_sw_group.group_sid,
				sys_sw_group.group_members
			FROM 
				sys_sw_group,
				system
			WHERE 
				sys_sw_group.system_id = system.system_id AND
				sys_sw_group.timestamp = system.timestamp AND
				system.system_id = ?
			GROUP BY 
				sys_sw_group.group_id";
	...
	function process_group($input, $details) {
		...
		$sql = "SELECT 
			...
			WHERE 
				...
				sys_sw_group.group_name = ? AND 
				sys_sw_group.group_sid	= ? AND
				( sys_sw_group.timestamp = ? OR sys_sw_group.timestamp = ? )";
		$sql = $this->clean_sql($sql);
