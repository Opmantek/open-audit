	...
	function get_system_hard_drive($system_id) {
		$sql = "SELECT 	
			...
			FROM 	
				...
				system
			WHERE 	
				sys_hw_hard_drive.system_id = system.system_id AND 
				sys_hw_hard_drive.timestamp = system.timestamp AND 
				system.system_id = ? 
			GROUP BY 
				sys_hw_hard_drive.hard_drive_id
			ORDER BY 
				sys_hw_hard_drive.hard_drive_index ";
	...
	function process_hard_drive($input, $details) {
		...
		$sql = "
			...
			WHERE 
				...
				hard_drive_model = ? AND 
				hard_drive_serial = ? AND 
				hard_drive_index = ? AND 
				hard_drive_size = ? AND 
				( sys_hw_hard_drive.timestamp = ? OR sys_hw_hard_drive.timestamp = ? ) 
			LIMIT 1";
		$sql = $this->clean_sql($sql);
		$data = array("$details->system_id", 
				"$input->hard_drive_model", 
				"$input->hard_drive_serial", 
				"$input->hard_drive_index", 
				"$input->hard_drive_size", 
		...
		if ($query->num_rows() > 0) {
			...
			$sql = "INSERT INTO sys_hw_hard_drive (	system_id, 
										hard_drive_caption, 
										hard_drive_index, 
			...
			$sql = $this->clean_sql($sql);
			$data = array("$details->system_id", 
					"$input->hard_drive_caption", 
					"$input->hard_drive_index", 
					mb_strtoupper($input->hard_drive_interface_type), 
					"$input->hard_drive_manufacturer", 
					"$input->hard_drive_model", 
					"$input->hard_drive_serial", 
					"$input->hard_drive_size", 
					"$input->hard_drive_device_id", 
					"$input->hard_drive_scsi_logical_unit", 
					"$input->hard_drive_status", 
					"$details->timestamp", 
					"$details->timestamp");
			$query = $this->db->query($sql, $data);
		}
