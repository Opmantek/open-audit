...
class M_applications extends MY_Model
{
    ...
    */
    ...
    public function read_sub_resource($id = 0)
    {
        ...
        $sql = 'SELECT system.id AS `system.id`, system.name AS `system.name`, system.ip AS `system.ip`, `system`.`description` AS `system.description`, application.id AS `application.id` FROM `application` LEFT JOIN `system` ON `application`.`system_id` = `system`.`id` WHERE `application`.`applications_id` = ? and `system`.`org_id` IN  (' . $org_ids . ')';
    ...
    public function count()
    {
        ...
        $sql = 'SELECT COUNT(id) AS `count` FROM applications WHERE org_id IN (' . implode(',', $org_list) . ')';
