...
if (!function_exists('response_create')) {
    ...
    function response_create()
    {
        ...
        // no dependencies - set in GET or POST or HEADERS
        $response->meta->current = response_get_current($instance->input->get('current'), $instance->input->post('current'));
        ...
        $response->meta->id = response_get_id(
            html_entity_decode(urldecode($instance->uri->segment(2))),
            $response->meta->collection,
        ...
        $response->meta->sub_resource = response_get_sub_resource(
            $instance->input->get('sub_resource'),
        ...
        $response->meta->sort = response_get_sort(
            $response->meta->collection,
            $instance->input->get('sort'),
            $instance->input->post('sort')
        );
...
if (!function_exists('response_valid_includes')) {
    ...
    {
        return array('application', 'attachment', 'audit_log', 'bios', 'change_log', 'cluster', 'credential', 'discovery_log', 'disk', 'dns', 'edit_log', 'field', 'file', 'image', 'ip', 'location', 'log', 'memory', 'module', 'monitor', 'motherboard', 'netstat', 'network', 'nmap', 'optical', 'pagefile', 'partition', 'policy', 'print_queue', 'processor', 'purchase', 'rack_devices', 'radio', 'route', 'san', 'scsi', 'server', 'server_item', 'service', 'share', 'software', 'software_key', 'sound', 'task', 'usb', 'user', 'user_group', 'variable', 'video', 'vm', 'windows');
    }
}
...
if (!function_exists('response_valid_sub_resources')) {
    ...
    {
        return array('application', 'attachment', 'audit_log', 'bios', 'change_log', 'cluster', 'credential', 'discovery', 'discovery_log', 'disk', 'dns', 'edit_log', 'field', 'image', 'ip', 'log', 'memory', 'module', 'monitor', 'motherboard', 'netstat', 'network', 'nmap', 'optical', 'pagefile', 'partition', 'partition_graph', 'policy', 'print_queue', 'processor', 'radio', 'route', 'server', 'server_item', 'service', 'share', 'software', 'software_key', 'sound', 'task', 'usb', 'user', 'user_group', 'variable', 'video', 'vm', 'windows', 'report', 'query', 'group');
    }
}
