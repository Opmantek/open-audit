...
$get_oid_details = function ($details) {
    ...
    if ($details->snmp_version == '2') {
        # serial
        $details->serial = snmp_clean(@snmp2_get($details->ip, $details->snmp_community, "1.3.6.1.4.1.43.47.1.1.3.1.10.1"));

        if ($details->serial == '') {
            $details->serial = snmp_clean(@snmp2_get($details->ip, $details->snmp_community, "1.3.6.1.4.1.43.10.27.1.1.1.13.1"));
        }
    }

    if ($details->snmp_version == '1') {
        # serial
        $details->serial = snmp_clean(@snmpget($details->ip, $details->snmp_community, "1.3.6.1.4.1.43.47.1.1.3.1.10.1"));

        if ($details->serial == '') {
            $details->serial = snmp_clean(@snmpget($details->ip, $details->snmp_community, "1.3.6.1.4.1.43.10.27.1.1.1.13.1"));
        }
    }
};
