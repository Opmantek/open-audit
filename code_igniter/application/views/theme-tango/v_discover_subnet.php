...
<fieldset id="subnet_details" class="niceforms">
    ...
    ?>
				<p><label for='ssh_username'><?php echo __("SSH User");
    ?>: </label> <input type='text' id='ssh_username' name='ssh_username' tabindex='4' title='SSH User' value='<?php echo $ssh_username;
    ?>' /></p>
				<p><label for='ssh_password'><?php echo __("SSH Password");
    ?>: </label> <input type='<?php echo $password_field;
    ?>' id='ssh_password' name='ssh_password' tabindex='5' title='SSH Password' value='<?php echo $ssh_password;
    ?>' /></p>
				<?php 
} ?>
    ...
    ?>
				<p><label for='windows_username'><?php echo __("Windows User");
    ?>: </label> <input type='text' id='windows_username' name='windows_username' tabindex='6' title='Windows User' value='<?php echo $windows_username;
    ?>' /></p>
				<p><label for='windows_password'><?php echo __("Windows Password");
    ?>: </label> <input type='<?php echo $password_field;
    ?>' id='windows_password' name='windows_password' tabindex='7' title='Windows Password' value='<?php echo $windows_password;
    ?>' /></p>
				<p><label for='windows_domain'><?php echo __("Windows Domain");
    ?>: </label> <input type='text' id='windows_domain' name='windows_domain' tabindex='8' title='Windows Domain' value='<?php echo $windows_domain;
    ?>' /></p>
				<?php 
} ?>
				...
				<?php if ($type == "" or $type == "device" or $type == "linux" or (php_uname('s') == "Linux" and $type == "windows")) {
    ?>* The ip address or resolvable hostname used by external devices to talk to Open-AudIT (This should be the real IP Address of this Open-AudIT server).<br /><?php 
} ?>

				<?php if ($type > "") {
    ?><p>** Do not set "debug" in normal use. Doing so may take a while for the page to return - please be patient.</p><?php 
} ?>
...
if ($type == '') {
    if ($this->config->config['default_snmp_community'] == '' or
        ...
        ?>
	<p><h3 style="color: blue">Warning</h3>Please ensure you have default for ALL attributes. Defaults can be set on the Config page - click <a href="<?php echo $oa_web_index;
        ?>/admin/edit_config" style="color: red";>here</a>.<br />
	<?php 
    }
    ?>
