...
$sql = "INSERT INTO `configuration` VALUES (NULL,'feature_news_url','https://news.firstwave.com/news','text','y','system','2000-01-01 00:00:00','The URL to connect to, to retrieve news articles.')";
$db->query($sql);
$output .= str_replace("\n", " ", (string)$db->getLastQuery()) . "\n\n";
log_message('info', (string)$db->getLastQuery());
...
$sql = "INSERT INTO `tasks` VALUES (null, 'Vulnerability Retrieval', 1, 'Retrieve an updated vulnerability list.', 0, '', 'y', 'vulnerabilities', (SELECT MINUTE(NOW() + INTERVAL 5 MINUTE)), '*', '*', '*', '*', 0, 0, '2000-01-01 00:00:00', '2000-01-01 00:00:00', '', '', 0, 'system', '2000-01-01 00:00:00')";
$result = $db->query($sql);
$output .= str_replace("\n", " ", (string)$db->getLastQuery()) . "\n\n";
log_message('info', (string)$db->getLastQuery());
...
$sql = "INSERT INTO `widgets` VALUES (null,'Other Computers',1,'Not Windows, Apple or Linux computers','status','','SELECT COUNT(devices.id) AS `count` FROM devices WHERE @filter AND devices.os_name NOT LIKE \'%Windows%\' AND devices.os_name NOT LIKE \'%MacOS%\' AND devices.os_group != \'Linux\' AND type = \'computer\'','devices?devices.os_name=NOT LIKEwindows&devices.os_name=NOT LIKEMacOS&devices.os_group=!=Linux','icon-database','Computers not running Windows or MacOS.','Other Computers','','','','','',0,'',0,0,0,0,'SELECT CONCAT(IF(COUNT(id) > 0, CONCAT(\'+\', COUNT(id), \' this week.\'), \'\')) AS `secondary_text` FROM devices WHERE @filter AND devices.os_name NOT LIKE \'%Windows%\' AND devices.os_name NOT LIKE \'%MacOS%\' AND devices.os_group != \'Linux\' AND first_seen > DATE(NOW() - INTERVAL 7 DAY) AND type = \'computer\'','primary','primary',0,0,'system','2000-01-01 00:00:00')";
$result = $db->query($sql);
$output .= str_replace("\n", " ", (string)$db->getLastQuery()) . "\n\n";
log_message('info', (string)$db->getLastQuery());
