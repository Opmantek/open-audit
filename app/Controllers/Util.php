...
class Util extends Controller
{
    ...
    public function amazon()
    {
        ...
        $cloud = $this->request->getPost('cloud');
        if (empty($cloud)) {
            log_message('error', 'A request for the Amazon API was received, but no cloud data was present in the POST.');
            $response->errors = 'A request for the Amazon API was received, but no cloud data was present in the POST.';
    ...
    public function microsoft()
    {
        ...
        $cloud = $this->request->getPost('cloud');
        if (empty($cloud)) {
            log_message('error', 'A request for the Microsoft API was received, but no cloud data was present in the POST.');
            $response->errors = 'A request for the Microsoft API was received, but no cloud data was present in the POST.';
            $response->meta->header = 400;
            $response->setStatusCode($response->meta->header);
            print_r(json_encode($response));
            return;
    ...
    public function google()
    {
        ...
        $cloud = $this->request->getPost('cloud');
        if (empty($cloud)) {
            log_message('error', 'A request for the Google API was received, but no cloud data was present in the POST.');
            $response->errors = 'A request for the Google API was received, but no cloud data was present in the POST.';
            $response->meta->header = 400;
            $response->setStatusCode($response->meta->header);
            print_r(json_encode($response));
            return;
        ...
        $jsonKey = json_decode($cloud, true);
