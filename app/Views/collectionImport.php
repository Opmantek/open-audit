...
if ($collection === "queries") {
    $sample = ' <tr><td>"name","org_id","menu_display","menu_category",sql"</td></tr>
                <tr><td>"Linux Device Hardware","1","y","Hardware",SELECT system.id AS `system.id`, system.icon AS `system.icon`, system.type AS `system.type`, system.name AS `system.name`, system.domain AS `system.domain`, system.ip AS `system.ip`, system.manufacturer AS `system.manufacturer`, system.model AS `system.model`, system.serial AS `system.serial`, system.os_family AS `system.os_family`, system.memory_count AS `system.memory_count`, system.form_factor AS `system.form_factor`, processor.description AS `processor.description` FROM system LEFT JOIN processor ON (processor.system_id = system.id AND processor.current = \'y\') WHERE @filter AND system.os_group = \'Linux\' ORDER BY system.name"</td></tr>
...
if ($collection === "tasks") {
    $sample = ' <tr><td style="word-wrap: break-word;min-width: 160px;max-width: 160px;">"name","org_id","description","enabled","type","sub_resource_id","first_run","minute","hour","day_of_month","month","day_of_week","uuid","options"</td></tr>
                <tr><td style="word-wrap: break-word;min-width: 160px;max-width: 160px;">"My Test Discovery","1","This is a test for discovering my local subnet each weekday at 10:05 am.","y","discoveries","1","2017-06-01","5","10","*","*","1,2,3,4,5","' . HTML::Entities::encode($self->config_item('uuid')) . '"</td></tr>
                <tr><td style="word-wrap: break-word;min-width: 160px;max-width: 160px;">"New Devices","1","Email me the new devices report each week.","y","reports","10002","2017-06-01","5","9","*","*","5","' . HTML::Entities::encode($self->config_item('uuid')) . '","{\'email_address\':\'my.email.address@open-audit.com\',\'format\':\'csv\'}"</td></tr>';
}
