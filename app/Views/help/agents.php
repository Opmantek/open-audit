...
$intro = '<p>' . __('Sometimes agentless discovery just doesn\'t fit your use-case. Maybe your PCs have tight firewalls. Maybe they\'re not on the network when your discoveries are scheduled. Maybe they\'re used by remote workers. Whatever the case, this is where an agent can help. Install it on the target machine and it will \'check-in\' with your Open-AudIT server on a schedule and accept tasks. The most common task the server requests is for the agent to audit itself and send the result.') . '</p>
    ...
    <p>Enabling advanced agents means an agent can be instructed to download any file and execute any command. <strong>WARNING</strong>: This comes with security risks. This configuration will <strong>only</strong> function when running Open-AudIT using HTTPS and a valid (Certificate Authority provided) certificate for ') . $uri->getHost() . __('. To enable this, edit the configuration item for ') . '<a href="' . url_to('configurationRead', 'feature_agents_advanced') . '">feature_agents_advanced</a>.</p>';

$body = '<p>' . __('We have tests for:') . '
    <ul>
    <li><strong>' . __('Minutes') . '</strong>: ' . __('If this many minutes (or more) have passed since the server last saw the device, pass.</li>') . '
    <li><strong>' . __('Subnet') . '</strong>: ' . __('If the primary IP (as reported by the device) is in this subnet, pass.</li>') . '
    <li><strong>' . __('OS') . '</strong>: ' . __('If the OS Family (as reported by the device) contains this item, pass.</li>') . '
    </ul>
    <br>' . __('We also have our action items (which are executed in the order below):') . '<br><br>
    <ul>
    <li><strong>' . __('Download a File') . '</strong>: ' . __('A URL provided to the agent which will then download the file and store it on the local disk.') . '</li>
    <li><strong>' . __('Run a Command') . '</strong>: ' . __('A command to run. When the agent is Windows based, this command is run from within the powershell agent.') . '</li>
    <li><strong>' . __('Assign the Device to a Location') . '</strong>: ' . __('Any discovered devices will be assigned to this Location when they run their audit script (if set).') . '</li>
    <li><strong>' . __('Assign the device to an Org') . '</strong>: ' . __('Any devices will be assigned to this Org when they run their audit script (if set).') . '</li>
    <li><strong>' . __('Audit') . '</strong>: ' . __('The agent should download a fresh copy of the audit script, run it and submit the result to the server.') . '</li>
    <li><strong>' . __('Uninstall') . '</strong>: ' . __('After completing the above, uninstall the agent.') . '</li>
    </ul></p>
    ...
    <br>
    <h2>' . __('Installing') . '</h2>
    <p>' . __('Have your user open a command prompt (using \'Run as Administrator\'). Paste the below into the prompt to download the Agent.') . '<br>
