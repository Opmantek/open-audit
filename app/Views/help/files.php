...
<p>' . __('When a device is Discovered the audit script will be injected with the file (or directory) details. Both audit_linux.sh and audit_windows.vbs will be populated, regardless of the file path. If you\'re thinking "but putting a Linux path into a Windows machine will break things!", it won\'t break, you just won\'t receive any data from that file or directory entry.<br>
    ...
    You will see a section in both the Windows and Linux audit scripts as below') . ':<br><br>
...
<br>' . __('If you would like to use the script outside of Discovery we have created another endpoint called "scripts". See the documentation here') . ' - <a href="' . url_to('scriptsHelp') . '">Scripts</a>.</p>';

$body = '<br>

<h2>' . __('Enabling the Feature Under Windows') . '</h2>

<p>' . __('There is no need to do anything if you\'re running Open-AudIT on a Linux server.<br>

<br>Windows clients are just fine and require no special actions, however.... to enable this feature the audit script must be run locally on the target Windows system. It cannot be run remotely as we do with WMI calls when running the audit script on one Windows machine, while targeting a second Windows machine. To do this we need to copy the audit script to the target Windows machine and then run it. Unfortunately the service account that Apache runs under is the Local System account. This account has no access to remote (network based) resources. To work around this issue the service must be run under another account. It is easiest to just use the local Administrator account, but you can try any account you like as long as it has the required privileges. The Local System account has as much local access as the local Administrator account.') . '<br>

<br>' . __('See our page on enabling') . ' <a href="' . url_to('helpFAQ') . '?name=Running Open-AudIT Apache Service Under Windows">' . __('Running Open-AudIT Apache Service Under Windows') . '</a></p><br>';
