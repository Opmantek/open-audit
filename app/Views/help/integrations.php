...
<br>' . __('If you wish to dig in and customize the Integration, click the Advanced button (and try not to be overwhelmed!).') . '<br>

<br>' . __('You can customize the entire integration down to which system (NMIS or Open-AudIT) is the authority on a per field basis. There are options to create and/or update devices in Open-AudIT or NMIS. Should we run a discovery on a created or updated device and so much more. There are explanations for each item on the right side of the page.') . '<br>

<br>' . __('You can select the devices to be integrated by choosing any attribute in the system (device) table and matching (see menu → Admin → Database → List Tables → system for a list of fields). An attribute match can only match a single value.') . '<br>
...
<br>' . __('Our default Integration also enables a discovery on any externally supplied devices. So we ship a default discovery for the Default Integration out of the box. These use the default matching rules, but also enable matching on IP address.') . '<br>

<br>' . __('Integrations can be scheduled as per Discoveries, Queries, et al.') . '</p>

<br>
<h2>' . __('Fields') . '</h2>

<p>' . __('The fields associated with an Integration are stored in the database as a JSON array of objects. Each field has the following attributes') . ':<br><br>

<code>internal_field_name</code>: ' . __('This should be the fully qualified field name in the database. Currently columns from the system, fields and credentials tables are supported. EG: system.nmis_role.') . '<br><br>

<code>external_field_name</code>: ' . __('The fully qualified name returned by the external system. EG: for NMIS we use configuration.roleType.') . '<br><br>
...
<code>external_field_type</code>: ' . __('When we are converting an internally selected device to an object for the external system, what should the external data be. EG: integer, string, etc.') . '<br>
</p>

<br>
<h2>' . __('External Field Types') . '</h2>

<br><code>text</code>: a text field<br>
<br><code>integer</code>: a number<br>
<br><code>bool</code>: ' . __('true or false (in JSON context)') . '<br>
<br><code>bool_one_zero</code>: ' . __('If the local field is true (bool) or y (string) or Y (string) or 1 (integer) then set the external data to 1.') . '<br>
<br><code>bool_y_n</code>: ' . __('If the local field is true (bool) or y (string) or Y (string) or 1 (integer) then set the external data to y.') . '<br>
...
<br>
' . __('NOTE - For more info on DateTime formats, see') . ' <a href="https://www.php.net/manual/en/datetime.createfromformat.php" target="_blank">https://www.php.net/manual/en/datetime.createfromformat.php</a>.</p>

<br>
<h2>' . __('Troubleshooting') . '</h2>
<p>' . __('After an Integration has been created, when you view the details of an Integration you will notice a field named Debug. This is set to no by default. Changing it to yes provides extra logging when an Integration is executed. Debug should not be left as yes. This is because the debug output will contain everything retrieved from external systems - including items such as WMI and SNMP credentials. The debug option should be used only when an issue has occurred and you wish to dig deeper in to see if you can determine why.') . '</p>
<br>';
