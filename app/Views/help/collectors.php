...
$body = '<h2>' . __('Notes') . '</h2>
<br>
<p>' . __('Collector / Server is designed so that you can have a remote or \'collector\'  instance of Open-AudIT running on a remote machine. That collector instance might be in another security zone, subnet, cloud hosting instance or at a customer site.  The remote collector is then doing all the discovering and auditing of devices locally avoiding networking and firewalling complexity.  The Collector once it has registered is completely controlled by the \'server\'.  This means you only need an HTTP or HTTPS connection from that Collector to the Server.') . '<br>
<br>
