...
<br>
<p>' . __('Devices will not be matched if their status is set to "deleted". Any other status will allow a match to occur.') . '</p>
<br>
...
<ul>
    ...
    <li>Serial - ' . __('It is very common for second tier motherboard manufacturers to not set this, to set it to all 0\'s or even all F\'s.') . '</li>
...
<br>
' . __('If you don\'t normally set devices_assigned_to_org, then it will have no effect. We only check using the OrgID if it has been set in discovery (or manually in an audit script) and enabled in the configuration.') . '<br>
<br>
