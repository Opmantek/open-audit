...
$GLOBALS["lang"]['And provide a name - we humbly recommend Open-AudIT &#128512;, but you can name it anything you like. Make sure the option for <i>Integrate any other application you do not find in the gallery (Non-gallery)</i> is selected. And then click <i>Create<i>.'] = 'І надати ім\'я - ми рекомендуємо Open-AudIT 😀, але ви можете назвати це все, що вам подобається. Переконайтеся, що варіант для <i>Інтегрувати будь-яку іншу програму, яку ви не знайдете в галереї (Нон-галерея)</i> вибрано. А потім натисніть <i>Спілкування<i>й</i></i>';
...
$GLOBALS["lang"]['If a new Org is created, an LDAP group name is automatically derived from the name. For example, if a new Org is created and is named Test, the corresponding LDAP group will be named open-audit_orgs_test.'] = 'Якщо створюється новий Org, ім\'я групи LDAP автоматично виходить з назви. Наприклад, якщо створюється новий Org і названий тест, відповідна група LDAP буде названа відкрито-audit_orgs_test.';
...
$GLOBALS["lang"]['If no operator is specified, the default is =. Properties should be fully qualified - ie, devices.hostname (not just hostname).'] = 'Якщо оператор не вказаний, за замовчуванням =. Властивості повинні бути повністю кваліфікованими - тобто, пристрої.hostname (не просто ім\'я).';
...
$GLOBALS["lang"]['If the OS Family (as reported by the device) contains this item, pass.</li>'] = 'Якщо ОС Сім\'я (як повідомляється пристроєм) містить цей пункт, пропуск.';
...
$GLOBALS["lang"]['If you are on Windows or your NMIS server is remote, you will need to supply the URL, username and password to access that server.'] = 'Якщо ви перебуваєте на Windows або сервері NMIS, вам потрібно буде надати URL, ім\'я користувача та пароль для доступу до цього сервера.';
...
$GLOBALS["lang"]['Location Name'] = 'Ім\'я користувача';
...
$GLOBALS["lang"]['Name: The name you are giving to this field. Ideally it should be unique.'] = 'Назва: Ім\'я, яку ви надаєте цій галузі. В ідеалі вона повинна бути унікальною.';
...
$GLOBALS["lang"]['On the field <i>Match String</i> you should provide the name of the software that you want to track. You can use the percent sign (%) as a wildcard in where required.'] = 'На поле <i>Стрінг матчу</i> Вам необхідно надати ім\'я програмного забезпечення, яке ви хочете відстежувати. Ви можете використовувати знак відсотка (%) як дикийкарт, в якому потрібно.';
...
$GLOBALS["lang"]['Pie charts must return name or my_name, description or my_description and count.'] = 'Пиріжки повинні повернути ім\'я або ім\'я, опис або my_description і кількість.';
...
$GLOBALS["lang"]['Should we convert the hostname to lowercase.'] = 'Якщо ми перетворимо ім\'я в нижній регістр.';
...
$GLOBALS["lang"]['The API uses a cookie. You can request a cookie by sending a POST to the URL below, containing the username and password attributes and values:'] = 'API використовує cookie. Ви можете запросити cookie, відправивши POST на URL нижче, що містить ім\'я користувача та атрибути пароля та значення:';
...
$GLOBALS["lang"]['The SELECT section of the query should use full dot notation and also request the field with its full dot name. IE - SELECT devices.id AS `devices.id`. Each field should be selected in this fashion to enable GUI side attribute filtering.'] = 'СЕЛЕКТРОННИЙ розділ запиту має використовувати повне позначення, а також запитувати поле з його повним ім\'ям. IE - SELECT пристрої. id AS `devices.id`. Кожне поле слід вибрати в цій моді, щоб увімкнути фільтрацію атрибутів GUI.';
...
$GLOBALS["lang"]['The only required permission / policy name (in our testing) is'] = 'Єдиний необхідний дозвіл / ім\'я політики (у нашому тестуванні)';
...
$GLOBALS["lang"]['The requested properties should be in a comma-separated list. Properties should be fully qualified - ie, system.hostname (not just hostname).'] = 'Запропоновані властивості повинні бути в окремому списку. Властивості повинні бути повністю кваліфікованими - тобто, система.hostname (не просто ім\'я).';
...
$GLOBALS["lang"]['The username (if not using the user running the script).'] = 'Ім\'я користувача';
...
$GLOBALS["lang"]['Then create a new Auth method in Open-AudIT by going to menu -> Admin -> Auth Methods -> Create Auth. Provide a name and set the <i>type</i> to Entra.'] = 'Далі створіть новий метод Auth у Open-AudIT, перейшовши до меню -> адмін -> Аутто методи -> Створити авт. Надайте ім\'я та встановіть ім\'я <i>Тип</i> до Ентра.';
...
$GLOBALS["lang"]['To retrieve the Tenant ID. Open the Azure portal click on Azure Active Directory, Properties and the Tenant ID is displayed as your Directory ID - do not blame me for the name mismatch, that is how Microsoft roll.'] = 'Для отримання Tenant ID. Відкрийте портал Azure натисніть на Azure Active Directory, властивості та ідентифікатор Tenant відображається як ідентифікатор каталогу - не відмивайте мене за ім\'я mismatch, тобто як Microsoft рулон.';

$GLOBALS["lang"]['To retrieve your key, log in to the AWS Console and select your username, then My Security Credentials.'] = 'Щоб отримати ключ, ввійдіть в Консоль AWS і виберіть ім\'я користувача, після чого My Security Credentials.';
...
$GLOBALS["lang"]['To use this Enterprise feature, you first need to create your user in Open-AudIT as per usual. Assigning a password is not necessary. You do have to assign Roles and Orgs. The username in Open-AudIT should match the Entra <i>preferred username</i> attribute. There is no need for a Full Name or Email - these will be populated from Entra. If you do not know your users <i>preferred username</i>, fear not. You can create the new Auth Method in Open-AudIT and the Enterprise Application in Entra and when a user attempts to sign-on to Open-AudIT without a pre-existing user, check the logs and you will see their <i>preferred username</i> logged for your convienence.'] = 'Щоб використовувати цю функцію Enterprise, вам спочатку потрібно створити свій користувач у Open-AudIT у звичайному режимі. Присвоєння паролю не потрібно. Ви повинні призначити Ролі та оргії. Ім\'я користувача в Open-AudIT повинен відповідати Entra <i>Ім\'я користувача</i> атрибут. Не потрібно для повного імені або електронної пошти - це буде заселений з Entra. Якщо ви не знаєте користувачів <i>Ім\'я користувача</i>не боятися. Ви можете створити новий метод Auth у Open-AudIT і додатку Enterprise в Entra і коли користувач намагається зареєструватися на Open-AudIT без попереднього користувача, перевірити журнали і ви побачите їх <i>Ім\'я користувача</i> Зареєструватися на вашу конвенція.';

$GLOBALS["lang"]['To use this Enterprise feature, you first need to create your user in Open-AudIT as per usual. Assigning a password is not necessary. You do have to assign Roles and Orgs. The username in Open-AudIT should match the OKTA <i>name</i> attribute. There is no need for a Full Name or Email - these will be populated from OKTA. If you do not know your users <i>name<i>, fear not. You can create the new Auth Menthod in Open-AudIT and the Application in OKTA and when a user attempts to sign-on to Open-AudIT without a pre-existing user, check the logs and you will see their <i>name<i> logged for your convienence.<br></p><p>Then create a new Auth method in Open-AudIT by going to menu -> Admin -> Auth Methods -> Create Auth. Provide a name and set the <i>type</i> to Okta.'] = 'Щоб використовувати цю функцію Enterprise, вам спочатку потрібно створити свій користувач у Open-AudIT у звичайному режимі. Присвоєння паролю не потрібно. Ви повинні призначити Ролі та оргії. Ім\'я користувача в Open-AudIT повинен відповідати OKTA <i>Ім\'я</i> атрибут. Не потрібно для повного імені або електронної пошти - це буде заселений з OKTA. Якщо ви не знаєте користувачів <i>Ім\'я<i>не боятися. Ви можете створити новий аут Ментод в Open-AudIT і додаток в OKTA і коли користувач намагається зареєструватися на Open-AudIT без попереднього користувача, перевірити журнали і ви побачите їх <i>Ім\'я<i> Зареєструватися на вашу конвенція.<br/><p>Далі створіть новий метод Auth у Open-AudIT, перейшовши до меню -> адмін -> Аутто методи -> Створити авт. Надайте ім\'я та встановіть ім\'я <i>Тип</i> до Okta.</p></i></i></i></i>';
...
$GLOBALS["lang"]['Use a <i>Sign-in method</i> of OIDC - OpenID Connect and an <i>Application Type</i> of Web Application. Click <i>Next</i>.'] = 'Використання <i>Ім\'я користувача</i> OIDC - Відкрите підключення та підключення <i>Тип програми</i> Веб-додаток. Зареєструватися <i>Про нас</i>й';
...
$GLOBALS["lang"]['Your Host is'] = 'Ваше ім\'я Хост';
