...
$GLOBALS["lang"][', of which we supply over thirty as a default set. Professional users cannot alter these, but an Enterprise user has complete customisation abilities - including creating your own.'] = 'De kiu ni maksas pli ol tridek kiel dea... Profesiaj uzantoj ne povas ршенный tiujn, sed Enterprise uzanto havas kompletajn specialadaptitajn kapablojn - inkluzive de kreado de via propra.';

$GLOBALS["lang"][', you <i>must</i> be using https on your Open-AudIT server to use Entra for Auth. Please configure Apache to use <strong>https</strong> before configuring Entra for auth.'] = '- Oui. <i>Dévas</i> Uzi https sur via Open-Audit-servilo por uzi Entra por Auth. Bonvolu agordi a recomendaro por uzi <strong>Htps</strong> Entra por Auth.';

$GLOBALS["lang"]['. To enable this, edit the configuration item for '] = '. Por ebligation, redakki la konfiguracion objectkto por ';

$GLOBALS["lang"]['10 Minutes'] = '10 minutia';

$GLOBALS["lang"]['15 Minutes'] = '15 minutia';

$GLOBALS["lang"]['30 Minutes'] = '30 minuties';

$GLOBALS["lang"]['5 Minutes'] = '5 minutia';
...
$GLOBALS["lang"]['<br> <strong>NOTE</strong> - You are accessing this URL from the local Open-AudIT server. The downloaded script will not be able to submit when run on any other machine. If you need to audit other machines, please download the script from any remote machine, not using a browser on the Open-AudIT server itself.'] = '<br/><strong>Ei</strong> Vi Aliris URL de la loka Open- Servilo Audit. La el apretutita manuskripto ne povas submeti Se vi bezonas revizii alphajn ma apretinojn, bonvolu el apretuti la manuskripton de iu malproksima ma divonaino, ne uzante retumonilon sur la Open-Audit-servilo mem.';

$GLOBALS["lang"]['<p></p>'] = '<p></p>';

$GLOBALS["lang"]['<p><strong class="text-danger">NOTE</strong> - You must have working SSH or SSH Key credentials to execute benchmarks upon a target device.</p>'] = '<p><strong class="text-danger">Ei</strong> Vivavenda</p>';

$GLOBALS["lang"]['<p><strong>Baseline</strong> - The overarching document that contains the baseline definition and the individual policy tests.<br> <br> <strong>Policies</strong> - The individual tests contained within a Baseline. Each test is for a specific item. An example would be testing for SSH version 1.2.3.<br> <br> <strong>Details</strong> - Baselines can compare netstat ports, users and software.<br> <br> <strong>Software</strong> - To compare software we check the name and version. Because version numbers are not all standardised in format, when we receive an audit result we create a new attribute called software_padded which we store in the database along with the rest of the software details for each package. For this reason, baselines using software policies will not work when run against a device that has not been audited by 1.10 (at least). Software policies can test against the version being "equal to", "greater than" or "equal to or greater than".<br> <br> <strong>Netstat Ports</strong> - Netstat Ports use a combination of port number, protocol and program. If all are present the policy passes.<br> <br> <strong>Users</strong> - Users work similar to Netstat Ports. If a user exists with a matching name, status and password details (changeable, expires, required) then the policy passes.<br> <br> </p>'] = '<p><strong>BazlinioNom de la ville (facultatif, sans doute pas besoin d\'une traduction)</strong> La superkaddokumento kiu enhavas la bazliniodifinon kaj la indifuajn strategitestojn.<br/> <br/> <strong>Politique</strong> La ininfutuaj testoj enhavitaj ene de Bazlinio. Mina sintoiu testo estas por specifa objectkto. Ekzemplo estus testata pri SSH-versio 1.2.3.<br/> <br/> <strong>Detaloj</strong> Bazlinioj povas kompari retstat hasnojn, uzantojn kaj softvaron.<br/> <br/> <strong>Mousseline</strong> Por kompari softvaron ni kontrolas la nomon kaj versioon. Identificar versio nombroj ne estas Tial, bazlinioj uzantaj softvarpolitikojn ne funkcias dum kuro kontra epäilty aparato kiu ne estis reviziita per 1.10 (almena kaptlus). Softvarpolitikoj povas testi kontraы la versio estanta "egalaj al", "pli granda ol" kaptus "egala al a defect pli granda ol."<br/> <br/> <strong>Puertos Netstat</strong> Netstat Ports uzas kombina Se <br/> <br/> <strong>L\'Union européenne</strong> Uzantoj laboras similaj al Netstat Puertos. Se uzanto eksistas kun egala nomo, stato kaj pasvorton detaloj (publikaan progressiveblaj, eksvalidias, postulis) tiam la politiko pasas.<br/> <br/> </p>';
...
$GLOBALS["lang"]['<p>A network is derived from discovery entries and device attributes.<br> <br></p>'] = '<p>Reto estas definita de eltrova.o kontribouj kaj aparato atributas.<br/> <br/></p>';

$GLOBALS["lang"]['<p>A summary will display a list of items, grouped by the distinct values of the attribute specified by the <code>table</code> and <code>column</code> attributes.<br> <br>When a summary is executed, the result will be a list of distinct values for that <code>table</code> and <code>column</code>. There will be links on the values that allow the user to see the matching devices.<br> <br>If the attribute of <code>extra_columns</code> is populated, the resulting page will contain these columns in addition to the standard device columns.<br> <br></p>'] = '<p>Resumo montros Listn de eroj, grupigitaj per la apartaj valoroj de la atributo precizigita per la <code>table</code> kaj <code>column</code> de l\'aide.<br/> <br/>Kiam resumo estas efekkivigita, la rezulto estos lista de apartaj valoroj portio. <code>table</code> kaj <code>column</code>. Eksistas ligiloj sur la Valoroj kiuj permesas al la uzanto vidi la egalajn aparatojn.<br/> <br/>La contribution <code>extra_columns</code> La rezulta pa<br/> <br/></p>';

$GLOBALS["lang"]['<p>Agents let you audit PCs without a discovery. Install the agent and it will check-in with the server each day and audit itself. It does not matter if your computers are firewalled, audit data will still appear in Open-AudIT.</p><p>When testing <strong>if</strong> an agent should perform actions, all three tests must pass (if the test is set). <strong>Then</strong> the actions are taken.</p>'] = '<p>Agentoj lasas vin a transfugedi Komputilojn sen malkovro. Instali la agenton kaj εi kontrolus kun la servilo Ne gravas se viaj komputiloj estas pare-feu, revizio datumoj ankora</p><p>Testas de Kiam oni <strong>Se</strong> Agento devus elfari agojn, <strong>Tiam la</strong> La agoj estas prenitaj.</p>';
...
$GLOBALS["lang"]['<p>Define your racks and assign devices into their positions. You already have your devices in Open-AudIT and you know your locations. Open-AudIT extends this to allow you to create a rack and assign devices to it. Open-AudIT will even provide a visualisation of the rack and the devices contained within. If you provide a device picture, that picture will be used in the visualisation. You can look at the rack on the screen and see the same items you would see if you were standing in front of it.<br> <br></p>'] = '<p>Difini viajn rakojn kaj asagi aparatojn en iliajn poziciojn. Vijam havas viajn aparatojn en Open-Audit kaj vi scias viajn lokojn. Audit etendas tion por permesi al vi krei rakon kaj asigni aparatojn al εi. Open-Audit e Se vi disponigas aparatobildon, tiu bildo esto uzita en la bildigo. Vi povas rigardi la rakon sur la ekrano kaj vidi la samajn erojn vidus se vi staris anta. εi.<br/> <br/></p>';

$GLOBALS["lang"]['<p>Devices on your network need to be managed. But how do you keep your records up to date? A spreadsheet - defintley not. That will be out of date in hours, if not days. Why manually try to keep up. Use Open-AudIT to automatically scan your networks and record your devices - manufacturer, model, serial and more than 100 other attributes. Full lists of software, services, disks, open ports, users, etc. Automatically see if an attribute has been added, removed or changed.<br> <br>Once Open-AudIT is setup, you can sit back and relax. Have change reports emailed to you on a schedule, for example - what new devices did we discover this week? What new software was installed this week? Were there any hardware changes last month?<br> <br>Expand on the stored fields easily with your own custom attributes.<br> <br>Even add devices that are not connected to your network or those devices your Open-AudIT server cannot reach.<br> <br>Computers, switches, routers, printers or any other device on your network - Open-AudIT can audit them all.<br> <br></p>'] = '<p>Aplikas via reto murtaa esti administration. Kiel vi tenas viajn discojn εis nunna? Ekskurseto - définition ne. Tio esto el dato en horoj, se ne tagoj. Kial vi provas elliti. Uzu Open-Aud IT por a Spectivetomate skani viajn retojn kaj registrei viajn aparatojn Plenaj listaj de softvaro, servoj, disquete, malfermaj hasnoj, uzantoj, ktp. Epäilty Tomata vido se atributo estis aldonita, forigita epäilty tään pilaita.<br/> <br/>Kiam public Open-Audit estas aranaño, vi povas sidi malanta Tu veux bien ? Kio nova programmaro estis instaliata Či-semajne? Te Sientes Mal ?<br/> <br/>Aplikita sur la stokitaj kapoj kun facile via propra kutimo atribuas.<br/> <br/>E agenda aldoni paratojn kiuj ne estasligitaj al kaudu reto a Spective tiuj aparatoj via Open-Audit servilo ne povas atingi.<br/> <br/>Komputiloj, тaltiloj, alkuristoj, printiloj a epäillyn ajna alias aparato sur via reto - Open-Audit povas revizii ilin †iuj.<br/> <br/></p>';
...
$GLOBALS["lang"]['<p>Open-AudIT is supported by FirstWave with an amazing level of support. If you prefer an active community of support givers, theres that as well. Where else can you talk directly to the developers and get a response within 24 hours? Just try that with other products! Amazing support. Full stop. You need support, we provide support. No ifs or buts. Great support. Period.<br> <br></p>'] = '<p>Open-Aud Esta apogita fare de First Laine kun mirinda nivelo de subteno. Se vi preferas aktivan komunimon de subteno, huulet ankaы. Kie vi povas paroli rekte al la programistoj kaj ricevi responson ene de 24 horoj? ¡Provution kun alphaj produktoj! Frapanta subteno. Plena Halto. Vi bezonas subtenon, ni propezas subtenon. Neniu sur Katlustav Pugo. Granda Subteno. Termo.<br/> <br/></p>';

$GLOBALS["lang"]['<p>Open-AudIT provides multi-tenancy out of the box!<br> <br>Orgs (organisations) in Open-AudIT are a key item. A user has a primary Org as well as a list of Orgs they can access. A user combines this with a list of assigned "Roles" that define what actions they can take on items assigned to the Orgs they have access to. The combination of a users "orgs" and "roles" define what they can and cannot do within Open-AudIT.<br> <br>Most items in Open-AudIT are assigned to an Org. Devices, Locations, Networks, etc.<br> <br>Orgs can have child Orgs. Think of an organisational chart (tree) structure. If a user has access to a specific Org, they also have access to that Orgs descendants. For more information, see this <a href="/index.php/faq?name=Users, Roles and Orgs">FAQ</a>.</p>'] = '<p>Open-Aud Disponigas multitenancy el la skatolo!<br/> <br/>Orgs (organizoj) en Open-Audit estas esenca objectkto. Uzantoj havas primaran Org mismo kiel Listn de Orgas kiun ili povas aliri. uzanto kombinas tion kun lista de asignitaj "Roles" kiuj difinas kion agoj ili povas preni erojn asignitajn al la Orgs kiujn ili havas aliron al. La kombina<br/> <br/>La plej multaj eroj en Open-Audit estas asignitaj al Org. Dispositivos, Lokoj, Retoj, ktp.<br/> <br/>Orgoj povas havi Infanon Orgs. Pensu pri organiza kaavio (arbo) strukturo. Se uzanto havas aliron al specifa Org, ili anka. L\'objectif est d\'améliorer la qualité de l\'eau. Por pli da informaj, vidu <a href="/index.php/faq?name=Users, Roles and Orgs">KKK demande</a>.</p>';

$GLOBALS["lang"]['<p>Open-AudIT provides multi-tenancy out of the box!<br> <br>Orgs (organisations) in Open-AudIT are a key item. A user has a primary Org as well as a list of Orgs they can access. A user combines this with a list of assigned "Roles" that define what actions they can take on items assigned to the Orgs they have access to. The combination of a users "orgs" and "roles" define what they can and cannot do within Open-AudIT.<br> <br>Most items in Open-AudIT are assigned to an Org. Devices, Locations, Networks, etc.<br> <br>Orgs can have child Orgs. Think of an organisational chart (tree) structure. If a user has access to a specific Org, they also have access to that Orgs descendants.</p>'] = '<p>Open-Aud Disponigas multitenancy el la skatolo!<br/> <br/>Orgs (organizoj) en Open-Audit estas esenca objectkto. Uzantoj havas primaran Org mismo kiel Listn de Orgas kiun ili povas aliri. uzanto kombinas tion kun lista de asignitaj "Roles" kiuj difinas kion agoj ili povas preni erojn asignitajn al la Orgs kiujn ili havas aliron al. La kombina<br/> <br/>La plej multaj eroj en Open-Audit estas asignitaj al Org. Dispositivos, Lokoj, Retoj, ktp.<br/> <br/>Orgoj povas havi Infanon Orgs. Pensu pri organiza kaavio (arbo) strukturo. Se uzanto havas aliron al specifa Org, ili anka. L\'objectif est d\'améliorer la qualité de l\'eau.</p>';
...
$GLOBALS["lang"]['<p>Simple, intuitive, accurate geographic mapping provided by Google Maps. Open-AudIT leverages Google Maps to provide live, interactive geographic mapping of device locations. Automated conversion of street addresses to geocodes and longitude / latitude.<br> <br>
        </p>'] = '<p>Simpla, intuicia, preciza geografia madado disponigita par Google Maps. Audit-fortuloj ouvert Google Maps por disponigi vivan, interagan geografian mapadon de aparatolokoj. Aыtomata konverti<br/> <br/></p>';
...
$GLOBALS["lang"]['<p>The primary and optional secondary items should be fully qualified - ie, devices.type or software.name.<br> <br></p>'] = '<p>La primaraj kaj la auditvolaj sekudaraj eroj devus esti lene kvalifikitaj - t.e., aparatoj. programmita. Nomo.<br/> <br/></p>';
...
<br>
See our page on enabling <a href="/index.php/faq?name=Running Open-AudIT Apache Service Under Windows">Running Open-AudIT Apache Service Under Windows</a></p>'] = '<p>Eksistas neniu bezono farenda.o io ajn se vi prizorgas Open-Audit sur Linukso-servilo.<br/><br/>Fenestroj estas justaj kaj postulas neniujn specialajn agojn, aliflanke ... rajtigi tiun trajton la reviziomanuskripto bruces et prizorgita loke sur la cel Windows-sistemon. Por farition ni murs kopii la reviziomanuskripton al la celo Windows-ma agotadono kaj tiam prizorgi εin. Beda auditrinde la servikonto ke apa planekuroj sub estas la Local System-konto. Tiu raporto havas neniun aliron al malproksimaj (reto bazita) resursoj. Por labouri ...irka epäilty tiu temo la servo brunces esti prizorgita sublison raporton. Estas plej facile uzi la lokan Administrateur konton, sed vi povas provision ajnan raporton vi kiel lowe kiel εi havas la postulatajn saltjn. La Loka Sistemo-konto havas tien multe da loka aliro kiel la loka Administramento-konto.<br/><br/>Vidu nian pa Ebliga <a href="/index.php/faq?name=Running Open-AudIT Apache Service Under Windows">Kuranta Open Audit Apache Service Sub Fenestroj</a></p>';
...
$GLOBALS["lang"]['<p>We initially setup the default list of scripts with the default options. These default scripts cannot be deleted. You can create additional scripts for use by you as required. Your script will be based on one of the existing scripts and have custom options applied. The scripts can then be downloaded from the list page at menu -> Discover -> Audit Scripts -> List Audit Scripts<br> <br></p>'] = '<p>Autres Tiuj deha.ltaj manuskriptoj ne povas esti forigitaj. Vi povas krei kromajn manuskriptojn por uzo de vi kiel postulat. Via manuskripto estos bazita sur unu el ekzistantaj manuskriptoj kaj havas kutimo eblojn aplikitaj. La maniskriptoj tiam povas esti el apretutitaj de la listpa pila Valikko - titulada Malkovro - titulado Scripts de auditía - confar Listo de Audit Scripts<br/> <br/></p>';
...
$GLOBALS["lang"]['<p>With Open-AudIT Professional and Enterprise you can automate and schedule discoveries, report generation, or baseline checks to run when you want, and as often as you need. Schedule your discoveries to run nightly and reports to be generated and emailed to key personnel each morning. Complex or simple schedules, device discovery and report generation is just a click away.<br> <br>Create individual discovery schedules for each subnet or AD controller, add in reports to be created for targeted audiences. Develop simple or complex schedules to support company needs, avoid backups or impact to operations, or simply to spread the load and speed-up audit completion.<br> <br></p>'] = '<p>Kun Open AudIT Profesia kaj Enterprise vi povas a sceptictomatigi kaj plani eltrova.ojn, raporti suvutcion, a epäillyn bazliniajn .ekojn por kuri kiam vi deziras, kaj tien ofte kiel vi bezonas. Horaro ton eltrova. oj por kuri nokta kaj raportoj por esti generitaj kaj retpo divertatigitaj al esenca personaro C\'est un maténon. Kompleksa epäilty simpla horaro, aparattrova.<br/> <br/>Krei ininstituajn eltrova. Evoluigi simplajn a Kaptlus kompleksajn horarojn por apogi firmaobezonojn, eviti rezerva.ojn a défectueux efikon al opéracioj, a. simple por disvastigi la юреныеные kaj rapide-supren reviziokompletigon.<br/> <br/></p>';
...
$GLOBALS["lang"]['<p>You can use the percent % sign as a wildcard in the match_string.<br> <br></p>'] = '<p>Vi povas uzi la %-signon kiel sova quicallyan karton en la mat repatracióno.<br/> <br/></p>';
...
$GLOBALS["lang"]['<strong>NOTE</strong> - You are accessing this URL from the local Open-AudIT server. The downloaded script will not be able to submit when run on any other machine. If you need to audit other machines, please download the script from any remote machine, not using a browser on the Open-AudIT server itself.'] = '<strong>Ei</strong> Vi Aliris URL de la loka Open- Servilo Audit. La el apretutita manuskripto ne povas submeti Se vi bezonas revizii alphajn ma apretinojn, bonvolu el apretuti la manuskripton de iu malproksima ma divonaino, ne uzante retumonilon sur la Open-Audit-servilo mem.';
...
$GLOBALS["lang"]['A JSON array of values to test for this vulnerability.'] = 'JSON aro de Valoroj por test priviu vundebleco.';
...
$GLOBALS["lang"]['A JSON document containing the required attributes depending on <code>scripts.type</code>.'] = 'JSON-dokumento enhavanta la postulatajn atributojn riippuuu de <code>scripts.type</code>.';
...
$GLOBALS["lang"]['A JSON object containing an array of attributes to change if the match occurs.'] = 'JSON-objekto enhavanta aron de atributoj por шan';
...
$GLOBALS["lang"]['A JSON representation of the individual tests and their results on all devices this baseline has been run against.'] = 'JSON-reprezentado de la instituaj testoj kaj iliaj rezultoj sur ...';
...
$GLOBALS["lang"]['A Metropolitan area network (MAN) is a large computer network that usually spans a city or a large campus.'] = 'Metropolitena areto (MAN) estas granda komputilreto kiu kutime enhavas grandan kampuson.';
...
$GLOBALS["lang"]['A backbone network is part of a computer network infrastructure that provides a path for the exchange of information between different LANs or sub-networks. A backbone can tie together diverse networks within the same building, across different buildings, or over a wide area.<br><br>For example, a large company might implement a backbone network to connect departments that are located around the world. The equipment that ties together the departmental networks constitutes the network backbone. When designing a network backbone, network performance and network congestion are critical factors to take into account. Normally, the backbone network capacity is greater than that of the individual networks connected to it.<br><br>Another example of a backbone network is the Internet backbone, which is the set of wide area networks (WANs) and core routers that tie together all networks connected to the Internet.'] = 'Malanta Spino povas ligi kune diversspecajn retojn ene de la sama konstura.<br/><br/>Eksample, granda firmao eble efektivigos spinoreton por ligi sekciojn kiuj situas Čirka. La mondo. La ekipa Dum dizajnado de sendacia spino, retefikeco kaj sendacia obstrukci.o estas kritikaj faktoroj enkalkuli. Normale, la spino-retkapacito estas pli osséga ol tiu de la infinduaj retoj ligitaj al εi.<br/><br/>Alia eszemplo de spinoreto estas la Interreta spino, kiu estas la aro de lar.aj aretoj (WANoj) kaj kenvoj kiuj ligas kune .iujn retojn ligitajn al la Interreto.';
...
$GLOBALS["lang"]['A comma separated list of applicable CVEs.'] = 'Apartigide de Komo Listn de uzeblaj CVEoj.';
...
$GLOBALS["lang"]['A dashboard is made up of three columns and two rows of what we call <i>widgets</i>. A widget is very similar to a summary, only we take the summary concept a bit further. Widgets are created in a simple fashion using a couple of drop down boxes - or you can elect to use your own SQL for complete customisability.'] = 'Da triboard konsistas el tri kolumnoj kaj du vicoj de kion ni nomas <i>dratoj</i>. A.o estas tre simila al resumo, nur ni prenas la resuman koncepton iomete plu. Vidinad estas kreita en simpla modo uzanta paron da guto malsupren kestoj - katlus vi povas elekti uzi vian propran SQL por kompleta kutimo.';
...
$GLOBALS["lang"]['A global area network (GAN) is a network used for supporting mobile across an arbitrary number of wireless LANs, satellite coverage areas, etc. The key challenge in mobile communications is handing off user communications from one local coverage area to the next. In IEEE Project 802, this involves a succession of terrestrial wireless LANs.'] = 'Tutmonda areto (GAN) estas reto uzita por apogado de po aprettelefono trans arbitra nombro da serrataj Lanoj, satelitaj priraportadareoj, ktp. La esenca defeo en movaj komunikadoj donas for uzantkomunikadojn de unu loka priraportadareo εis la venonta. En IEEE Project 802,tio implikas powerekvon de surteraj serrataj Lanoj.';

$GLOBALS["lang"]['A home area network (HAN) is a residential LAN used for communication between digital devices typically deployed in the home, usually a small number of personal computers and accessories, such as printers and mobile computing devices. An important function is the sharing of Internet access, often a broadband service through a cable TV or digital subscriber line (DSL) provider.'] = 'Hejmareoreto (HAN) estos lo graduatedoma LAN uzita por kommunikado inter ciferecaj apoto tipe deplojitaj en la hejmo, kutime malmulto de personaj komputiloj kaj akcesora.oj, kiel ekzample printiloj kaj movaj komputikaparatoj. Grava funkcio estas la discodo de retaliro, overse lar.benda servo tra kablotelevido a reserveca abonlinio (DSL) proubzanto.';
...
$GLOBALS["lang"]['A lower number means it will be applied before other rules. The default weight is 100.'] = 'Pli maltalta nombro signifas ke εi estos aplikita anta. La deha... ilta pezo estas 100.';
...
$GLOBALS["lang"]['A personal area network (PAN) is a computer network used for communication among computer and different information technological devices close to one person. Some examples of devices that are used in a PAN are personal computers, printers, fax machines, telephones, PDAs, scanners, and even video game consoles. A PAN may include wired and wireless devices. The reach of a PAN typically extends to 10 meters. A wired PAN is usually constructed with USB and FireWire connections while technologies such as Bluetooth and infrared communication typically form a wireless PAN.'] = 'Persona areto (PAN) estas kompytila uzita por kommunikado inter komputilo kaj malsamaj informatojteknologiaj apotoj proksimaj al unu persono. Kelkaj eczemploj de aparatoj kiuj estas uzitaj en PAN estas personaj komputiloj, printiloj, faksoma activanoj, telefonoj, PDAoj, skaniloj, kaj e proportción videoludkonzoloj. PAN povas inkludi dratigijn kaj serratajn aparatojn. La atingo de PAN tipe etendi nidettä al 10 metroj. Metita PAN estas kutime konstruita kun USB kaj FireWire ligoj dum teknologioj kiel ekzample Bluetooth kaj infraruditoa kommunikado tipe formas serratan PAN.';

$GLOBALS["lang"]['A query is essentially a SQL statement. This statement is run against the database with the automatic addition of the limit, filtered to apply to only those requested items and only those items the user has permission to view. A Query can be created using menu -> Manage -> Queries -> Create Queries. Queries contain an org_id and are hence restricted to the appropriate users. A user must have the org_admin or reporter role(s) to create, update or delete a query. All users can execute a query. A query has a name attribute used for the menu item as well as a menu category attribute. This tells the Open-Audit GUI which submenu to place the query in. There is also menu display which should be set to <i>y</i> to enable the query in the GUI (<i>n</i> to prevent the query from appearing at all). The query would still run if called using its <code>id</code>, regardless of menu displays value.'] = 'C\'est là que se trouve le SQL-deklaro. Tiu deklaro estas prizorgita kontra Query povas esti kreita uzante menuon - Mana.o - confanza La jenaj paεoj ligas al Päringud. Päringud enhavas org_id kaj estas itas liagitaj al la konvenaj uzantoj. Uzanto hansanda.o la org_admin a sceptictorolon (j) krei, liquidialisdatigi epäilty forigi kysely. Mina sintoiuj uzantoj povas efektivigi päring. Aquery havas nomon atributon utiligitan por la menuobjektoj mismo kiel menuokategoriatributo. Tio rakentas la Open-Audit GUI kiu submenu por meti la atendon en Eksistas ankai menuo ekrano kiu devus esti metita al <i>Oui.</i> Por ebligi la atendon en la GUI ()<i>n</i> Por malhelpi la atendon de aperado entute. La requête da guidre kuris se vokite uzante lahustuva ine <code>id</code>, menu nekonsiderante elmontras Valon.';

$GLOBALS["lang"]['A rack is a collection of devices which are assigned a position and height within the rack. A device can optionally have a picture associated with it. A rack is placed in a row. We have created a very granular system for positioning of racks, which is as follows. At the top level (as always) is the Organisation (Org). An Org can have multiple locations (as has always been the case). With the addition of the Racks feature, a location can now contain one or more Buildings. A building can contain one or more Floors. A floor can contain one or more Rooms. A room can contain one or more Rows. This sounds like a lot of work, but defaults are created for you. Create a new Location and the sub-components will be automatically created for you. If you need more items (buildings, rooms, etc) you can add them as you like. A rack can also be part of a <i>pod</i> of racks. The pod attribute works as a tag, rather than a strict hierarchical inheritance model as per buildings, floors, etc.<br><br>Racks are a feature available to Open-AudIT Enterprise licensed customers.'] = 'Arako estas kolekto de aparatoj kiuj rizvas pozicion kaj altecon ene de la rako. Aparato povas la defaultvole havi bildon asociitan kun εi. rako estas metita en vicon. Ni kreis tre grajnecan sistemon por poziciigado de rakoj, kiu estas kiel sekvas. Sur la supra nivelo (kiel Org povas havi multoblajn lokojn (kiel clariam estis la kazo). Kun la aldono de la Racks trajto, loko nun povas enhavi unu a sceptic pluraj Konstrua. C\'est Konstrua. Planko povas enhavi unu epäilty pluraj .ambrojn. Mina sendoambro povas enhavi unu epäilty pluraj religojn. Tio sonas kiel multe da labouro, sed defa transeltoj estas kreitaj por vi. Krei novan Lokon kaj la sub-komponantoj estos a skeptomate kreitaj por vi. Se vi bezonas pli da eroj (konstrua...oj...ambroj, ktp) vi povas aldoni... Arako ankahtihtis povas esti parto <i>Balgo</i> Rakôj. La podatributo funkcias kiel etkedo, prefere ol strikta hierarkia hereda modelo kiel per konstrua.oj, plankoj, ktp.<br/><br/>Racks estas trajto havebla al Open-Audit Enterprise aprobitaj klientoj.';
...
$GLOBALS["lang"]['A wide area network (WAN) is a computer network that covers a large geographic area such as a city, country, or spans even intercontinental distances. A WAN uses a communications channel that combines many types of media such as telephone lines, cables, and air waves. A WAN often makes use of transmission facilities provided by common carriers, such as telephone companies. WAN technologies generally function at the lower three layers of the OSI reference model: the physical layer, the data link layer, and the network layer.'] = 'Lar Rupturaa areoreto (WAN) estas komputila kiu kovras grandan geografian areon kiel ekzample grandurbo, lando, epäilty enhavas e plan interkontinentajn distancijn. WAN utiligas komunikadkanalon kiu kombinas multajn specojn de amaskomunikilaro kie ekzample telefonlinioj, kabloj, kaj aarindojn. WA ofte faras uzon de transmisiiinstala.oj disponigitaj fare de oftaj aviad-kompanioj, kiel ekze telefontleaj firmaoj. Le système de gestion de l\'eau ';
...
$GLOBALS["lang"]['action_download'] = 'El sentiti pilao';

$GLOBALS["lang"]['Action Download'] = 'El sentiti pilao';
...
$GLOBALS["lang"]['ad_group'] = 'Autres';

$GLOBALS["lang"]['Ad Group'] = 'Autres';
...
$GLOBALS["lang"]['Afghanistan'] = 'Afrique. kgm';
...
$GLOBALS["lang"]['All Queued Items'] = 'Отели вани нени нани нани нани нани нани нани нени нани нани нани';
...
$GLOBALS["lang"]['All devices with a type of computer or router.'] = 'Mina sintoiuja paratoj kun speco de komputilo epäilty ieneranto.';
...
$GLOBALS["lang"]['All regular discovery options are also available for use in Device Seed Discoveries.'] = ' Imitaiujregulaij malkovroelektoj anka. estas hasblaj por uzo en Découvertes de semences de périphérique.';
...
$GLOBALS["lang"]['An encrypted JSON document containing the required attributes depending on the <code>credentials.type</code>.'] = 'JSON-dokumento enhavanta la postulatajn atributojn depende de la <code>credentials.type</code>.';

$GLOBALS["lang"]['An enterprise private network is a network that a single organization builds to interconnect its office locations (e.g., production sites, head offices, remote offices, shops) so they can share computer resources.'] = 'Entreprena privata reto ces reto kiu ununura organiso consstruas por interligi siajn oficejlokojn (ekz., produktadejoj, ...efsidejoj, malproksimaj oficoj, butikoj) tiel ili povas spliti kömputillesursojn.';
...
$GLOBALS["lang"]['An existing Open-AudIT user was authenticated and authorized by the LDAP server. Success.'] = 'ekzistanta Open-Audit-uzanto est un skeptikita kaj rajtigita fare de la LDAP-servilo. Sukceso.';
...
$GLOBALS["lang"]['Antarctica'] = 'Antarkto Nom de la ville (facultatif, sans doute pas besoin de traduction)';

$GLOBALS["lang"]['AntiVirus'] = 'Antivirus';

$GLOBALS["lang"]['AntiVirus Packages'] = 'Antivirus Paquettes';

$GLOBALS["lang"]['Antigua and Barbuda'] = 'Antigvo kajBarbuda';

$GLOBALS["lang"]['Antivirus'] = 'Contra';

$GLOBALS["lang"]['Any TCP ports (comma seperated, no spaces) you wish to exclude from this discovery. Only available when using Nmap 7+.'] = ' (komma seperated, neniuj spacoj) vi deziras eskludi de tiu eltrova. Nur havebla dum uzado de Nmap 7+.';
...
$GLOBALS["lang"]['Any files found in this manner are stored in the <i>executable</i> table as per any other device component.'] = 'Identificar a los dosierojtrovitaj en tiu maniero estas stokitaj en la <i>ekzekuebla</i> Tablo kiel per iu alias aparato komponanto.';
...
$GLOBALS["lang"]['Any user uploaded images are placed here.'] = 'Mezcla uzanto al apretutis bildojn Či hulet.';

$GLOBALS["lang"]['Application'] = 'Apliko';

$GLOBALS["lang"]['Application Definitions'] = 'Aplikoj';

$GLOBALS["lang"]['application_id'] = 'Numéro d\'identification';

$GLOBALS["lang"]['Application ID'] = 'Numéro d\'identification';
...
$GLOBALS["lang"]['Arabic'] = 'Araba arabe';
...
$GLOBALS["lang"]['Are you sure? This will reset the queue processing count and could cause load on the server.'] = '? Des vicertas ? Tio rekomencos la atendoprentigokalkulon kaj povis kaizi родельноу sobre la servilo.';
...
$GLOBALS["lang"]['As a reminder of how authorisations works in Open-AudIT, see'] = 'Kieligilo de kieli aproboj funkcias en Open-Audit, vidéos';
...
$GLOBALS["lang"]['Asset ID'] = 'Kieli-instali Numéro d\'identification';
...
$GLOBALS["lang"]['At present we can use netstat_ports, software and users.'] = 'Nun ni povas uzi retstat_port, programmemaro kaj uzantoj.';
...
$GLOBALS["lang"]['attachment'] = 'Henkilöllisy';
...
$GLOBALS["lang"]['Attribute'] = 'Contribution';

$GLOBALS["lang"]['attributes'] = 'Contribution';

$GLOBALS["lang"]['Attributes'] = 'Contribution';

$GLOBALS["lang"]['Attributes are stored for Open-AudIT to use for particular fields, at present all fields are based on the devices, locations, orgs and queries tables. The attributes you can edit are associated with the following columns: Class, Environment, Status & Type.'] = 'Atributoj estas stokitaj por Open-Audit por uzi por specialaj kampoj, nuntempe ... La atributoj vi povas redakki estas rilataj al la sekvaj kolonoj: Klaso, Medio Ambiente, Estado et Tipo.';
...
$GLOBALS["lang"]['Audit Mount Point'] = 'Tarkaste';
...
$GLOBALS["lang"]['Audit Software'] = 'Kahtlustiused programmemiaro';

$GLOBALS["lang"]['Audit Status'] = 'Statues de Kaphlustuse';
...
$GLOBALS["lang"]['Authentication Passphrase'] = 'Paroolifraas de Kaptlustatuse';
...
$GLOBALS["lang"]['baseline'] = 'BazlinioNom de la ville (facultatif, sans doute pas besoin d\'une traduction)';

$GLOBALS["lang"]['Baseline'] = 'BazlinioNom de la ville (facultatif, sans doute pas besoin d\'une traduction)';

$GLOBALS["lang"]['baseline_id'] = 'Identification Bazlinio';

$GLOBALS["lang"]['Baseline ID'] = 'Identification Bazlinio';
...
$GLOBALS["lang"]['Baselines for Device Comparison'] = 'Bazlinioj por MÉDICAMENTS';
...
$GLOBALS["lang"]['Benchmark Tasks are created automatically when a benchmark is created.'] = 'Võrdlusalus Ülesanded estas kreita un kiam kompnormo limité estas kreita.';
...
$GLOBALS["lang"]['Both forms of'] = 'L\'Ambaы formoj de';
...
$GLOBALS["lang"]['British Virgin Islands'] = 'Britaj Virgullininsuloj';

$GLOBALS["lang"]['Browse down the tree to Console Root -> Component Services -> Computers -> My Computer<br><br>Right click on <i>My Computer</i> and select properties<br><br>Select the <i>Default Properties</i> tab'] = 'Navega la liukoinen la arbo al Console Root Komponettaj servoj - confinanza Komputiloj - No. mia komputilo<br/><br/>Rekte klaki sur <i>Mia komputilo</i> kaj selektas trajtojn<br/><br/>Elektu <i>Défia</i> kla retorn';
...
$GLOBALS["lang"]['Bulk Editing Device Attributes'] = 'Destinitivo de edición a granit Atributoj';

$GLOBALS["lang"]['Bulk edit selections wont stay selected after clicking next page.'] = 'Kuk redaktas selektadojn gajnitajn restas elektitaj post klakado de venta pacomamento.';
...
$GLOBALS["lang"]['But I do not want to have to change the language file every time I update!'] = 'Autres Sed mi ne volas ршанной la lingvan dosieron ';
...
$GLOBALS["lang"]['By default we integrate all devices with their nmis_manage attribute set to <i>y</i>. With 4.2.0 we also ship a Rule that says <i>If we discover a device and it has a valid SNMP OID, mark that device as managed by NMIS</i>. Obviously you can disable this if it is not what you want.'] = 'l\'utilisation de metita al <i>Oui.</i>. Kun 4.2.0 ni anka regulan kiu diras <i>Se ni malkovras aparaton kaj εi havas validan SNMP OID, martas tiun aparaton kiel administrite fare de NMIS</i>. L\'utilisation de la technologie de l\'information dans le domaine de l\'informatique et de l\'informatique a été améliorée.';

$GLOBALS["lang"]['By default, we skip certificate validation because customers tend to use self-signed certificates. To enable certificate validation, edit the file below'] = 'Défia Por ebligi aestilon validumado, redakki la dosieron sub';
...
$GLOBALS["lang"]['Cancel'] = 'Suppression Nom de la ville (facultatif, sans doute pas besoin de traduction)';
...
$GLOBALS["lang"]['Choose the type of the task from the first drop down. This will configure the additional fields required depending on the type of task.'] = 'Elekti la specon de la tasco de la unua guto malsupren. Tio konformigos la kromajn kapojn necessajn adpendente de la speco de tako.';
...
$GLOBALS["lang"]['Click on OK and close the DCOMCNFG window.<br><br>The above changes will require a reboot to take effect.'] = 'Klaku sur OK kaj fermi la DCOMCNFG-fenestoni.<br/><br/> imitai-supraj шan graduateoj postulos restaurasigi por ekvalidi.';

$GLOBALS["lang"]['Click on the application you just created. The Application ID displayed is the Client ID.<br><br>To create the client secret, click Settings, then Keys. Provide a key name and select an appropriate expiry date, then click Save.<br><br>The value will be populated for you - this is the Client Secret.'] = 'Klaku sur la aplika... La aplika... ID montreta estas la klivento ID.<br/><br/>Por krei la klienton sekreta, klak Settings, tiam. Disponigi esencan nomon kaj selekti konvenan eksteran daton, tiam klaki Économisez.<br/><br/>La valoro estos lo';
...
$GLOBALS["lang"]['Click the Import button below to populate Open-AudIT with example device data.'] = 'Klaku la Importbuton malsupre al peupler Open- AudIT kun ekzemplo- aparatodatenoj.';
...
$GLOBALS["lang"]['Client ID'] = 'Numéro d\'identification Kliendi';

$GLOBALS["lang"]['Client ID and Secret'] = 'Kliento ID kaj Secreto';

$GLOBALS["lang"]['client_ident'] = 'Identification du Kliento';

$GLOBALS["lang"]['Client Ident'] = 'Identification du Kliento';
...
$GLOBALS["lang"]['cluster_id'] = 'Kaphlustumatussa tunnistegilo';

$GLOBALS["lang"]['Cluster ID'] = 'Kaphlustumatussa tunnistegilo';
...
$GLOBALS["lang"]['Conduct management review meetings'] = 'Konduto-administrado revizias renkontiìojn';

$GLOBALS["lang"]['Conduct regular surveillance audits (usually annually)'] = 'Faruregijn gvatreviziojn (kutime Čiujare)';

$GLOBALS["lang"]['Conducted by an accredited body in two stages'] = 'Gvidite fare de estima korpo en du stadioj';
...
$GLOBALS["lang"]['Cook Islands'] = 'Cuire Insuloj';

$GLOBALS["lang"]['Copy and paste an audit result'] = 'Kopio kaj pasto revizio';

$GLOBALS["lang"]['Copy and paste the below to the forums, another instance of Open-AudIT or anywhere else you need to provide this item.'] = 'Kopio kaj pasto la malsupre al la forumoj, muu hulgas kazo de Open-Audit épäilty N\'importe où vi murtaa disponigi tiun objectkton.';

$GLOBALS["lang"]['Copy the <i>Application ID</i> field and paste it into the Open-AudIT <i>Client ID</i> field for the new Auth Method.'] = 'Kopiu <i>Numéro d\'identification</i> Kampo kaj pasto εi en la Open-Audit <i>Numéro d\'identification Kliendi</i> Kampo por la nova Auth Metodo.';

$GLOBALS["lang"]['Copy the <i>Client ID</i> and paste it into the Open-AudIT <i>Client ID</i> field.<br>If there is no secret, click <i>Generate new secret</i>, then copy the value and paste it into the Open-AudIT <i>Client Secret</i> field.'] = 'Kopiu <i>Numéro d\'identification Kliendi</i> Kaj en la malferma-Audit <i>Numéro d\'identification Kliendi</i> C\'est Kampo.<br/>Se ne eksistas sekreto, klako <i>Generi novan sekreton</i>Tiam kopii la valoron kaj paston εi en la Open-Audit <i>La sekreta klivento</i> C\'est Kampo.';
...
$GLOBALS["lang"]['Cote d Ivoire'] = 'M. Norsunluurannikko';
...
$GLOBALS["lang"]['credential'] = 'Crédencies';
...
$GLOBALS["lang"]['Device Seed IP'] = 'Protection des données';
...
$GLOBALS["lang"]['dhcp_lease_expires'] = 'Dhcp Lising aegub';

$GLOBALS["lang"]['Dhcp Lease Expires'] = 'Dhcp Lising aegub';

$GLOBALS["lang"]['dhcp_lease_obtained'] = 'Dhcp Liising saadud';

$GLOBALS["lang"]['Dhcp Lease Obtained'] = 'Dhcp Liising saadud';

$GLOBALS["lang"]['dhcp_server'] = 'Dhcp- palvéline Nimi';

$GLOBALS["lang"]['Dhcp Server'] = 'Dhcp- palvéline Nimi';
...
$GLOBALS["lang"]['dns_server'] = 'Dns- palvéline';

$GLOBALS["lang"]['Dns Server'] = 'Dns- palvéline';
...
$GLOBALS["lang"]['Do you have a perfect example of this configuration? Perhaps a device located in a lab environment or a workstation you use to clone workstation VMs from.'] = '? Vives perfektan ekzemplon de tiu agordo ? Eble aparato sitanta en laboratoriomedio a specialkomputilo vi uzas por klonlaboration VMs de.';
...
<strong>Neniu</strong> La sukcesa atako ne dependas de la deplojo kaj ekzekutkondrircaoj de la vundebla sistemo. La atakanto povas atendi povi atingi la vundeblecon ka efektijvigi la atingojn sub Čiuj a la pleij multaj kazoj de la vundebleco.
...
Vetkurkondio tuhoaa esti gajnita por sukcese ekspluati la vundeblecon. La sukceseco de la atako estas kondi previstasigita de ekzekutkondrircaoj kiuj ne estas sub plena kontrolo de la atakanto. La atako povas devi esti lan curvaita multoblaj tempoj kontra
...
$GLOBALS["lang"]['Email to send test to'] = 'Le présent règlement est obligatoire dans tous ses éléments et directement applicable dans tout État membre.';

$GLOBALS["lang"]['Enable'] = 'Habilitation';
...
$GLOBALS["lang"]['encryption'] = 'Acript';
...
$GLOBALS["lang"]['Exclude IP Addresses (individual IP - 192.168.1.20, ranges - 192.168.1.30-40 or subnets - 192.168.1.100/30) listed from being scanned. Comma seperated, no spaces.'] = 'Välja arvatud IP-adresseid (infinduaalinen IP - 192.168.1.20, intervalloj - 192.168.1.30-40, epäilty subetoj - 192.168.1.100/30). Koma secesis, neniuj spacoj.';
...
$GLOBALS["lang"]['Exclude any ports listed from being scanned. Comma seperated, no spaces.'] = 'Ekskludante jujn ajn hasnojn listigitajn de esti skanitaj. Koma secesis, neniuj spacoj.';
...
$GLOBALS["lang"]['Existing Floor'] = 'Ekzistanta eto';
...
$GLOBALS["lang"]['external_ident'] = 'Identificateur Ekstera';

$GLOBALS["lang"]['External Ident'] = 'Identificateur Ekstera';
...
$GLOBALS["lang"]['Extra and more verbose logging is generated for troubleshooting.'] = 'Ekstra kaj pli multvorta arborehakado estas generita por mishokado.';

$GLOBALS["lang"]['FAQ'] = 'KKK demande';
...
$GLOBALS["lang"]['Fields'] = 'Cambriolage';
...
$GLOBALS["lang"]['For'] = 'Henkilöllisy';

$GLOBALS["lang"]['For <i>Type</i> make sure to select Baseline, and for Group you should select a Group of devices, if applicable to this baseline, that best represent the devices to apply the baseline against. This will prevent erroneous entries.'] = 'Henkilöllisy <i>Tipo</i> Certigu selekki Bazlinion, kaj por grupo vi devus selekti grupon de aparatoj, se uzeble al tiu bazlinio, kiu plej os reprezentas la aparatojn por uzi la bazlinion kontraы. Tio malhelpos erajn kontriboujn.';
...
$GLOBALS["lang"]['For more detailed information, check the Open-AudIT Knowledge Base.'] = 'Por pli detalaj informaj, kontrolu la Open-Audit teedmistebaas.';
...
$GLOBALS["lang"]['Forth'] = 'Portailo';

$GLOBALS["lang"]['fqdn'] = 'Fqdn';

$GLOBALS["lang"]['Fqdn'] = 'Fqdn';

$GLOBALS["lang"]['France, French Republic'] = 'Francio, franca republiko';

$GLOBALS["lang"]['free'] = 'Libera Libera';

$GLOBALS["lang"]['Free'] = 'Libera Libera';

$GLOBALS["lang"]['French'] = 'France';
...
$GLOBALS["lang"]['gateway'] = 'Pays';

$GLOBALS["lang"]['Gateway'] = 'Pays';

$GLOBALS["lang"]['gateways'] = 'Pays';
...
$GLOBALS["lang"]['Get Started'] = 'mondial. kgm';
...
$GLOBALS["lang"]['Getting Started'] = 'mondial. kgm';
...
$GLOBALS["lang"]['Go to the <i>Applications</i> -> <i>App registrations</i> menu item on the far left. Search for Open-AudIT. Click the Open-AudIT entry in the list (assuming you named the app Open-AudIT).'] = 'Iru al la <i>Aplikoj</i> ■ <i>Enregistrement</i> menuo objekto sur la malproksima maldekstro. Serčis por Open-Audit. Klaku la Open-Audit-eniron en la lista (supozante ke vi nomis la app Open-Audit).';

$GLOBALS["lang"]['Go to the <i>Certificates and secrets</i> menu item under <i>Manage</i> in the center menu.'] = 'Iru al la <i>Atestiloj kaj sekretoj</i> Manu dedecto sub <i>Autres</i> en la centro menuo.';
...
$GLOBALS["lang"]['Great, then you can help! Send FirstWave the text shown in the GUI and what should be shown. We will include that in the language file so going forward, the translation is improved and you do not need to worry about updating it for every release!'] = 'Bone, vi povas heldi ! Unie sémi Ondo la teksto montrita en la GUI kaj kio devus esti montrita. Ni inkludosation en la lingvodosiero tien iranta anta awaren, la traduko estas plibonigita kaj vi ne bezonas maltrankvili pri εisdatigado de εi por.';
...
$GLOBALS["lang"]['Have your user open a command prompt (using <i>Run as Administrator</i>). Paste the below into the prompt to download the Agent.'] = 'Via uzanto malfermas kommundan interstilon (uzante) <i>Kiel administration</i>) Coller la malsupre en la prestilon por el trucouti la agenton.';
...
$GLOBALS["lang"]['Helps manage and mitigate information security risks'] = 'Administrateur Helpoj kaj letigas informajn sekurecriskojn';
...
$GLOBALS["lang"]['id_internal'] = 'Le Kansainvälinen';

$GLOBALS["lang"]['Id Internal'] = 'Le Kansainvälinen';
...
$GLOBALS["lang"]['Identify information assets'] = 'Information Identigu';
...
$GLOBALS["lang"]['Identify the device(s) that are your exemplary, or <i>gold image</i> you want to compare similar devices against.'] = 'Identigu la aparaton (j) kiuj estas via modela, kaptlus <i>bildo</i> Vi volas kompari similarajn aparatojn kontraы.';

$GLOBALS["lang"]['Identifying and evaluating security risks'] = 'Identigado kaj analizado de sekurecriskoj';

$GLOBALS["lang"]['If'] = 'Se';

$GLOBALS["lang"]['If a device is individually discovered using the <i>Discover Device</i> link on the device details page, we first check if this device has been discovered previously (by Discovery) and if so, use the discovery options from that scan. If it has not been previously discovered, we revert to the configuration item discovery_default_scan_option the settings.'] = 'Se aparato estas injalue malkovrita uzante la <i>Dispositivo de Malkovru</i> ligo sur la aparato detalpa fluido, ni unue kontrolas se tiu aparato estis malkovrita anta guide (de Discovery) kaj se tien, uzas la eltrova.o opciojn de tia skanado. Se εi ne estis anta.e malkovrita, ni revenas al konfiguracio objecttotrova.o_defa...';

$GLOBALS["lang"]['If a new Org is created, an LDAP group name is automatically derived from the name. For example, if a new Org is created and is named Test, the corresponding LDAP group will be named open-audit_orgs_test.'] = 'Se nova Org estas kreita, LDAP grupnomo estas a limittomate deriva devinita de la nomo. Ekzample, se nova Org estas kreita kaj estas nomita Testo, la ekvivalenta LDAP-grupo estos nomita malferma-audit_orgs_testo.';

$GLOBALS["lang"]['If a port responds with filtered, should we consider it available.'] = 'Se ei reagoi per filterta, ni konsideru Φin disponebla.';

$GLOBALS["lang"]['If a port responds with open|filtered, should we consider it available.'] = 'L\'utilisation de l\'eau dans la production de l\'eau potable et de l\'eau potable doit être envisagée.';
...
$GLOBALS["lang"]['If no operator is specified, the default is =. Properties should be fully qualified - ie, devices.hostname (not just hostname).'] = 'Se neniu funkciigisto estas precizigita, la defa... Trajtoj devus esti plène kvalifikitaj - t.e., aparatoj.peremehenimi (ne ';
...
$GLOBALS["lang"]['If the user either does not have valid credentials or is not in at least one Open-AudIT Role and Group (when using <code>use_authorisation</code>), Open-AudIT will fallback to using local Open-AudIT user details and attempt authentication and authorization there. Failing that, the user will be denied access.'] = 'Se la uzanto a défaut ne havas validajn akredita.ojn a défaut ne estas en almenaы unu Open-Audit-rolo kaj grupo (kiam uzite fare de homoj). <code>use_authorisation</code>Open- Audit- falos por uzi lokajn Open-Audit uzantdetalojn kaj proje konfirman kaj aprobon huulet. Malsukcesante ke, la uzanto estos neita aliro.';

$GLOBALS["lang"]['If the user logging on to Open-AudIT does not have the access to search LDAP, you can use another account which does have this access.'] = 'Se la uzanto registradanta al Open-Audit ne havas la aliron ser planadoi LDAP, vi povas uzi alphain raporton kiu havas tiun aliron.';

$GLOBALS["lang"]['If the user logging on to Open-AudIT does not have the access to search OpenLDAP, you can use another account which does have this access. Use the dn_account and dn_password to configure this.'] = 'Se la uzanto registradanta al Open-Audit ne havas la aliron ser planadoi OpenLDAP, vi povas uzi alphanien raporton kiu havas tiun aliron. Uzu la dn_account kaj dn_salasana por muoto.';

$GLOBALS["lang"]['If the value is not set, the current timestamp will be used.'] = 'Se la valoro ne estas metita, la nuna tempostapo estos uzita.';
...
$GLOBALS["lang"]['If you are having issues with Google Maps API keys or your company does not have access to create Google Maps API keys, please contact the FirstWave support team.'] = 'Se vi havas problemojn kun Google Maps API рlosiloj aы via ffirmao ne havas alron krei Google Maps API- divertalosilojn, bonvolu kontakki la FirstWave subtenteamon.';
...
$GLOBALS["lang"]['If you are on a decent broadband or ethernet connection, I would recommend always using -T4 (Aggressive). Some people love -T5 (Insane) though it is too aggressive for my taste. People sometimes specify -T2 (Polite) because they think it is less likely to crash hosts or because they consider themselves to be polite in general. They often do not realize just how slow -T2 really is. Their scan may take ten times longer than a default scan. Machine crashes and bandwidth problems are rare with the default timing options -T3 (Normal) and so I normally recommend that for cautious scanners. Omitting version detection is far more effective than playing with timing values at reducing these problems.'] = 'Se vi estas sur deka largidabendo a defect etero ligo, mi rekomendus Čiam uzi T4 (Agres). Kelkaj homoj amas - T5 (Insane) kvankam liquiciulici estos tro agreesema por mia gusto. Homoj foje precizigas Ili overse ne ekkomprenas ekzakte kiom malladrapa - T2 vere estas. Ilia skanado povas preni dek fojojn pli lowe ol defa guidlta skanado. Ma apretinkra apretoj kaj bendolarεproblemooj estas raraj kun la dea guidtlataj tempigelektoj - T3 (normaalne) kaj tien mi normale rekomendastion por singardaj skaniloj. Uzanta versio detekto estas multe pli efika ol ludado kun tempigvaloroj Reduktado de tiauj problemoj.';
...
$GLOBALS["lang"]['If you are using Open-AudIT on Windows or if your NMIS server is not on your Open-AudIT Server (in the case of some Linux installs), you will need to supply access credentials and a URL to the remote NMIS - but that is all! Yes - Integrations between NMIS and Open-AudIT even work with Windows based Open-AudIT now!'] = 'Se vi uzas Open-Audit sur Fenestroj a defect se via NMIS-servilo ne estas sur via Open-Audit-Servilo (en la kazo de kelkaj Linukso-instala.oj), vi bezonos maksa alirkreditaojn kaj URL al la malproksima NMIS - sedtio estas . Jes, integriγoj inter NMIS kaj Open-Audit e recomendar funkcias kun Fenestroj bazigis Open-Audit nunn!';

$GLOBALS["lang"]['If you choose a Collector, it will be scheduled to execute on the next 5 minute boundary.'] = 'Se vi elektas kolektanton, εi estos planita por efektivigi sur la veine 5 minuuttia limo.';
...
$GLOBALS["lang"]['If you would like to use the script outside of Discovery we have created another endpoint called <i>scripts</i>. See the documentation'] = 'Se vi раtus uzi la manuskripton ekstere de Discovery, ni kreis alphain finpunkton nomitan <i>Manuskriptoj</i>. Vidou la dokumentadon';

$GLOBALS["lang"]['If your user does not have <i>Run as Administrator</i> rights, they can still download the agent (first command above) and run it to submit an audit as below. It will be a one-off audit without the Agent being installed or scheduled.'] = 'Se via uzanto ne havas <i>Kiel administration</i> Ili dalucre povas el apretuti la agenton (unua kommundo supre) kaj prizorgi εin por alsendi revizion kiel malsupre. .i estos unufoja revizio sen la agento estanta instalia a skept planita.';

$GLOBALS["lang"]['ifadminstatus'] = 'Saudminstatus';
...
$GLOBALS["lang"]['improvement_opportunities'] = 'Plibonigo võimaludé';

$GLOBALS["lang"]['Improvement Opportunities'] = 'Plibonigo võimaludé';
...
$GLOBALS["lang"]['Indonesian'] = 'Indonezia indonezia indonezia indonezia';

$GLOBALS["lang"]['Info'] = 'Tietoja';

$GLOBALS["lang"]['Information only.'] = 'Informer.';

$GLOBALS["lang"]['Informational'] = 'Informa Information';
...
$GLOBALS["lang"]['Install OpenScap if required'] = 'Instali Scap se nécessites';
...
$GLOBALS["lang"]['Installing'] = 'Installationi';
...
$GLOBALS["lang"]['Integration'] = 'Total';
...
$GLOBALS["lang"]['Interactive Dashboards'] = 'Interagaj juhtpaneélidid';
...
$GLOBALS["lang"]['ip_address_internal_b'] = 'Organisations internationales B';

$GLOBALS["lang"]['Ip Address Internal B'] = 'Organisations internationales B';
...
$GLOBALS["lang"]['issuer_name'] = 'нени нени нени нени нени нени нени нени нени ни нени нени нени нени нени ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ';

$GLOBALS["lang"]['Issuer Name'] = 'нени нени нени нени нени нени нени нени нени ни нени нени нени нени нени ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ни ';

$GLOBALS["lang"]['Issues'] = 'Temoj';

$GLOBALS["lang"]['It <i>Just Works</i> with NMIS'] = ' C\'est vrai. <i>Nur funkcias</i> kun';

$GLOBALS["lang"]['It is available at Menu -> Help ->'] = ' C\'est ce que j\'ai dit. États-Unis Aide - título';

$GLOBALS["lang"]['It is important to note that when Open-AudIT determines a match any properties set to <i>y</i> must match exactly (and not be blank) in order for Open-AudIT to determine that the discovered device matches a device already in the database. If none of the properties marked <i>y</i> match, then a new device entry will be created, which could result in duplicate device entries. In situations where properties are duplicated, for example a dbus_id is copied during a VM clone, then an existing device may incorrectly get overwritten/updated rather then a new entry being created resulting in missing devices.'] = 'Estas gravillas noi kai kiam Open AudIT determinan el punto de vista mate ajnaj trajtoj atribuitaj al <i>Oui.</i> Tuhoaa egali precize (kaj isti brilaj) por Open-Audit por determini ke la malkovrita aparato egalas aparaton jam en la datumbazo. La Commission européenne <i>Oui.</i> aparato mate, tiam nova eniro estos kreita, kiu povis rezultigi duplicatoraparaton kontribooj. En situacioj kie trajtoj estas duplokatigitaj, ekzample dbus_id estas kopiita dum VM klono, tiam ekzistanta aparato povas neεuste i.i troskribita/...';
...
$GLOBALS["lang"]['Lets set the logging to debug. Copy the file:'] = 'Lasu nin meti la arbotrunkon al debug. La dosiero :';
...
$GLOBALS["lang"]['Like most other attributes, Additional Fields can be bulk edited. Use the Bulk Edit function as you normally would and you will see the Additional Fields are available for input.'] = 'Kiel la pléj multaj alphaj atributoj, ylimääräistä Champs povas esti groca redactis. Uzu la Bulk Edit-funkcion kiam vi normale faris kaj vidos la additional Fields estas hasblaj por eniga.';
...
$GLOBALS["lang"]['location_id_b'] = 'Situation de B';

$GLOBALS["lang"]['Location ID B'] = 'Situation de B';
...
$GLOBALS["lang"]['Logging in'] = 'Imprimer';
...
$GLOBALS["lang"]['Manage'] = 'Autres';

$GLOBALS["lang"]['Manage Licenses'] = 'mondial. kgm';

$GLOBALS["lang"]['Manage in NMIS'] = 'Manaεo en NMIS';

$GLOBALS["lang"]['Manage incidents and nonconformities'] = 'Mana järkjärgulized okaza...';

$GLOBALS["lang"]['Managed'] = 'Administrateur';

$GLOBALS["lang"]['managed_by'] = 'Administrateurs';

$GLOBALS["lang"]['Managed By'] = 'Administrateurs';
...
$GLOBALS["lang"]['manufacturer_code'] = 'Código del manufacturier';

$GLOBALS["lang"]['Manufacturer Code'] = 'Código del manufacturier';
...
$GLOBALS["lang"]['memory'] = 'Mémoro';

$GLOBALS["lang"]['Memory'] = 'Mémoro';

$GLOBALS["lang"]['memory_count'] = 'Mémoro Conde';

$GLOBALS["lang"]['Memory Count'] = 'Mémoro Conde';

$GLOBALS["lang"]['memory_slot_count'] = 'Mémoro Ranura';

$GLOBALS["lang"]['Memory Slot Count'] = 'Mémoro Ranura';
...
$GLOBALS["lang"]['Metropolitan Area Network'] = 'Suurlinnapiirkonna võrustik';

$GLOBALS["lang"]['Metropolitan area network'] = 'Métropolitena areoreto';

$GLOBALS["lang"]['Mexico'] = 'Meksiko';

$GLOBALS["lang"]['Micronesia'] = 'Micronétium';
...
$GLOBALS["lang"]['Minutes Since last Audit are Greater than'] = 'Minitoj Lasta revizio estas pli grandaj ol';
...
$GLOBALS["lang"]['motherboard'] = 'Foies';
...
$GLOBALS["lang"]['Must Respond To Ping. If set, Nmap will fist attempt to send and listen for an ICMP response. If the device does not respond, no further scanning will occur.<br>Previously a device did not have to respond to a ping for Open-AudIT to continue scanning.'] = 'Ni tuhoaa reaktion al Ping. Se aro, Nmap Pugnoprovos senti kaj a shortskulti por ICMP-respondo. Se la aparato ne responseas, neniu plu skanado okazos.<br/>Anta';
...
$GLOBALS["lang"]['NOTE - You can prevent plain text credentials being displayed below by setting the configuration item for <code>decrypt_credentials</code> to <code>n</code>.'] = 'NEE - Vi povas malhelpi simplajn teksstajn akredita <code>decrypt_credentials</code> Autres <code>n</code>.';
...
$GLOBALS["lang"]['News is our way of keeping you up-to-date with the latest fixes, announcements, query updates, software versions and more.<br><br>You do not need to keep on top of the wiki or check for new releases, we can send these to you!'] = 'Nouvelle<br/><br/>Vi ne bezonas teni supre de la Vikio une réserve ';
...
$GLOBALS["lang"]['On the field <i>Match String</i> you should provide the name of the software that you want to track. You can use the percent sign (%) as a wildcard in where required.'] = 'Sur la kampo <i>Cierre de partido</i> Vi bruts provisioni la nomon de la programmaro, kiun vi volas printi. Vi povas uzi la procentan signon (%) kiel sova quicallyan karton en kie necesse.';
...
$GLOBALS["lang"]['One of: antivirus, backup, firewall, approved, banned, ignored or other.'] = 'Unu el: kontra katlustusvirirus, sekurmuro, aprobita, malpermeita, unittererib kaphust.';

$GLOBALS["lang"]['One other item to be careful of - make sure you use the hostname (or fqdn) of your LDAP Server that matches what is in the certificate (not just the LDAP Servers IP address) when creating the LDAP entry in Open-AudIT.'] = 'Uniu également objectkto por esti singarda de - certigu ke vi uzas la mastro-nomon (kahtlus fqdn) de via LDAP Servilo kiu egalas kio estas en la aestilo (ne ...';
...
$GLOBALS["lang"]['Open-AudIT can be configured to use LDAP servers (Microsoft Active Directory and/or OpenLDAP) to authenticate and authorize a user and in addition, to create a user account in Open-AudIT using assigned roles and orgs based on LDAP group membership. It can also use'] = 'Open-Aud IT povas esti formita por uzi LDAP-servilojn (Microsoft Active Directory kaj/a stided OpenLDAP) al a auditedtentikudo kaj rajtigi uzanton krome, krei uzantkonton en Open-Audit uzanta asignitajn rolojn kajn bazitajme surmbre ...';
...
$GLOBALS["lang"]['Open-AudIT uses Nmap in its Discovery function. Nmap probes devices for open ports. Devices ideally should respond with open for working ports. Devices often do not respond with this. The varying responses are detailed below.'] = 'Open-Audit uzas Nmap en Φia Discovery-funkcio. Nmap sondid aparatoj por malfermaj hasnoj. Destinivos idéale devus responsi kun malferma por labourhavenoj. Il s\'agit d\'une réaction de type al tio. La réponseoj estas detalaj malsupre.';

$GLOBALS["lang"]['Open-AudIT uses Nmap in its Discovery function. Without Nmap, Open-AudIT will not function correctly. Every Open-AudIT install assumes Nmap is installed and will also test for it on the Discovery web pages. More information about Open-AudIT and Nmap is available here'] = 'Open-Audit uzas Nmap en Φia Discovery-funkcio. Sen Nmap, Open-Audit ne funkcios εuste. Alternativaiu Open- Audit- instala.o supozas Nmap estas instalita kaj ankai testos por εi sur la Discovery retpa fapoj. Pli da informaoj pri Open-Audit kaj Nmap nämä haveblaj clari huulet';

$GLOBALS["lang"]['Open-AudIT uses Nmap in its Discovery function. Without Nmap, Open-AudIT will not function correctly. Every Open-AudIT install assumes Nmap is installed and will test for it on the Discovery web pages.'] = 'Open-Audit uzas Nmap en Φia Discovery-funkcio. Sen Nmap, Open-Audit ne funkcios εuste. Alternatituiu Open- Audit- instala.o supozas Nmap estas instalita kaj testos por εi sur la Discovery retpaγoj.';
...
$GLOBALS["lang"]['Other than that restriction, you are free to select attributes as required. It is a good idea to use the menu'] = 'Krom tiu restrikko, vi estas libera selekti atributojn kiel postulat. Estas bona ideo uzi la menuojn';

$GLOBALS["lang"]['Others'] = 'Aliaj aliasj';

$GLOBALS["lang"]['Our default Integration also enables a discovery on any externally supplied devices. So we ship a default discovery for the Default Integration out of the box. These use the default matching rules, but also enable matching on IP address.'] = 'Nia deha...... Tien ni ekspedis dea par défautltan eltrova Tiuj uzas la dea...lltajn egalajn regulojn, sed ankaы ebligas egalan IP-adreson.';
...
$GLOBALS["lang"]['Packages are used to report on things such as <i>banned software</i>, AntiVirus on servers, etc.<br><br>Open-AudIT provides widgets that build upon also supplied repots to detemine the status of antivirus, firewall and other software.'] = 'Paka <i>malpermesita programmearo</i>AntiVirus sur serviloj, ktp.<br/><br/>Open-Aud IT disponigas widgets kiuj konstruas sur anka...';
...
$GLOBALS["lang"]['Personal area network'] = 'Personnes';
...
$GLOBALS["lang"]['Pie charts must return name or my_name, description or my_description and count.'] = 'Tuhoaa pié-furorlisto Resendi Nomonin. Epäiltyni sur Preskribon.';
...
$GLOBALS["lang"]['Please have an Open-AudIT administrator logon and upgrade the database.'] = 'Bonvolu havi Open-Audit administrateur logon kaj ...isdatigi la datumbazon.';
...
$GLOBALS["lang"]['Please send all three files to your support contact at Opmantek and describe your issue.'] = 'Bonvolu sémi ';
...
$GLOBALS["lang"]['Policies'] = 'Politique';
...
$GLOBALS["lang"]['policy_id'] = 'Politique';

$GLOBALS["lang"]['Policy ID'] = 'Politique';

$GLOBALS["lang"]['Policy Result'] = 'Politique de politique';

$GLOBALS["lang"]['Policy Results'] = 'Politique';

$GLOBALS["lang"]['Polish'] = 'La pola';

$GLOBALS["lang"]['Polite'] = 'Politique';
...
$GLOBALS["lang"]['Portuguese'] = 'Portugal';
...
$GLOBALS["lang"]['Preferences'] = 'Préféré';

$GLOBALS["lang"]['Prepare for recertification every 3 years'] = 'Préparation';
...
$GLOBALS["lang"]['printer_port_name'] = 'Nombre de puerto de impressionnión';

$GLOBALS["lang"]['Printer Port Name'] = 'Nombre de puerto de impressionnión';
...
$GLOBALS["lang"]['Privacy Passphrase'] = 'Paroolifraas privés';
...
$GLOBALS["lang"]['processor_type'] = 'Produits';

$GLOBALS["lang"]['Processor Type'] = 'Produits';
...
$GLOBALS["lang"]['Queries are an important part of Open-AudIT. They are what enables you to get meaningful information from all the device data you have discovered. Along with summaries and groups, they provide a powerful mechanism to extract crucial information.'] = 'Päringud estas grava parto de Open-Audit. Ili estas kio rajtigas vin rizvi senchavajn informatojn de clariuj aparatoj kiujn vi malkovris. Kune kun ressumuj kaj grupoj, ili disponigas potencan mekanismon eltiri decidajn informatojn.';
...
$GLOBALS["lang"]['Queued IP Scans'] = 'Escaneos IP puestos en consultationa';

$GLOBALS["lang"]['Queued Items'] = 'Documents sollicités';
...
$GLOBALS["lang"]['Retrieve the first 10 devices running Windows ordered by hostname'] = 'Récupérer la unuaj 10 aparato prizorgantaj Fenestrojn ordonitajn fare de mastro-nomo';
...
$GLOBALS["lang"]['Retrieved from the device - Windows:Win32_ComputerSystemProduct, Linux:dmidecode, MacOS:system_profiler, ESXi:vim-cmd hostsvc/hostsummary, HP-UX:machinfo, Solaris:smbios, AIX:uname.'] = 'Prenite de la aparato - Fenestroj:Win32_ComputerSystemProduit, Linukso:dmidecode MacOS:system_profilar, ESXi:vim-cmd gastigantojvc/hostsummery, HP-UX:machinfo, Solaris: smbios, AIX: uname.';
...
$GLOBALS["lang"]['Return an individual device\\'] = 'Reveno de indica';
...
$GLOBALS["lang"]['Risk Management'] = 'Risquea administration';
...
$GLOBALS["lang"]['Roles can only be created and edited if you have an Open-AudIT Enterprise license. For most users, the default set of Roles should be all that is required.'] = 'Roloj povas nur esti kreitaj kaj redaktitaj se vi havas Open-Audit Enterprise licence. Por la pluj multaj uzantoj, la defa guidlta aro de Roloj devus esti Čiu kiu estas postulata.';
...
$GLOBALS["lang"]['Run the script by typing <code>cscript test_windows_client.vbs</code> in the console.'] = 'La manuskripto par tajpado <code>cscript test_windows_client.vbs</code> En la konzolo.';
...
$GLOBALS["lang"]['SQL not containing this condition will result in the widget failing to be executed.'] = 'SQL ne enhavanta tiun kondrircaon rezultigos la dracomingon malsukcesantan esti efekkivigita.';
...
$GLOBALS["lang"]['Save and run the file. Make sure you run the file as an Administrator in order to install it (right click, Run as Administrator).'] = 'Savi Kaj Prizorgi la dosieron. Certigu ke vi prizorgas la dosieron kiel administrateur por instali liquidin (deksstra klako, Run kiel administrateur).';
...
$GLOBALS["lang"]['Scheduling - discovery, reporting &amp; more'] = 'Programme - eltrova';

$GLOBALS["lang"]['scope'] = 'Ambito';

$GLOBALS["lang"]['Scope'] = 'Ambito';

$GLOBALS["lang"]['Scope is known as Authorization Scope, representing whether a vulnerability affects resources beyond the original security authority of the vulnerable component.<br><br>The only two valid values for this attribute are'] = 'Scope estas konata kiel Autoriseringu ulaitus, reprezentante .u vundebleco influas resursojn preter la origina sekurecadminstracio de la vundebla kompunto.<br/><br/>La nuraj du validaj valoroj por tiu atributo estas';
...
$GLOBALS["lang"]['Searching Using DataTables'] = 'Serrugaante Uzante etmetabelid';
...
$GLOBALS["lang"]['Select Menu -> Manage -> Baselines -> List Baselines.'] = 'Elektu menüüon - confier Mana graduano - confier Bazlinioj - Lista lähtötilanteet.';
...
$GLOBALS["lang"]['Select the <i>COM Security</i> tab<br><br>Click on Access Permissions Edit Default'] = 'Elektu <i>KOM Sekureco</i> kla retorn<br/><br/>Klaku sur Aliro Lubaduse Defa Editer...';
...
$GLOBALS["lang"]['Set by the audit_domain or discover_domain scripts. Do not set.'] = 'Metite fare de la revizoro épäilty malkovri_domain manuskriptoj. Ne metu.';
...
$GLOBALS["lang"]['Should we apply this license to the selected Org as well as the Orgs children?'] = 'Отели вали ват ван не permesilon al laekteja Vous avez mismo kiel la infanoj de Orgs ?';

$GLOBALS["lang"]['Should we audit a SAN if the management software is detected.'] = 'Se la administrateur programaro estas detektita.';
...
$GLOBALS["lang"]['Should we hide the audit script window when executing.'] = 'Se ni ka kindu la a scepticatorian fanestronikiam oni ekzekutas.';
...
$GLOBALS["lang"]['Should we retrieve all DNS names'] = 'Ni prenu Governanceiujn nomojn';
...
$GLOBALS["lang"]['Should we uninstall the agent (y/n).'] = 'S. explitalosu la agenton (jah/ei).';
...
$GLOBALS["lang"]['Slovakia (Slovak Republic)'] = 'Slovaquie';

$GLOBALS["lang"]['Slovenia'] = 'Slovénie';

$GLOBALS["lang"]['Slovenian'] = 'La Slovène';
...
$GLOBALS["lang"]['snmp_enterprise_id'] = 'Numéro d\'identification';

$GLOBALS["lang"]['Snmp Enterprise ID'] = 'Numéro d\'identification';
...
$GLOBALS["lang"]['So a script is not working on a recalcitrant device. Grrr. The scripts for Windows, Linux, and MacOS all accept the debugging argument. This can be set in the script itself, or provided on the command line. Run the script using that option set to 5 and you should see in which section of the script the failure occurs. For example'] = 'Tiel manuskripto ne laboras pri rekalsitant aparato. La manuskriptoj por Fenestroj, Linukso, kaj MacOS Čiuj akceptas la malkonstruan argumenton. Tio povas esti metita en la manuskripton mem, epäilty disponigita sur la komandlinio. Run la manuskripto uzanta tiun opcion metian al 5 kaj vi devus vidi en kiu sekcio de la manuskripto kiun la fiasko okazas. Ekzample';

$GLOBALS["lang"]['So apart from a regular word, what other options are there - I am glad you asked! As per the standard Open-AudIT API, we have several operators for filtering. Operators that can precede the value are'] = 'Kromregul vorto, kion alphaj opcioj estas corbatas - mi εojas ke vi demandis! Kiel per la norma API Open-Audit, ni havas plurajn funkciigistojn por filtrado. Opérateuroj kiuj povas anta.i la Valon estas';

$GLOBALS["lang"]['So you have an issue providing Open-AudIT credentials to discover your devices. I have an answer - dont then! Sure, I mean, discovery is the best thing since sliced bread. You do not need to know ahead of time <i>Whats On Your Network</i>. But how can you extract data from devices without providing credentials?<br><br>In the case of computers, an easy option is to copy the appropriate audit script to the target machines and set it to execute on a schedule. More details on the wiki. The machines will send their data to Open-AudIT on that schedule, almost as if you were running discovery. The device simply runs the audit script on a schedule (again, you can read the audit script) as the user you tell it to and sends the output to the server. No credentials are involved at all.<br><br>And now we have Agents for Windows. The advantages of an actual installed Agent are'] = 'Tiel vi havas temon disponigantan Open-Audit-kredita.ojn por malkovri viajn aparatojn. Mi havas responsonon - ne plu! Mi pensas, ke malkovro estas la pléj bona a.o ekde tran. Vi ne bezonas scii antaluc <i>Kio estas sobre Via Reto</i>. Kiel vi povas eltiri datumojn de aparatoj sen provisioni akredita...<br/><br/>Koncerne komputilojn, facila opcio dedes kopii la konvenan reviziomanuskripton al la celma negociainaj kaj meti εin por efektivigi horaron. Pli da detaloj sur la Vikio. La ma apretinoj sentos siajn datumojn al Open-Audit en tiu horaro, preska La aparato simple prizorgas la reviziomanuskripton en horaro (deno, vi povas legi la reviziomanuskripton) kiel la uzanto vi rakontas εin al kaj sendas la produkta. Neniuj akredita.oj estas implikitaj entute.<br/><br/>Nun ni havas agentojn por Vindozo. La avanta pilaoj de fakta instalita agento estas';
...
$GLOBALS["lang"]['Software Found Last 7 Days'] = 'Programme de recherche la 7 Tagojn';
...
$GLOBALS["lang"]['Somalia, Somali Republic'] = 'somalio, soma republiko';
...
$GLOBALS["lang"]['Some examples are at the bottom of this page. All endpoints also have a minimum list of required fields. These are below.'] = 'Kelkaj ekzemploj estas Régie la fondo de tiu pa fusiono. Imitaiuj finpunktoj anka. havas miniman Listen de postulataj kampoj. Mina sentio malsupre.';
...
$GLOBALS["lang"]['speed_down_a'] = 'Rapida Malsupre A. A. A. A.';

$GLOBALS["lang"]['Speed Down A'] = 'Rapida Malsupre A. A. A. A.';
...
$GLOBALS["lang"]['sub_resource_id'] = 'Numéro d\'identification Suba Rimedo';

$GLOBALS["lang"]['Sub Resource ID'] = 'Numéro d\'identification Suba Rimedo';
...
$GLOBALS["lang"]['Subscription ID'] = 'Sous-kribo-tunnus';
...
$GLOBALS["lang"]['Sudan'] = 'Soudano Soudano';
...
$GLOBALS["lang"]['Support'] = 'Sous-téno';
...
$GLOBALS["lang"]['Svalbard & Jan Mayen Islands'] = 'Svalbardo et Jan Mayen Insuloj';
...
$GLOBALS["lang"]['Tasks can be one of several distinct types: Baseline, Discovery, Report, Query, Summary or Collector.'] = 'Taskoj povas esti unu el pluraj apartaj tipoj: Bazlinio, Discovery, Raporto, Query, Yhteenveto épäilty kolektanto.';
...
$GLOBALS["lang"]['Thank you for starting your journey with the best discovery and inventory tool around.'] = 'Dankon pro via voja financiacióno kun la plei bona malkovro kaj inventaro.';
...
$GLOBALS["lang"]['The Applications endpoint allows you to define an application which you can then associate to a device (or devices).'] = 'La Aplicoj finpunkto permesas al vi difini petskribon kiun vi tiam povas asocii al aparato (a.a.a.';
...
$GLOBALS["lang"]['The OS this package should be reported against. Blank for all. Use the percent sign % as a wildcard. Will be tested against os_group, os_family and os_name in that order.'] = 'La OS tiu paka.o devus esti raportita kontra épäilty. Pour Oni uzas la realan modon (AS-finajxon). Estos testita kontra stide os_group, os_familio kaj ostnomo en tiu pesekvo.';
...
$GLOBALS["lang"]['The URL of your Open-AudIT Server that this Collector will report to (no trailing slash).'] = 'URL de via Open-Audit Servilo kiun TIu kokonto raportos al (neiu malanta transfertran círculoo).';

$GLOBALS["lang"]['The URL the audit scripts should submit their result to.'] = 'La URL-manuskriptoj devus submeti sian rezulton.';

$GLOBALS["lang"]['The User endpoint allows you to manage user accounts within Open-AudIT.'] = 'La Uzanta finpunkto permesas al vi administr uzantkontojn ene de Open-Audit.';
...
$GLOBALS["lang"]['The application creates temporary audit scrtips here, copies them to the target machines, then deletes them.'] = 'La apliko kreas suppezorajn revizorojn Kopias ilin al la celma divertaiinoj, tiam forigas ilin.';
...
$GLOBALS["lang"]['The column name from the foreign table. Should be one of: class, environment, status, type or menu_category.'] = 'La kolonnomo de la fremda tablo. Devus esti unu el: klaso, medio, statuo, tipo epäillyn valikko_ luokka.';
...
$GLOBALS["lang"]['The date this item was changed or added (read only). NOTE - This is the timestamp from the server.'] = 'La dato tiu objectkto estis дальна Tio estas la tempostapo de la servilo.';
...
$GLOBALS["lang"]['The debug level (will output to command prompt window). Lower means less output with 0 being no output.'] = 'La debug nivelo (volas kommundi prostan fanestroni). Pli malta signifas malpli produkta.';
...
$GLOBALS["lang"]['The exploited vulnerability can affect resources beyond the security authority of the vulnerable component. This usually means a privilege or trust boundary is crossed (for example, escaping from a container to the host OS).'] = 'La ekspluatita vundebleco povas infui resursojn preter la sekurecadminstracio de la vundebla kompunto. Tio kutime signifas saltan a fidolimon estas krucita (ekzample, eskapante de ujo εis la gastiganto Os).';

$GLOBALS["lang"]['The exploited vulnerability only impacts resources within the same security authority as the vulnerable component. In other words, the attacker’s actions stay within the original trust boundary.'] = 'La ekspluatita vundebleco nur trafas resonsojn ene de la sama sekurecadminstracio kiel la vundebla kompunto. Esimerkiksi J vortoj, la agoj de la atakanto restas ene de la originea fidolimo.';
...
$GLOBALS["lang"]['The identifier column (integer) in the database (read only).'] = 'La identigilo (entier) en la datumbazo (legita nur).';
...
$GLOBALS["lang"]['The last time this benchmarks was executed.'] = 'La Lastan fojon tii kompranormo estis ekzekutita.';
...
$GLOBALS["lang"]['The locations room.'] = 'La gouvernambro de la lokoj.';
...
$GLOBALS["lang"]['The password for the dn_account attribute.'] = 'La pasvorton por la dn account atribuas.';
...
$GLOBALS["lang"]['The physical height (in CMs) of the rack.'] = 'La fizika alteco (fr CM) de la rako.';
...
$GLOBALS["lang"]['The purpose of this licensing page is to list the products and licenses included in Firstwave Commercial products including products based upon open source works.Open-AudIT is licensed from Firstwave under AGPLv3 or later with Enterprise and Professional functionality licensed under commercial terms.Open-AudIT when downloaded from FirstWave may include the following libraries and projects, which are supplied unmodified and licensed as indicated:'] = 'La celo de tiu licencadpa graduateo breacs listigi la produktojn kaj licencojn inkluditajn en Firstwave Commercial-produktoj inkluzive de produktoj bazitaj sur malfermfontecverkoj. Open-Audit estas licencita de Firstwave sub AGPLv3 a respect poste kun Enterprise kaj Profesia funkcieco licencita sub komercaj esprimej. Open-Audit kiam el apretutite de FirstWave povas inkludi la sekvajn bibliotekojn kaj projektojn, kiuj estas maksataj nemodifita kaj licencita kiel indikite:';

$GLOBALS["lang"]['The query that provides a list of devices for the integration. Links to <code>queries.id</code>.'] = 'La atendo kiu disponigas Listen de aparatoj por la integri/1993/o. Ligiloj al <code>queries.id</code>.';
...
$GLOBALS["lang"]['The requested properties should be in a comma-separated list. Properties should be fully qualified - ie, system.hostname (not just hostname).'] = 'La petite trajtoj devus esti en kom-separita lista. Trajtoj devus esti plene kvalifikitaj - t.e., sistemo.';
...
$GLOBALS["lang"]['The software end-of-life date.'] = 'La programmation fino de viva dato.';

$GLOBALS["lang"]['The software end-of-service-life date.'] = 'La programmation fino de servo viva dato.';

$GLOBALS["lang"]['The specific database column from the specified database table.'] = 'La specifa datenarkolono de la precizigita base de donnéestablo.';
...
$GLOBALS["lang"]['The steps below outline the process.'] = 'La юатитов sub skizas la procezon.';
...
$GLOBALS["lang"]['Then you can use Open-AudIT (unsupported) to receive audit data directly from the clients. Discovery <strong>will fail</strong>, but if youre not using Discovery (the main feature Open-AudIT was designed around), you can still get device data. You would need to run the audit scripts directly on the clients using cron, or use the Windows Agent feature (Enterprise only).'] = 'Tiam vi povas uzi Open-Audit (nepruvita) por rizvi reviziodatenojn rekte de la klientoj. Découverte <strong>malsukcesos</strong>, sed se vi ne uzas Discovery (la Vi devus prizorgi la reviziomanuskriptojn rekte sur la klientoj uzantaj Kronon, a. uzi la Windows Agent trajton (Enterprizo nur).';
...
$GLOBALS["lang"]['There is no need to do anything if you are running Open-AudIT on a Linux server.<br><br>Windows clients are just fine and require no special actions, however.... to enable this feature the audit script must be run locally on the target Windows system. It cannot be run remotely as we do with WMI calls when running the audit script on one Windows machine, while targeting a second Windows machine. To do this we need to copy the audit script to the target Windows machine and then run it. Unfortunately the service account that Apache runs under is the Local System account. This account has no access to remote (network based) resources. To work around this issue the service must be run under another account. It is easiest to just use the local Administrator account, but you can try any account you like as long as it has the required privileges. The Local System account has as much local access as the local Administrator account.'] = 'Eksistas neniu bezono farenda.o io ajn se vi prizorgas Open-Audit sur Linukso-servilo.<br/><br/>Fenestroj estas justaj kaj postulas neniujn specialajn agojn, aliflanke ... rajtigi tiun trajton la reviziomanuskripto bruces et prizorgita loke sur la cel Windows-sistemon. Por farition ni murs kopii la reviziomanuskripton al la celo Windows-ma agotadono kaj tiam prizorgi εin. Beda auditrinde la servikonto ke apa planekuroj sub estas la Local System-konto. Tiu raporto havas neniun aliron al malproksimaj (reto bazita) resursoj. Por labouri ...irka epäilty tiu temo la servo brunces esti prizorgita sublison raporton. Estas plej facile uzi la lokan Administrateur konton, sed vi povas provision ajnan raporton vi kiel lowe kiel εi havas la postulatajn saltjn. La Loka Sistemo-konto havas tien multe da loka aliro kiel la loka Administramento-konto.';

$GLOBALS["lang"]['These are only examples. You may need to adjust these attributes to suit your particular LDAP.'] = 'Tiuj estas nur ekzemploj. LDAP est un programme d\'éducation et de formation pour les jeunes.';

$GLOBALS["lang"]['These parameters enable you to audit only what is useful and relevant to you, saving processing time and allowing you to discover your network in an orderly manner.'] = 'Tiuj parametroj ebligas al vi revizii nur kio estas utila kaj signifa al vi, шparante pretigtempon kaj permesante al vi malkovri vin reton en bonorda maniero.';
...
$GLOBALS["lang"]['This code relys on the great work done by the'] = 'Tiu kodo adpendenteas de la granda laboro farita per la';
...
$GLOBALS["lang"]['This example creates a list of devices where the Function or Description fields are blank OR the Purchase Date is the default.'] = 'Tiu ekzemplo kreas Listn de aparatoj kie la Funkcio a Kaptlus Preskribo kappoj estas brilaj VÕI la Avisioneta.dato estas la dea defa skeptlto.';
...
$GLOBALS["lang"]['This file tells Linux to poll the application to determine if there are any scheduled tasks that should be run.'] = 'Tiu dosiero rakontas al Linukso sondi la petskribon por determini Eksistas juj planitaj takoj kiuj devus esti prizorgitaj.';
...
$GLOBALS["lang"]['This is likely due to MySQL time being off. MySQL typically uses the host devices time. You can check this by'] = 'Tio ver apretajne рuldi MySQL pointe utiligas la mastro-aparaton tempon. Vi povas kontroli';
...
$GLOBALS["lang"]['This should be the fully qualified field name in the database. Currently columns from the system, fields and credentials tables are supported. EG: system.nmis_role.'] = 'Tio devus esti la tute kvalifikita kampa nomo en la datumbazo. Nuntempe kolonoj de la sistemo, kamopj kaj akredita. oj estas apogitaj. EG: sistemo.nmis_role.';

$GLOBALS["lang"]['This state is used when Nmap is unable to determine whether a port is closed or filtered. It is only used for the IP ID idle scan.'] = 'Tiu шtato estas uzita kiam Nmap estas nekapabla determini Ei estas fermita a Kahtlusfilita. Estas nur uzita por la IP ID joutokäynti-skanado.';
...
$GLOBALS["lang"]['To select devices to be integrated, we use nmis_manage = y, but you might like (for example) type = router. You can also use a Group or Query if you prefer something more complex.'] = 'Por selekki aparatojn por esti integrita, ni uzas nmis_manage = y, sed vi eble choratas (ekzample) tipon = itineraton. C\'est vrai. Kysely se vi preferas ion pli kompleksan.';

$GLOBALS["lang"]['To sort by a database column, use'] = 'Por ordigi por datenarkolono, uzo';

$GLOBALS["lang"]['To use this Enterprise feature, you first need to create your user in Open-AudIT as per usual. Assigning a password is not necessary. You do have to assign Roles and Orgs. The username in Open-AudIT should match the Entra <i>preferred username</i> attribute. There is no need for a Full Name or Email - these will be populated from Entra. If you do not know your users <i>preferred username</i>, fear not. You can create the new Auth Method in Open-AudIT and the Enterprise Application in Entra and when a user attempts to sign-on to Open-AudIT without a pre-existing user, check the logs and you will see their <i>preferred username</i> logged for your convienence.'] = 'Por uzi tiun Enterprise trajton, vi unue murtaa krei vian uzanton en Open-Audit kiel per kutima. Necesas Pasvorton. - Oui. La uzantinomo en Open-Audit devus egali la Entra <i>Préfiereata uzantnomo</i> Atribute. Eksistas neniu bezono de plena nomo aы repo chetto - tiaj estos lo fusionitaj de Entra. L\'Union européenne <i>Préfiereata uzantnomo</i>Netimu. Entra kaj kiam uzanto provas subskribi al Open-AudIT sen sen anta advidekzista uzanto kontroli la tagalojn kaj vidos lian <i>Préfiereata uzantnomo</i> Kaj vi venis al via Batalo.';

$GLOBALS["lang"]['To use this Enterprise feature, you first need to create your user in Open-AudIT as per usual. Assigning a password is not necessary. You do have to assign Roles and Orgs. The username in Open-AudIT should match the OKTA <i>name</i> attribute. There is no need for a Full Name or Email - these will be populated from OKTA. If you do not know your users <i>name<i>, fear not. You can create the new Auth Menthod in Open-AudIT and the Application in OKTA and when a user attempts to sign-on to Open-AudIT without a pre-existing user, check the logs and you will see their <i>name<i> logged for your convienence.<br></p><p>Then create a new Auth method in Open-AudIT by going to menu -> Admin -> Auth Methods -> Create Auth. Provide a name and set the <i>type</i> to Okta.'] = 'Por uzi tiun Enterprise trajton, vi unue murtaa krei vian uzanton en Open-Audit kiel per kutima. Necesas Pasvorton. - Oui. La uzantinomo en Open-Audit devus egali la OKTA <i>nomo</i> Atribute. Eksistas neniu bezono de plena nomo a Se vi ne konas viajn uzantojn <i>nomo<i>Netimu. Vio povas krei la novan Auth Menthod en Open-Audit kaj la Aplikiεon en OKTA kaj kiam uzanto provas subskribi al Open-Audit sen anta...eksista uzanto kontroli la tagalojn kaj vidos lian <i>nomo<i> Kaj vi venis al via Batalo.<br/><p>Tiam krei novan Auth-metodon en Open-Audit irante al menuo - confar Admin - título Auth Metodoj - título Krei Auth. Prénu nomon kaj metis la <i>tipo</i> Oui.</p></i></i></i></i>';
...
$GLOBALS["lang"]['Turks and Caicos Islands'] = 'Turko kaj Caïque Insuloj';
...
$GLOBALS["lang"]['Type: This can be either a varchar (a text field) or a List (a list of values that can be selected).'] = 'Tipo: Tio povas esti a sceptic varchar (tekstokampo) a sceptic Listo (listo de valoroj kiuj povas et selektititaj).';
...
$GLOBALS["lang"]['Unfiltered'] = 'Néfiltrita';

$GLOBALS["lang"]['uninstall'] = 'Désinstalación';

$GLOBALS["lang"]['Uninstall'] = 'Désinstalación';

$GLOBALS["lang"]['Uninstall the Agent'] = 'Désinstalar el agente';
...
$GLOBALS["lang"]['Use Service Version Detection. When a detected port is detected as open, if set to <i>y</i>, Nmap will query the target device in an attempt to determine the version of the service running on this port.<br>This can be useful when identifying unclassified devices. This was not previously used.'] = 'Uzu versión de la versio. Kiam detektita hasno estas detektita kiel malferma, se metite al <i>Oui.</i>Nmap request la celaparato en promo determini la version de la servo kuranta sur tiun hasnon.<br/>Tio povas esti utila kiam identifigi nesekretajn aparatojn. Tio ne estis anta... uzata.';

$GLOBALS["lang"]['Use WMI'] = 'Uzu WMI';

$GLOBALS["lang"]['Use a <i>Sign-in method</i> of OIDC - OpenID Connect and an <i>Application Type</i> of Web Application. Click <i>Next</i>.'] = 'Uzu la <i>Métodoosi sisselogimiini</i> OIDC - OpenID Konektu kaj <i>Apliki tipon</i> Site Web d\'excuses. Klakou al Click <i>La Sekva</i>.';
...
$GLOBALS["lang"]['Used when searching OpenLDAP to match a users uid to a groups members. Default of <code>memberUid</code>. Used by OpenLDAP only.'] = 'Uzita dum ser reveladoado Le programme OpenLDAP por egali uzantojn kojn al grupoj membroj. Defa defultlto de <code>memberUid</code>. Uzita fare de OpenLDAP nur.';
...
$GLOBALS["lang"]['user_interaction'] = 'Autres';

$GLOBALS["lang"]['User Interaction'] = 'Autres';

$GLOBALS["lang"]['User Membership Attribute'] = 'Afrique. kgm';

$GLOBALS["lang"]['user_name'] = 'Uzannomo Nom de la ville (facultatif, sans doute pas besoin de traduction)';

$GLOBALS["lang"]['User Name'] = 'Uzannomo Nom de la ville (facultatif, sans doute pas besoin de traduction)';

$GLOBALS["lang"]['User Policies'] = 'Uzanta Politikojn';

$GLOBALS["lang"]['Username'] = 'Uzannomo Nom de la ville (facultatif, sans doute pas besoin de traduction)';
...
$GLOBALS["lang"]['Users work similar to Netstat Ports. If a user exists with a matching name, status and password details (changeable, expires, required) then the policy passes.'] = 'Uzantoj laboras similaj al Netstat Puertos. Se uzanto eksistas kun egala nomo, stato kaj pasvorton detaloj (publikaan progressiveblaj, eksvalidias, postulis) tiam la politiko pasas.';
...
$GLOBALS["lang"]['View All'] = 'Riikardi Governanceiujn';

$GLOBALS["lang"]['View Details'] = 'Vidû detalojn';

$GLOBALS["lang"]['View Device'] = 'Destitivo Vido';

$GLOBALS["lang"]['View Discovery'] = 'Découverte de la vido';

$GLOBALS["lang"]['View Policy'] = 'Politique';

$GLOBALS["lang"]['View Racks'] = 'Raquettes Vidu';
...
$GLOBALS["lang"]['vlan_id'] = 'Identification Vlani';

$GLOBALS["lang"]['Vlan ID'] = 'Identification Vlani';
...
$GLOBALS["lang"]['vm_ident'] = 'Identification Vm';

$GLOBALS["lang"]['Vm Ident'] = 'Identification Vm';
...
$GLOBALS["lang"]['When this benchmark is scheduled to be executed. Derived from the associated <code>tasks.type</code> and <code>tasks.sub_resource_id</code>.'] = 'Kiam tiu kompranormo estas planita por esti efektivigita. Tultatud de la rilata <code>tasks.type</code> kaj <code>tasks.sub_resource_id</code>.';
...
$GLOBALS["lang"]['When using Open-AudIT Professional or Enterprise, widgets are available and customisable to be displayed on Dashboards.'] = 'Dum uzado de Open-Audit Professional a. Enterprise, widgets estas hasblaj kaj aptitaj por esti elmontritaj sur Dashboards.';

$GLOBALS["lang"]['When we are converting an internally selected device to an object for the external system, what should the external data be. EG: integer, string, etc.'] = 'Kiam ni transformas interne elekttitan aparaton al objektto por la ekstera sistemo, kio devus la eksteraj datenoj esti. Entjero, grinuro, ktp.';
...
$GLOBALS["lang"]['Whether the vulnerability affects components beyond its own using Impact, Availability and Confidentiality.'] = ' imitau la vundebleco influas kompentojn preter sia propra uzante Impact, Disponibilidad kaj Konfidentsiaalsus.';
...
$GLOBALS["lang"]['Which network should we discover. This should be in the format of'] = 'Kiun reton ni devus malkovri. Tio devus esti en la forme de format';

$GLOBALS["lang"]['Which sub-menu should we display this query in.'] = 'kiun sous-menu ni murens montri tiun atendon fr.';
...
$GLOBALS["lang"]['You are able to define extra directories to be scanned as entries in the Executables collection. When defined, the directory and all sub-directories are scanned for files that are executable. You can also define directories and files to be excluded. When activated, these are injected into the linux audit script and performed whenever it is run (usually as part of a discovery). If you want these in a script you can manually copy to the target, use menu -> Discover -> Scripts -> List Scripts and download the script from there.'] = 'Vi povas dfini ekstrajn adresarojn por esti skanitaj kiel kontribouj en la Exécutables kolekto. Se difinite, la adresaro kaj †iuj sub-direktoj estas skanitaj por dosieroj kiuj estasvi efektigeblaj. V i ankai povas difini adresarojn kaj dosierojn esti ekskluditaj. Se aktivigite, tiaj estas injektitaj en la lin guidrevizian manuskripton kaj rezultis kiam ajn εi estas prizorgita (kutime kiel parto de eltrova.o). Se vi deziras tiujn en manuskripto vi povas mane kopii al la celo, uzi menoon - Malkovru - título Lista Scripts kaj el sortutas la manuskripton de lie.';

$GLOBALS["lang"]['You are always free to modify the audit scripts themselves to suit your specific requirements. You will need to (re)incorporate those changes whenever you upgrade as they will be overwritten by the default scripts. I would suggest saving a master copy of your modified script, upgrading Open-AudIT, then running a diff between the new default script and your modified script and applying the differences. The audit scripts are native scripts (Windows is VBScript, all others are Bash). Simple and easy to modify - just make sure to have a copy of your changes before you upgrade.'] = 'Vi L\'utilisation de l\'énergie nucléaire dans l\'industrie de l\'électricité et de l\'électricité n\'est pas considérée comme un problème. Mi rekomendas рpari majstran kopion de via modifita manuskripto, εisdatigante Open-Audit, tiam kurante fosaon inter la nova defa... La reviziomanuskriptoj estas indiilibleenaj manuskriptoj (Windows estas VBScript, Simpla kaj facila por modifi - nur certigu havi kopion de viaj рan pilaoj anta. Le taux d\'activité de l\'industrie de l\'Union européenne est passé de 2 % à 2 %.';
...
$GLOBALS["lang"]['You can also see at the bottom of the page a table showing the devices the changes will be applied to.'] = 'Vi ankaы pos vidi Čevae la fondo de la pa. o tablon montrantan la aparatojn kiujn la яариноюoj estos aplikitaj al.';
...
$GLOBALS["lang"]['You do not have the PHP SNMP extension installed. This extension is highly recommended.'] = 'L\'objectif de cette initiative est d\'améliorer la qualité de l\'information et de la communication. Tiu etendaoo estas tre rekomendinda.';
...
$GLOBALS["lang"]['You should substitute the required column (eg, org_id) for ATTRIBUTE_NAME.'] = 'L\'auteur de l\'avis est M. Vi devus anstatalucigi la postulatan kolononon (ekz., org_ id) por ATTRIBUTE_ NIMETUS.';
...
$GLOBALS["lang"]['You should use only those tables that contain attributes you need. I usually use a left join. For example'] = 'Devus uzi nur tiujn tabelojn kiuj enhavas atributojn vi bezonas. Mi kutime uzas maldekstran ali liquidon. Ekzample';
...
$GLOBALS["lang"]['Your user should now be able to logon by clicking the <i>Logon with Entra</i> button on the Open-AudIT logon page. As mentioned above, if the logon fails, please check the logfile and verify the users <i>preferred username</i>.<br><br>And that should be all there is too it. Now your users should have one less set of credentials to remember!<br><br>As always, if you have troubles with this feature, please email'] = 'Via uzanto nun povas logo klaki la <i>Kun Logon Entra</i> Butono sur la Open- Audit- tunnuksenoplasiao. Kiel menciite supre, se la logo malsukcesas, bonvolu kontroli la tagdosieron kaj konfirmi la uzantojn <i>Préfiereata uzantnomo</i>.<br/><br/>Kaj tio devus esti . Nun viaj uzantoj devus havi unu malpli aron de akredita...oj por memori!<br/><br/>Kiel clariam, se vi havas problemojn kun ';
...
$GLOBALS["lang"]['and download the Latest Release self-installer. You may need to scroll down the page to find it.'] = 'Kaj el sortu la Lasto-Liberigan meminstaliston. Vio povas devi ruligi la pa';

$GLOBALS["lang"]['and go to Applications -> Enterprise Applications (far left menu), then click <i>New application</i>.'] = 'Kaj iru al Aplikoj - Propiedad Aplicaciones (menu maldekstro demalproksima), tiam klakas <i>Nouvelle aplika</i>.';
...
$GLOBALS["lang"]['and the final lines will likely be of most interest. These lines should give you the exact point at which the login failed.'] = 'La finaj linioj ver sortajne estos de plei multe de la intereso. Tiuj linioj devus doni al vi la precizan punkton Malsukcesis.';
...
$GLOBALS["lang"]['column will contain either <i>y</i> or <i>n</i>. This indicates if this row is currently present on the device. For example software may be installed (which would result in <i>software.current = y</i>), but on a subsequent audit it may not be detected. Open-AudIT will then change this rows current attribute to <i>n</i>.'] = 'La kolumno tekitab kaplust <i>Oui.</i> a <i>n</i>. Tio indikas se tiu vico clareestas nuntempe sur la aparato. Ekszample, softvaro povas et instalita (kiu rezultivus en) <i>Komputilo = y</i>Sed sur posta revizio εi ne povas esti detektita. OpenAudit tiam рan pilaos tiun vicojn aktualan atributon al <i>n</i>.';

$GLOBALS["lang"]['column. The'] = 'Kolono. La';

$GLOBALS["lang"]['compute'] = 'Komputilo';

$GLOBALS["lang"]['config'] = 'Pour les';
...
$GLOBALS["lang"]['does <strong>not</strong> provide vulnerability reports for the following'] = 'de loin <strong>Oui.</strong> Provizas vundeblecojn raportojn por la sekvanta';
...
$GLOBALS["lang"]['if you clone a Linux virtual machine, unless you manually regenerate this (and in my experience, people do not) it will remain the same.'] = 'Se vi klonas Linukso virtuala ma apretino, se vi mane regenrastion (kaj en mia sperto, homoj ne) εi restos la sama.';

$GLOBALS["lang"]['ignored'] = 'des';
...
$GLOBALS["lang"]['is an international standard for managing information security. It provides a framework for establishing, implementing, maintaining, and continually improving an Information Security Management System (ISMS).'] = 'estas internationalcia normo por administrado de informasekureco. ';
...
$GLOBALS["lang"]['key required to retrieve lat/long and display map. See'] = 'рlosilo postulata por preni lat/longan kaj ekranmapon. Vitume';
...
$GLOBALS["lang"]['notification'] = 'Néificación';
...
$GLOBALS["lang"]['or the GitHub page at'] = 'a la GitHub-pacienteo Tšehhi';
...
$GLOBALS["lang"]['team'] = 'Équipea Teamo';

$GLOBALS["lang"]['text. The first is the array index and should be left as-is. The index is used in the templates to lookup the translated text.'] = 'Teksto. La unua estas la aro-indekso kaj devus esti lasita kiel-estas. La Indekso estas uzita en la юалеblonoj por levi la tradukitan tekston.';

$GLOBALS["lang"]['then clicking on the details button for the table.'] = 'sobre sur; pri; je supérieur supra; рúdorso supérieur';

$GLOBALS["lang"]['then clicking on the details button for the table. Device details are stored in the'] = 'somre sur; pri; je superior supra; тudoorso superior Destitivo detaloj estas stokitaj en la';

$GLOBALS["lang"]['timestamp'] = 'La tempo';

$GLOBALS["lang"]['to'] = 'Autres';

$GLOBALS["lang"]['to create an item of this type'] = 'eron de ';

$GLOBALS["lang"]['to the URL, you will get the full database entry, including all component tables, of a given device.'] = 'Al la URL, vi ricevos la plenan databasen eniron, inkluzive de ...iuj kompontoj tabloj, de anta.fiksita aparato.';
...
Konfidencivo - Armea/registro kaj Secteur privado-klasifiko. Datenoj kiuj estas konfidencaj inkludas komercsekretojn, intelektajn datenojn, apliki corroboraoprograman kodon, kajelijn datennojn kiuj povis grave influi la organison se nealuctorizita rive okloazis. Datenoj sur tiu nivelo estus hasblaj nur al personaro en la organiso kies laborbezonoj, a. estas rekte rilatitaj al, la aliritaj datenoj. Aliro al konfidencaj datennoj kutime postulas aprobon por

Privata sektoro. Datenoj kiuj estas privataj inkludas jujn ajn informatojn ligitajn al personaro, inkluzive de homarajtaj rekordoj, kuracistaj rekordoj, kaj salajroinformoj, kiu estas uzita nur ene de la organiso.

Sentema - Armea/registraro kaj Privatsektora klasifiko. Datenoj kiuj estas sentiemaj inkludas organizajn financajn informatojn kaj postulas ekstrajn iniciatojn certigi εian CIA kaj precizecon. Divulgación povas damaεi la nacian sekurecon.
...
Neklasifikita - Armea/registrara klasifiko. Mezclaiuj informaoj kiuj povas εenerale esti distribuitaj al publiko sen iu minaco al nacia intereso.
...
$GLOBALS["lang"]['unchanged'] = 'sénican';
...
$GLOBALS["lang"]['web'] = 'WEB WEB WEB WEB WEB WEB WEB';

$GLOBALS["lang"]['with Local and Remote access permissions set.'] = 'Kun lokaj kaj Remote-aliro permesoj metis.';

$GLOBALS["lang"]['with devices::update permission.'] = 'C\'est moi. Isdatpermeso.';

$GLOBALS["lang"]['with the Sign-On Redirect URI being'] = 'La Sign-Sur-imitaefo-Amba.o';

$GLOBALS["lang"]['with the intention of providing simple and intuitive access in a manner familiar to developers. In addition to this API, the web interface will use the same request format and supply some additional actions (eg: HTML forms for creating items).'] = 'Kun la intenco de disponigado de simpla kaj intuicia aliro en maniero konata al programmistoj. Aldone al tiu API, la interretinterfaco uzos la saman peti formaton kaj suppézos kalkajn kromajn agojn (ekz. HTML-formularoj por kreado de eroj).';

$GLOBALS["lang"]['yes'] = 'Jessus.';

$GLOBALS["lang"]['you do not require a HighCharts license to use Firstwave products. License'] = 'Vi ne postulas HighCharts-licencon uzi Firstwave produktojn. Licence';
