                    ...
                    <div class="row">
                        <div class="col-6">
                            ...
                            <?php if ($resource->type === 'amazon') {
                                echo read_field('credentials.key', $resource->credentials->key, $dictionary->columns->key, $update);
                                echo read_field('credentials.secret_key', '', $dictionary->columns->secret_key, $update);
                            } else if ($resource->type === 'microsoft') {
                                echo read_field('credentials.subscription_id', $resource->credentials->subscription_id, $dictionary->columns->subscription_id, $update, 'Subscription ID');
                                echo read_field('credentials.tenant_id', $resource->credentials->tenant_id, $dictionary->columns->tenant_id, $update, 'Tenant ID');
                                echo read_field('credentials.client_id', $resource->credentials->client_id, $dictionary->columns->client_id, $update, 'Client ID');
                                echo read_field('credentials.client_secret', '', $dictionary->columns->client_secret, $update, 'Client Secret');
                            } else if ($resource->type === 'google') {
                                echo read_field('credentials.json', '', $dictionary->columns->json, $update, "Google JSON Credentials");
                            } ?>
