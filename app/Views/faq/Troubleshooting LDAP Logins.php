...
$intro = '<p><h2>' . __('Using LDAPS') . '</h2>
<p>' . __('By default, we skip certificate validation because customers tend to use self-signed certificates. To enable certificate validation, edit the file below') . ':<br>

<br><strong>' . __('Linux') . '</strong><br>

<code>/usr/local/open-audit/app/Models/logonModel.php</code><br>

<br><strong>' . __('Windows') . '</strong><br>
...
<br>' . __('One other item to be careful of - make sure you use the hostname (or fqdn) of your LDAP Server that matches what is in the certificate (not just the LDAP Servers IP address) when creating the LDAP entry in Open-AudIT.') . '<br>
<br></p>
...
<br>' . __('To a new file named .env (in the same directory). Now edit that new file and uncomment (remove the #) the line below (line 17)') . ':<br>

<code># CI_ENVIRONMENT = development</code><br>

<br>' . __('This should be commented out after troubleshooting as debug mode will generate a lot of logs on the disk for no benefit.') . '<br><br></p>

<h2>' . __('Second') . '</h2>

<p>' . __('Try logging in using an LDAP user. We are assuming this will fail (otherwise, why are you still reading this?). Next, comment out the line from ther file above.') . '<br><br></p>

<h2>' . __('Third') . '</h2>

<p>' . __('Read through the log file at') . ' open-audit\writable\logs\log-$year-$day.log ' . __('and the final lines will likely be of most interest. These lines should give you the exact point at which the login failed.') . '<br><br></p>
<br><br>
<table class="table">
	<thead>
		<tr>
	...
	<tbody>
		...
		<tr>
			...
			<td>' . __('Where $x is a number. This many LDAP entries are in the DB and have been retrieved.') . '</td>
			<td>debug</td>
		</tr>
		...
		<tr><td>An invalid LDAP version was supplied $ldap-&gt;version, skipping.</td>
			<td>' . __('Usually should be set to 3.') . '</td>
			<td>error</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>' . __('See above. NOTE - This could also be caused by a self-signed certificate on the LDAP server. We are working to allow for this in a future release.') . '</td>
			<td>notice</td>
			</tr>
		<tr>
			<td>Invalid user supplied credentials for LDAP server at $ldap-&gt;host, skipping.</td>
			<td>' . __('The credentials supplied by the user have failed.') . '</td>
			<td>info</td>
			</tr>
		<tr>
		...
		<tr>
			...
			<td>' . __('The LDAP server was connected to and the user credentials accepted for bind.') . '</td>
			<td>debug</td>
			</tr>
		<tr>
			<td>Invalid DN supplied credentials for LDAP server at $ldap-&gt;host, skipping</td>
			<td>' . __('The administrator supplied credentials to bind to the LDAP server, but they have not been accepted by the LDAP sevrer. Double check the credentials work on the LDAP server, and then check (or reset them) in the Open-AudIT LDAP Server entry.') . '</td>
			<td>info</td>
			</tr>
		<tr>
			<td>Bound to LDAP using supplied dn details: $ldap-&gt;dn_account</td>
			<td>' . __('The administrator supplied credentials that were successfully used to bind to LDAP.') . '</td>
			<td>debug</td>
			</tr>
		<tr>
		...
		<tr>
			...
			<td>' . __('LDAP was searched for this user and their account was found.') . '</td>
			<td>debug</td>
			</tr>
		<tr>
			<td>LDAP entries retrieval successful for user $username at $ldap-&gt;host</td>
			<td>' . __('The users details were retrieved from LDAP.') . '</td>
			<td>debug</td>
			</tr>
		<tr>
			<td>LDAP entries retrieval failed for user $username at $ldap-&gt;host</td>
			<td>' . __('The users details were not retrieved from LDAP. Check the LDAP server logs.') . '</td>
			<td>info</td>
			</tr>
		<tr>
		...
		<tr>
			...
			<td>' . __('Information only.') . '</td>
			<td>debug</td>
			</tr>
		<tr>
			<td>User $username is a member of LDAP group for Role $role-&gt;ad_group</td>
			<td>' . __('The user is in the LDAP group that matches this Role.') . '</td>
			<td>debug</td>
			</tr>
		<tr>
			<td>No AD group associated with role $role-&gt;name, skipping.</td>
			<td>' . __('This Role has no AD group specified. Check the roles details within Open-AudIT.') . ' <a href="/display/OA/Roles">' . __('Roles') . '</a></td>
			<td>debug</td>
			</tr>
		<tr>
			<td>User $username is a member of LDAP group for Org $org-&gt;ad_group</td>
			<td>' . __('The user is in the LDAP group that matches this Org.') . '</td>
			<td>debug</td>
			</tr>
		<tr>
			<td>No AD group associated with org $org-&gt;name, skipping.</td>
			<td>' . __('This Org has no AD group specified. Check the roles details within Open-AudIT.') . ' <a href="/display/OA/Orgs">' . __('Orgs') . '</a></td>
			<td>debug</td>
			</tr>
		<tr>
		...
		<tr>
			...
			<td>' . __('The user is not in the LDAP group that matches this Role.') . '</td>
			<td>debug</td>
			</tr>
		<tr>
			<td>LDAP search failed for groups (roles) $user-&gt;name at $ldap-&gt;host</td>
			<td>' . __('The search for group on the LDAP server failed. Check the LDAP server logs. Have you created these groups (for roles and orgs) on the LDAP server and assigned LDAP users to them?') . '</td>
			<td>debug</td>
			</tr>
		<tr>
		...
		<tr>
			...
			<td>' . __('The user is not in the LDAP group that matches this Org.') . '</td>
			<td>debug</td>
			</tr>
		<tr>
		...
		<tr>
			<td>No AD group associated with org $org-&gt;name, skipping.</td>
			<td>' . __('This Org has no AD group specified. Check the roles details within Open-AudIT. <span class="confluence-link">Have you created these groups (for orgs) on the LDAP server and assigned LDAP users to them?') . '</span></td>
			<td>debug</td>
			</tr>
		<tr>
			<td>New user $username logged on (AD account)</td>
			<td>' . __('A new user logged in to Open-AudIT and was authenticated and authorized by the LDAP sever. That user was then created in Open-AudIT and logged in. Success.') . '</td>
			<td>notice</td>
			</tr>
		<tr>
			<td>Existing user $username logged on (AD account).</td>
			<td>' . __('An existing Open-AudIT user was authenticated and authorized by the LDAP server. Success.') . '</td>
			<td>debug</td>
			</tr>
		<tr>
			...
			<td>' . __('The user is in LDAP and their credentials are valid, but is not in any of the required Open-AudIT LDAP groups.') . '</td>
			<td>info</td>
			</tr>
		<tr>
			...
			<td>' . __('The user is in LDAP and their credentials are valid, but is not in any of the Open-AudIT LDAP groups for Orgs.') . '</td>
			<td>info</td>
			</tr>
		<tr>
    		<td>User $username exists in LDAP $ldap-&gt;name and attempted to logon, but does not belong to any OA groups for Roles.</td>
			<td>' . __('The user is in LDAP and their credentials are valid, but is not in any of the Open-AudIT LDAP groups for Roles.') . '</td>
			<td>info</td>
			</tr>
	</tbody>
</table>
...
<p>' . __('If you are a supported FirstWave customer,a couple of items will make helping your easier. Open a support ticket with FirstWave and attach the logfile. Also include the output from the Support button on the upper right of the') . ' <a href="' . url_to('supportCollection') . '">' . __('Support') . '</a> ' . __('page. Export your LDAP server from menu -> Admin -> LDAP Servers -> Details to JSON and save and include this data as well.') . '<br>

<br>' . __('Please send all three files to your support contact at Opmantek and describe your issue.') . '<br><br></p>';
