...
<p>' . __('The SELECT section of the query should use full dot notation and also request the field with it\'s full dot name. IE - SELECT devices.id AS `devices.id`. Each field should be selected in this fashion to enable GUI side attribute filtering.') . '</p>
<p>' . __('Queries for Groups should select only the') . ' <code>devices.id</code> ' . __('column') . '.</p>
<p>
...
<p>' . __('All device sub-tables contain a couple of important columns.') . ' <code>$table.device_id</code> ' . __('and') . ' <code>$table.current</code>. <code>$table.device_id</code> ' . __('is the link to the') . ' <code>devices.id</code> ' . __('column. The') . ' <code>$table.current</code> ' . __('column will contain either \'y\' or \'n\'. This indicates if this row is currently present on the device. For example software may be installed (which would result in <code>software.current = \'y\'</code>), but on a subsequent audit it may not be detected. Open-AudIT will then change this rows current attribute to \'n\'.') . '</p>
...
<p>' . __('Other than that restriction, you are free to select attributes as required. It\'s a good idea to use the menu -> Admin -> Database ->') . ' <a href="' . url_to('databaseCollection') . '">' . __('List Tables') . '</a> ' . __('item to view the specific tables and their columns. This will enable you to find exactly what you need, rather than trawling through the MySQL console of the schema creation script.') . '</p>
