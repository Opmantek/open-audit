 ...
 */
function ssh_juniper_audit(string $ip = '', int $discovery_id = 0, array $credentials = null): object
{
    ...
    $log->file = 'ssh_juniper_helper';
    $log->function = 'ssh_juniper_audit';
    $log->ip = $ip;
    ...
    if (!empty($command_result)) {
        log_message('debug', 'Parsing \'show version\' for Juniper.');
    }
    $explode = explode("\n", $command_result);
    unset($explode[0]);
    unset($explode[1]);
    ...
    $command_result = $ssh->read();
    if (!empty($command_result)) {
        log_message('debug', 'Parsing \'show configuration\' for Juniper.');
    }
    $explode = explode("\n", $command_result);
    unset($explode[0]);
    unset($explode[1]);
