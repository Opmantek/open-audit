...
function my_snmp_get_command($ip, $credentials, $oid)
{
    ...
    if (intval($credentials->credentials->version) === 3) {
        ...
        $command = 'snmpget -v3 -On ' . $security_level . $security_name . $authentication_protocol . $authentication_passphrase . $privacy_protocol . $privacy_passphrase . $context_name . $context_engine_id . ' ' . escapeshellarg($ip) . ' ' . escapeshellarg($oid);
        exec($command, $output, $return_var);
        $array = array();
        foreach ($output as $line) {
            $temp = explode(' = ', $line);
...
function my_snmp_walk_command($ip, $credentials, $oid)
{
    ...
    if (intval($credentials->credentials->version) === 3) {
        ...
        $context_name =                 !empty($credentials->credentials->context_name) ?               ' -n ' . escapeshellarg($credentials->credentials->context_name) : '';
        $context_engine_id =            !empty($credentials->credentials->context_engine_id) ?          ' -e ' . escapeshellarg(implode(unpack("H*", $credentials->credentials->context_engine_id))) : '';
        $command = 'snmpwalk -v3 -On ' . $security_level . $security_name . $authentication_protocol . $authentication_passphrase . $privacy_protocol . $privacy_passphrase . $context_name . $context_engine_id . ' ' . escapeshellarg($ip) . ' ' . escapeshellarg($oid);
        exec($command, $output, $return_var);
        $array = array();
        foreach ($output as $line) {
            $temp = explode(' = ', $line);
