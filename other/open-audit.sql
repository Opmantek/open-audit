...
INSERT INTO `configuration` VALUES (NULL,'display_version','5.3.0','text','n','system','2000-01-01 00:00:00','The version shown on the web pages.');
...
INSERT INTO `configuration` VALUES (NULL,'internal_version','20240810','number','n','system','2000-01-01 00:00:00','The internal numerical version.');
...
INSERT INTO `queries` VALUES (35,1,'NMIS','Device','y','NMIS Export query','SELECT devices.id AS `devices.id`, devices.name AS `devices.name`, devices.ip AS `devices.ip`, devices.hostname AS `devices.hostname`, devices.fqdn AS `devices.fqdn`, devices.dns_hostname AS `devices.dns_hostname`, devices.omk_uuid AS `devices.omk_uuid`, devices.nmis_name AS `devices.nmis_name`, devices.nmis_group AS `devices.nmis_group`, devices.nmis_role AS `devices.nmis_role`, devices.nmis_manage AS `devices.nmis_manage`, devices.nmis_business_service AS `devices.nmis_business_service`, devices.nmis_poller AS `devices.nmis_poller` FROM `devices` WHERE @filter AND devices.nmis_manage = \'y\'','','n','system','2000-01-01 00:00:00');
