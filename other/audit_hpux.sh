...
system_hostname=$(eval "machinfo $safety | grep -i \"Nodename\" | cut -d: -f2")
...
system_uuid=$(eval "machinfo $safety | grep -i \"Machine ID number:\" | cut -d: -f2")
...
system_os_version=$(eval "machinfo $safety | grep -i \"Release:\" | cut -d: -f2")
system_serial=$(eval "machinfo $safety | grep \"Machine serial number:\" | cut -d: -f2")
system_model=$(eval "machinfo $safety | grep -i \"Model:\" | cut -d: -f2")
system_uptime=$(eval "uptime $safety")
memory_count=$(eval "machinfo $safety | grep -i \"Memory:\" | cut -d: -f2 | cut -d\" \" -f1 ")
processor_count=$(eval "machinfo $safety | grep -i \"logical processors\"")
processor_count=$(escape "$processor_count")
processor_count=$(eval "echo \"$processor_count\" | cut -d\" \" -f1")
...
physical_count=$(eval "machinfo $safety | grep sockets | cut -ds -f1")
core_count=$(eval "machinfo $safety | grep cores | cut -dc -f1")
logical_count=$(eval "machinfo $safety | grep \"logical processors\" | cut -dl -f1")
speed=$(eval "machinfo $safety | grep GHz | cut -d( -f2 | cut -d\" \" -f1")
description=$(eval "machinfo $safety | grep -i \"processor (\"")
processor_manufacturer=$(eval "machinfo $safety | grep \"processor (\" | cut -d( -f1")
cat >>"$xml_file" <<EndOfFile
	<processor>
		<item>
...
echo "	<user>" >> "$xml_file"
for username in $(eval "listusers | awk '{print \$0}' $safety"); do
	user_name=$(echo $username | cut -f1 -d' ')
	#user_name="$username"
	#user_sid=$(eval "lsuser -f -a id $username | grep id | cut -d= -f2 $safety")
	user_full_name=$(echo $username | awk '{print $2,$3,$4}')
	#user_full_name=$(eval "listusers $safety")
...
for line in $(eval "netstat -an | grep -i LISTEN | awk '{print \$1 \" \" \$2 \" \" \$5}' $safety"); do 
...
for software in $(eval "swlist -l product $safety"); do
	#if [[ -n "$software" ]]; then
		software_name=$(echo $software | awk '{print $1}')
		#user_full_name=$(echo $username | awk '{print $2,$3,$4}')
		#software_name=$(eval "echo \"$software\" | cut -d':' -f2 $safety")
		software_version=$(echo $software | awk '{print $2}')
		#software_status=$(eval "echo \"$software\" | cut -d':' -f5 $safety")
		software_description=$(echo $software | awk '{$1=$2=""; print $0}')
		cat >>"$xml_file" <<EndOfFile
		<item>
