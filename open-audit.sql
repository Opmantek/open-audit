...
INSERT INTO `configuration` VALUES (117,'internal_version','20230208','number','n','system','2000-01-01 00:00:00','The internal numerical version.');
...
CREATE TABLE `credential` (
  ...
  CONSTRAINT `credential_system_id` FOREIGN KEY (`system_id`) REFERENCES `devices` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_general_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `disk` (
  ...
  KEY `system_id` (`system_id`),
  CONSTRAINT `disk_system_id` FOREIGN KEY (`system_id`) REFERENCES `devices` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_general_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `dns` (
  ...
  PRIMARY KEY (`id`),
  KEY `system_id` (`system_id`),
  CONSTRAINT `dns_system_id` FOREIGN KEY (`system_id`) REFERENCES `devices` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_general_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `graph` (
  ...
  PRIMARY KEY (`id`),
  KEY `system_id` (`system_id`),
  CONSTRAINT `graph_system_id` FOREIGN KEY (`system_id`) REFERENCES `devices` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_general_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `log` (
  ...
  PRIMARY KEY (`id`),
  KEY `system_id` (`system_id`),
  CONSTRAINT `log_system_id` FOREIGN KEY (`system_id`) REFERENCES `devices` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_general_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `nmap` (
  ...
  PRIMARY KEY (`id`),
  KEY `system_id` (`system_id`),
  CONSTRAINT `nmap_system_id` FOREIGN KEY (`system_id`) REFERENCES `devices` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_general_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
INSERT INTO `queries` VALUES (3,1,'Consumed IP Addresses','Network','y','The ip addresses used by a group.','SELECT system.id AS `system.id`, system.icon AS `system.icon`, system.type AS `system.type`, system.name AS `system.name`, system.domain AS `system.domain`, system.ip AS `system.ip`, system.os_family AS `system.os_family`, system.description AS `system.description`, ip.ip as `ip.ip` FROM ip LEFT JOIN system ON (system.id = ip.system_id AND ip.current = \'y\') WHERE @filter AND ip.ip IS NOT NULL AND ip.ip != \'127.000.000.001\' AND ip.ip != \'\' AND ip.ip != \'0.0.0.0\' AND ip.ip != \'000.000.000.000\' AND ip.version = \'4\' GROUP BY ip.id, ip.ip ORDER BY ip.ip','','system','2000-01-01 00:00:00');
INSERT INTO `queries` VALUES (4,1,'Devices Without Credentials','Device','y','Device details - name, ip, last seen on and by for those devices only discovered by Nmap and have therefore not been audited.','SELECT system.id AS `system.id`, system.icon AS `system.icon`, system.type AS `system.type`, system.name AS `system.name`, system.domain AS `system.domain`, system.ip AS `system.ip`, audit_log.timestamp AS `audit_log.timestamp`, audit_log.ip AS `audit_log.ip` FROM audit_log LEFT JOIN system ON (audit_log.system_id = system.id) WHERE @filter AND audit_log.system_id NOT IN (SELECT DISTINCT(audit_log.system_id) FROM audit_log WHERE type != \'nmap\') GROUP BY system.id','','system','2000-01-01 00:00:00');
INSERT INTO `queries` VALUES (5,1,'Disk Partition Use','Device','y','Partition details where partition free and used space aren\'t 0 and type isn\'t Volume or Network Drive and mount point isn\'t [SWAP].','SELECT system.id AS `system.id`, system.icon AS `system.icon`, system.type AS `system.type`, system.name AS `system.name`, system.domain AS `system.domain`, system.ip AS `system.ip`, partition.id AS `partition.id`, partition.hard_drive_index AS `partition.hard_drive_index`, partition.mount_point AS `partition.mount_point`, partition.mount_type as `partition.mount_type`, partition.size AS `partition.size`, partition.used AS `partition.used`, partition.free AS `partition.free`, ROUND(((partition.free / partition.size) * 100), 0) AS percent_free, partition.name AS `partition.name` FROM system INNER JOIN `partition` ON (partition.system_id = system.id AND partition.current = \'y\') WHERE @filter AND partition.used > 0 AND partition.free > 0 AND LOWER(partition.type) != \'volume\' AND LOWER(partition.type) != \'network drive\' AND LOWER(partition.mount_point) != \'[swap]\' ORDER BY system.name, partition.id','','system','2000-01-01 00:00:00');
INSERT INTO `queries` VALUES (6,1,'Export Details','Device','y','Icon, type, name, first seen on, last seen on, last seen by, manufacturer, model, serial, owner, organisation, location, operating system.','SELECT system.id AS `system.id`, system.icon AS `system.icon`, system.type AS `system.type`, system.name AS `system.name`, system.domain AS `system.domain`, system.ip AS `system.ip`, system.fqdn AS `system.fqdn`, system.serial AS `system.serial`, system.class AS `system.class`, system.function AS `system.function`, system.owner AS `system.owner`, system.asset_number AS `system.asset_number` FROM system WHERE @filter','','system','2000-01-01 00:00:00');
INSERT INTO `queries` VALUES (7,1,'Failed Audits','Device','y','Name, ip address, audit time, audit stage.','SELECT system.id AS `system.id`, system.icon AS `system.icon`, system.type AS `system.type`, system.name AS `system.name`, system.domain AS `system.domain`, system.ip AS `system.ip`, audit_log.timestamp AS `audit_log.timestamp`, audit_log.debug AS `audit_log.debug` FROM system LEFT JOIN audit_log ON (system.id = audit_log.system_id) WHERE @filter AND audit_log.debug > \'\'','','system','2000-01-01 00:00:00');
...
INSERT INTO `queries` VALUES (10,1,'Workstations','Hardware','y','Device details - name, ip, manufacturer, model, serial, form factor, memory, processor.','SELECT system.id AS `system.id`, system.icon AS `system.icon`, system.type AS `system.type`, system.name AS `system.name`, system.domain AS `system.domain`, system.ip AS `system.ip`, system.manufacturer AS `system.manufacturer`, system.model AS `system.model`, system.serial AS `system.serial`, system.os_family AS `system.os_family`, system.memory_count AS `system.memory_count`, system.form_factor AS `system.form_factor`, processor.description AS `processor.description` FROM system LEFT JOIN processor ON (processor.system_id = system.id AND processor.current = \'y\') WHERE @filter AND system.type = \'computer\' AND system.class != \'server\' AND system.class != \'hypervisor\' ORDER BY system.name','','system','2000-01-01 00:00:00');
INSERT INTO `queries` VALUES (11,1,'Acrobat','Software','y','Adobe Acrobat installations (software name contains \'acrobat\' or \'adobe reader\').','SELECT system.id AS `system.id`, system.icon AS `system.icon`, system.type AS `system.type`, system.name AS `system.name`, system.domain AS `system.domain`, system.ip AS `system.ip`, system.class AS `system.class`, system.os_family AS `system.os_family`, orgs.name AS `orgs.name`, windows.user_name AS `windows.user_name`, software.name as `software.name`, software.version AS `software.version`, software.id as `software.id` FROM software LEFT JOIN system ON (software.system_id = system.id AND software.current = \'y\' AND (software.name LIKE \'%acrobat%\' OR software.name LIKE \'adobe reader%\')) LEFT JOIN orgs ON (orgs.id = system.org_id) LEFT JOIN windows ON (windows.system_id = system.id AND windows.current = \'y\') WHERE @filter','','system','2000-01-01 00:00:00');
INSERT INTO `queries` VALUES (12,1,'Antivirus','Software','y','Installed AntiVirus software (software name contains \'virus\' or \'trend micro\' or \'endpoint\').','SELECT system.id AS `system.id`, system.icon AS `system.icon`, system.type AS `system.type`, system.name AS `system.name`, system.domain AS `system.domain`, system.ip AS `system.ip`, system.class AS `system.class`, system.os_family AS `system.os_family`, orgs.name AS `orgs.name`, windows.user_name AS `windows.user_name`, software.name as `software.name`, software.version AS `software.version` FROM system LEFT JOIN software ON (software.system_id = system.id AND software.current = \'y\' AND (software.name LIKE \'%virus%\' or software.name LIKE \'%trend micro%\' or software.name LIKE \'%endpoint%\')) LEFT JOIN orgs ON (orgs.id = system.org_id) LEFT JOIN windows ON (windows.system_id = system.id AND windows.current = \'y\') WHERE @filter AND system.type = \'computer\'','','system','2000-01-01 00:00:00');
INSERT INTO `queries` VALUES (13,1,'Flash','Software','y','Flash Player installations.','SELECT system.id AS `system.id`, system.icon AS `system.icon`, system.type AS `system.type`, system.name AS `system.name`, system.domain AS `system.domain`, system.ip AS `system.ip`, system.class AS `system.class`, system.os_family AS `system.os_family`, orgs.name AS `orgs.name`, windows.user_name AS `windows.user_name`, software.name as `software.name`, software.version AS `software.version`, software.id as `software.id` FROM software LEFT JOIN system ON (software.system_id = system.id AND software.current = \'y\' AND software.name LIKE \'%Flash Player%\') LEFT JOIN orgs ON (orgs.id = system.org_id) LEFT JOIN windows ON (windows.system_id = system.id AND windows.current = \'y\') WHERE @filter','','system','2000-01-01 00:00:00');
...
INSERT INTO `queries` VALUES (15,1,'MS Office','Software','y','MS Office installations.','SELECT system.id AS `system.id`, system.icon AS `system.icon`, system.name AS `system.name`, system.domain AS `system.domain`, system.os_family AS `system.os_family`, orgs.name AS `orgs.name`, windows.user_name AS `windows.user_name`, software.name as `software.name` FROM software LEFT JOIN system ON (software.system_id = system.id) LEFT JOIN orgs ON (orgs.id = system.org_id) LEFT JOIN windows ON (windows.system_id = system.id AND windows.current = \'y\') WHERE @filter AND software.current = \'y\' AND software.name LIKE \'Microsoft Office%\' AND (software.name LIKE \'%Starter%\' OR software.name LIKE \'%Basic%\' OR software.name LIKE \'%Personal%\' OR software.name LIKE \'%Home%\' OR software.name LIKE \'%Student%\' OR software.name LIKE \'%Business%\' OR software.name LIKE \'%Standard%\' OR software.name LIKE \'%Ultimate%\' OR software.name LIKE \'%Enterprise%\' OR software.name LIKE \'%Professional%\' OR software.name LIKE \'%Professional Plus%\') AND (software.name LIKE \'%2003%\' OR software.name LIKE \'%2007%\' OR software.name LIKE \'%2010%\' OR software.name LIKE \'%2013%\' OR software.name LIKE \'%2016%\' OR software.name LIKE \'%365%\')','','system','2000-01-01 00:00:00');
...
INSERT INTO `queries` VALUES (17,1,'MS Visio','Software','y','MS Visio installations (software name contains \'Microsoft Visio\').','SELECT system.id AS `system.id`, system.icon AS `system.icon`, system.type AS `system.type`, system.name AS `system.name`, system.domain AS `system.domain`, system.ip AS `system.ip`, system.class AS `system.class`, system.os_family AS `system.os_family`, orgs.name AS `orgs.name`, windows.user_name AS `windows.user_name`, software.name as `software.name`, software.version AS `software.version`, software.id as `software.id` FROM software LEFT JOIN system ON (software.system_id = system.id AND software.current = \'y\' AND software.name LIKE \'Microsoft%Visio%\') LEFT JOIN orgs ON (orgs.id = system.org_id) LEFT JOIN windows ON (windows.system_id = system.id AND windows.current = \'y\') WHERE @filter','','system','2000-01-01 00:00:00');
...
INSERT INTO `queries` VALUES (19,1,'Log Files','Device','y','Log file details.','SELECT system.id AS `system.id`, system.icon AS `system.icon`, system.type AS `system.type`, system.name AS `system.name`, system.domain AS `system.domain`, system.ip AS `system.ip`, log.name as `log.name`, log.file_name AS `log.file_name`, log.file_size AS `log.file_size`, log.max_file_size AS `log.max_file_size`, log.overwrite AS `log.overwrite` FROM log LEFT JOIN system ON (system.id = log.system_id and log.current = \'y\') WHERE @filter ORDER BY system.name','','system','2000-01-01 00:00:00');
INSERT INTO `queries` VALUES (20,1,'Netstat Ports','Device','y','Detected Netstat ports.','SELECT system.id AS `system.id`, system.icon AS `system.icon`, system.type AS `system.type`, system.name AS `system.name`, system.domain AS `system.domain`, system.ip AS `system.ip`, system.os_name AS `system.os_name`, netstat.protocol AS `netstat.protocol`, netstat.port AS `netstat.port`, netstat.ip AS `netstat.ip`, netstat.program AS `netstat.program` FROM system LEFT JOIN netstat ON (system.id = netstat.system_id AND netstat.current = \'y\') WHERE @filter','','system','2000-01-01 00:00:00');
...
INSERT INTO `queries` VALUES (30,1,'New Devices','Change','y','Any changes in the table \'system\'.','SELECT system.id AS `system.id`, system.icon AS `system.icon`, system.type AS `system.type`, system.name AS `system.name`, system.domain AS `system.domain`, system.ip AS `system.ip`, change_log.timestamp AS `change_log.timestamp`, change_log.db_table AS `change_log.db_table`, change_log.db_action AS `change_log.db_action`, change_log.details AS `change_log.details`, change_log.id AS `change_log.id`, CONCAT(\"devices?sub_resource=change_log&change_log.id=\", change_log.id) AS `link` FROM change_log LEFT JOIN system ON (change_log.system_id = system.id) WHERE @filter AND change_log.ack_time = \'2000-01-01 00:00:00\' AND change_log.db_table = \'system\'','','system','2000-01-01 00:00:00');
INSERT INTO `queries` VALUES (31,1,'Settings','Change','y','Any changes in the tables \'dns\', \'ip\', \'log\', netstat\', \'pagefile\', \'print_queue\', \'route\', \'task\', \'user\', \'user_group\' and \'variable\'.','SELECT system.id AS `system.id`, system.icon AS `system.icon`, system.type AS `system.type`, system.name AS `system.name`, system.domain AS `system.domain`, system.ip AS `system.ip`, change_log.timestamp AS `change_log.timestamp`, change_log.db_table AS `change_log.db_table`, change_log.db_action AS `change_log.db_action`, change_log.details AS `change_log.details`, change_log.id AS `change_log.id`, CONCAT(\"devices?sub_resource=change_log&change_log.id=\", change_log.id) AS `link` FROM change_log LEFT JOIN system ON (change_log.system_id = system.id) WHERE @filter AND change_log.ack_time = \'2000-01-01 00:00:00\' AND change_log.db_table in (\'dns\', \'ip\', \'log\', \'netstat\', \'pagefile\', \'print_queue\', \'route\', \'task\', \'user\', \'user_group\', \'variable\')','','system','2000-01-01 00:00:00');
...
INSERT INTO `queries` VALUES (35,1,'NMIS','Device','y','NMIS Export query','SELECT system.id AS `system.id`, system.name AS `system.name`, system.ip AS `system.ip`, system.hostname AS `system.hostname`, system.fqdn AS `system.fqdn`, system.dns_hostname AS `system.dns_hostname`, system.omk_uuid AS `system.omk_uuid`, system.nmis_name AS `system.nmis_name`, system.nmis_group AS `system.nmis_group`, system.nmis_role AS `system.nmis_role`, system.nmis_manage AS `system.nmis_manage`, system.nmis_business_service AS `system.nmis_business_service`, system.nmis_poller AS `system.nmis_poller` FROM `system` WHERE @filter AND system.nmis_manage = \'y\'','','system','2000-01-01 00:00:00');
...
INSERT INTO `queries` VALUES (39,1,'Expiring Certificates (action immediately)','Server','y','Certificates expiring between in the next 8 days','SELECT system.id AS `system.id`, system.name AS `system.name`, certificate.name AS `certificate.name`, certificate.valid_to AS `certificate.valid_to` FROM certificate LEFT JOIN system ON (certificate.system_id = system.id AND certificate.current = \'y\') WHERE @filter AND certificate.valid_to > NOW() AND certificate.valid_to < DATE(NOW() + INTERVAL 8 day)','','system','2000-01-01 00:00:00');
...
CREATE TABLE `radio` (
  ...
  PRIMARY KEY (`id`),
  KEY `system_id` (`system_id`),
  CONSTRAINT `radio_system_id` FOREIGN KEY (`system_id`) REFERENCES `devices` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_general_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `route` (
  ...
  PRIMARY KEY (`id`),
  KEY `system_id` (`system_id`),
  CONSTRAINT `route_system_id` FOREIGN KEY (`system_id`) REFERENCES `devices` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_general_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
INSERT INTO `rules` VALUES (15,'Detect Buffalo Terrastation in sysDescr',1,'Set the manufacturer, model and type and based on the sysDescr.',100,'[{\"table\":\"system\",\"attribute\":\"sysDescr\",\"operator\":\"li\",\"value\":\"buffalo terastation\"}]','[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"value\":\"Buffalo\",\"value_type\":\"string\"},{\"table\":\"system\",\"attribute\":\"type\",\"value\":\"nas\",\"value_type\":\"string\"},{\"table\":\"system\",\"attribute\":\"model\",\"value\":\"TeraStation\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (16,'Manufacturer Name TPLink',1,'Set the manufacturer.',100,'[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"operator\":\"li\",\"value\":\"tplink\"}]','[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"value\":\"TP-Link\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
...
INSERT INTO `rules` VALUES (18,'VoIP Phone guess based on Nmap port',1,'Set the type based on Nmap port 5060.',100,'[{\"table\":\"nmap\",\"attribute\":\"port\",\"operator\":\"eq\",\"value\":\"5060\"},{\"table\":\"system\",\"attribute\":\"snmp_oid\",\"operator\":\"eq\",\"value\":\"\"},{\"table\":\"system\",\"attribute\":\"uuid\",\"operator\":\"eq\",\"value\":\"\"}]','[{\"table\":\"system\",\"attribute\":\"type\",\"value\":\"voip phone\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
...
INSERT INTO `rules` VALUES (24,'Ubiquiti Model',1,'Set the type and model based on sysDescr.',100,'[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"operator\":\"eq\",\"value\":\"Ubiquiti Networks Inc.\"},{\"table\":\"system\",\"attribute\":\"sysDescr\",\"operator\":\"li\",\"value\":\"UAP\"}]','[{\"table\":\"system\",\"attribute\":\"type\",\"value\":\"wap\",\"value_type\":\"string\"},{\"table\":\"system\",\"attribute\":\"model\",\"value\":\"UniFi AP\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (25,'Ubiquiti Model',1,'Set the type and model based on sysDescr.',100,'[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"operator\":\"eq\",\"value\":\"Ubiquiti Networks Inc.\"},{\"table\":\"system\",\"attribute\":\"sysDescr\",\"operator\":\"li\",\"value\":\"UAP-LR\"}]','[{\"table\":\"system\",\"attribute\":\"type\",\"value\":\"wap\",\"value_type\":\"string\"},{\"table\":\"system\",\"attribute\":\"model\",\"value\":\"UniFi AP-LR\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (26,'Ubiquiti Model',1,'Set the type and model based on sysDescr.',100,'[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"operator\":\"eq\",\"value\":\"Ubiquiti Networks Inc.\"},{\"table\":\"system\",\"attribute\":\"sysDescr\",\"operator\":\"li\",\"value\":\"UAP-Pro\"}]','[{\"table\":\"system\",\"attribute\":\"type\",\"value\":\"wap\",\"value_type\":\"string\"},{\"table\":\"system\",\"attribute\":\"model\",\"value\":\"UniFi AP-Pro\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (27,'Ubiquiti Model',1,'Set the type and model based on sysDescr.',100,'[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"operator\":\"eq\",\"value\":\"Ubiquiti Networks Inc.\"},{\"table\":\"system\",\"attribute\":\"sysDescr\",\"operator\":\"li\",\"value\":\"UAP-AC\"}]','[{\"table\":\"system\",\"attribute\":\"type\",\"value\":\"wap\",\"value_type\":\"string\"},{\"table\":\"system\",\"attribute\":\"model\",\"value\":\"UniFi AP-AC\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (28,'Ubiquiti Model',1,'Set the type and model based on sysDescr.',100,'[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"operator\":\"eq\",\"value\":\"Ubiquiti Networks Inc.\"},{\"table\":\"system\",\"attribute\":\"sysDescr\",\"operator\":\"li\",\"value\":\"UAP-AC-EDU\"}]','[{\"table\":\"system\",\"attribute\":\"type\",\"value\":\"wap\",\"value_type\":\"string\"},{\"table\":\"system\",\"attribute\":\"model\",\"value\":\"UniFi AP-AC-EDU\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (29,'Ubiquiti Model',1,'Set the type and model based on sysDescr.',100,'[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"operator\":\"eq\",\"value\":\"Ubiquiti Networks Inc.\"},{\"table\":\"system\",\"attribute\":\"sysDescr\",\"operator\":\"li\",\"value\":\"UAP-AC-HD\"}]','[{\"table\":\"system\",\"attribute\":\"type\",\"value\":\"wap\",\"value_type\":\"string\"},{\"table\":\"system\",\"attribute\":\"model\",\"value\":\"UniFi AP-AC-HD\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (30,'Ubiquiti Model',1,'Set the type and model based on sysDescr.',100,'[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"operator\":\"eq\",\"value\":\"Ubiquiti Networks Inc.\"},{\"table\":\"system\",\"attribute\":\"sysDescr\",\"operator\":\"li\",\"value\":\"UAP-AC-Lite\"}]','[{\"table\":\"system\",\"attribute\":\"type\",\"value\":\"wap\",\"value_type\":\"string\"},{\"table\":\"system\",\"attribute\":\"model\",\"value\":\"UniFi AP-AC-Lite\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (31,'Ubiquiti Model',1,'Set the type and model based on sysDescr.',100,'[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"operator\":\"eq\",\"value\":\"Ubiquiti Networks Inc.\"},{\"table\":\"system\",\"attribute\":\"sysDescr\",\"operator\":\"li\",\"value\":\"UAP-AC-LR\"}]','[{\"table\":\"system\",\"attribute\":\"type\",\"value\":\"wap\",\"value_type\":\"string\"},{\"table\":\"system\",\"attribute\":\"model\",\"value\":\"UniFi AP-AC-LR\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
...
INSERT INTO `rules` VALUES (33,'Ubiquiti Model',1,'Set the type and model based on sysDescr.',100,'[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"operator\":\"eq\",\"value\":\"Ubiquiti Networks Inc.\"},{\"table\":\"system\",\"attribute\":\"sysDescr\",\"operator\":\"li\",\"value\":\"UAP-AC-PRO-Gen2\"}]','[{\"table\":\"system\",\"attribute\":\"type\",\"value\":\"wap\",\"value_type\":\"string\"},{\"table\":\"system\",\"attribute\":\"model\",\"value\":\"UniFi AP-AC-Pro gen 2\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (34,'Ubiquiti Model',1,'Set the type and model based on sysDescr.',100,'[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"operator\":\"eq\",\"value\":\"Ubiquiti Networks Inc.\"},{\"table\":\"system\",\"attribute\":\"sysDescr\",\"operator\":\"li\",\"value\":\"UAP-AC-SHD\"}]','[{\"table\":\"system\",\"attribute\":\"type\",\"value\":\"wap\",\"value_type\":\"string\"},{\"table\":\"system\",\"attribute\":\"model\",\"value\":\"UniFi AP-AC-SHD\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (35,'Ubiquiti Model',1,'Set the type and model based on sysDescr.',100,'[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"operator\":\"eq\",\"value\":\"Ubiquiti Networks Inc.\"},{\"table\":\"system\",\"attribute\":\"sysDescr\",\"operator\":\"li\",\"value\":\"UAP-AC-Mesh-Pro\"}]','[{\"table\":\"system\",\"attribute\":\"type\",\"value\":\"wap\",\"value_type\":\"string\"},{\"table\":\"system\",\"attribute\":\"model\",\"value\":\"UniFi AP-AC-Mesh-Pro\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (36,'Ubiquiti Model',1,'Set the type and model based on sysDescr.',100,'[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"operator\":\"eq\",\"value\":\"Ubiquiti Networks Inc.\"},{\"table\":\"system\",\"attribute\":\"sysDescr\",\"operator\":\"li\",\"value\":\"UAP-Outdoor\"}]','[{\"table\":\"system\",\"attribute\":\"type\",\"value\":\"wap\",\"value_type\":\"string\"},{\"table\":\"system\",\"attribute\":\"model\",\"value\":\"UniFi AP-Outdoor\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (37,'Compaq iLO',1,'Set type as a default based on OID.',90,'[{\"table\":\"system\",\"attribute\":\"snmp_oid\",\"operator\":\"st\",\"value\":\"1.3.6.1.4.1.232.9.4.10\"},{\"table\":\"system\",\"attribute\":\"type\",\"operator\":\"eq\",\"value\":\"\"}]','[{\"table\":\"system\",\"attribute\":\"type\",\"value\":\"remote access controller\",\"value_type\":\"string\"},{\"table\":\"system\",\"attribute\":\"model\",\"value\":\"Compaq iLO\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (38,'Netonix switches default',1,'Set type as a default based on OID.',90,'[{\"table\":\"system\",\"attribute\":\"snmp_oid\",\"operator\":\"st\",\"value\":\"1.3.6.1.4.1.46242\"},{\"table\":\"system\",\"attribute\":\"type\",\"operator\":\"eq\",\"value\":\"\"}]','[{\"table\":\"system\",\"attribute\":\"type\",\"value\":\"switch\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (39,'Apresia switches default',1,'Set type as a default based on OID.',90,'[{\"table\":\"system\",\"attribute\":\"snmp_oid\",\"operator\":\"st\",\"value\":\"1.3.6.1.4.1.278.1.27\"},{\"table\":\"system\",\"attribute\":\"model\",\"operator\":\"eq\",\"value\":\"\"}]','[{\"table\":\"system\",\"attribute\":\"type\",\"value\":\"switch\",\"value_type\":\"string\"},{\"table\":\"system\",\"attribute\":\"model\",\"value\":\"Apresia\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (40,'Apresia WAPs default',1,'Set type as a default based on OID.',90,'[{\"table\":\"system\",\"attribute\":\"snmp_oid\",\"operator\":\"st\",\"value\":\"1.3.6.1.4.1.278.1.19\"},{\"table\":\"system\",\"attribute\":\"model\",\"operator\":\"eq\",\"value\":\"\"}]','[{\"table\":\"system\",\"attribute\":\"type\",\"value\":\"wap\",\"value_type\":\"string\"},{\"table\":\"system\",\"attribute\":\"model\",\"value\":\"Apresia\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (41,'Ceragon Wireless Link default',1,'Set type as a default based on manufacturer.',80,'[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"operator\":\"li\",\"value\":\"Ceragon\"},{\"table\":\"system\",\"attribute\":\"type\",\"operator\":\"eq\",\"value\":\"\"}]','[{\"table\":\"system\",\"attribute\":\"type\",\"value\":\"wireless link\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (42,'VSS Monitor',1,'Set model and type based on OID.',90,'[{\"table\":\"system\",\"attribute\":\"snmp_oid\",\"operator\":\"st\",\"value\":\"1.3.6.1.4.1.21671.1.2\"},{\"table\":\"system\",\"attribute\":\"type\",\"operator\":\"eq\",\"value\":\"\"}]','[{\"table\":\"system\",\"attribute\":\"type\",\"value\":\"network device\",\"value_type\":\"string\"},{\"table\":\"system\",\"attribute\":\"model\",\"value\":\"VSS Monitor\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (43,'Form Factor based on Manufacturer (like VMware)',1,'Set the form factor based on the manufacturer.',90,'[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"operator\":\"li\",\"value\":\"vmware\"}]','[{\"table\":\"system\",\"attribute\":\"form_factor\",\"value\":\"Virtual\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (44,'Form Factor based on Manufacturer (like Parallels)',1,'Set the form factor based on the manufacturer.',90,'[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"operator\":\"li\",\"value\":\"parallels\"}]','[{\"table\":\"system\",\"attribute\":\"form_factor\",\"value\":\"Virtual\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (45,'Form Factor based on Manufacturer (like Systemtechnik)',1,'Set the form factor based on the manufacturer.',90,'[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"operator\":\"li\",\"value\":\"PCS Systemtechnik GmbH\"}]','[{\"table\":\"system\",\"attribute\":\"form_factor\",\"value\":\"Virtual\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (46,'Form Factor based on Manufacturer (like Virtual)',1,'Set the form factor based on the manufacturer.',90,'[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"operator\":\"li\",\"value\":\"virtual\"}]','[{\"table\":\"system\",\"attribute\":\"form_factor\",\"value\":\"Virtual\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
...
INSERT INTO `rules` VALUES (48,'Class based on Form Factor and OS (Physical Server)',1,'Set the class based on the form factor and OS.',100,'[{\"table\":\"system\",\"attribute\":\"form_factor\",\"operator\":\"ne\",\"value\":\"Virtual\"},{\"table\":\"system\",\"attribute\":\"os_group\",\"operator\":\"ne\",\"value\":\"Windows\"},{\"table\":\"system\",\"attribute\":\"os_group\",\"operator\":\"ne\",\"value\":\"\"},{\"table\":\"system\",\"attribute\":\"class\",\"operator\":\"eq\",\"value\":\"\"},{\"table\":\"system\",\"attribute\":\"manufacturer\",\"operator\":\"nl\",\"value\":\"Ubiquiti\"},{\"table\":\"system\",\"attribute\":\"type\",\"operator\":\"eq\",\"value\":\"computer\"}]','[{\"table\":\"system\",\"attribute\":\"class\",\"value\":\"server\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
...
INSERT INTO `rules` VALUES (50,'Class based on Form Factor and OS (Physical Windows Server)',1,'Set the class based on the form factor and OS.',100,'[{\"table\":\"system\",\"attribute\":\"form_factor\",\"operator\":\"ne\",\"value\":\"Virtual\"},{\"table\":\"system\",\"attribute\":\"os_group\",\"operator\":\"eq\",\"value\":\"Windows\"},{\"table\":\"system\",\"attribute\":\"os_name\",\"operator\":\"li\",\"value\":\"Server\"},{\"table\":\"system\",\"attribute\":\"os_group\",\"operator\":\"ne\",\"value\":\"\"},{\"table\":\"system\",\"attribute\":\"class\",\"operator\":\"eq\",\"value\":\"\"}]','[{\"table\":\"system\",\"attribute\":\"class\",\"value\":\"server\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (51,'Type based on Manufacturer and Hostname (playstation)',1,'Set the type based on the manufacturer and hostname.',100,'[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"operator\":\"li\",\"value\":\"sony\"},{\"table\":\"system\",\"attribute\":\"hostname\",\"operator\":\"li\",\"value\":\"playstation\"}]','[{\"table\":\"system\",\"attribute\":\"type\",\"value\":\"game console\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (52,'Type based on Manufacturer and DNS Hostname (playstation)',1,'Set the type based on the manufacturer and dns_hostname.',100,'[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"operator\":\"li\",\"value\":\"sony\"},{\"table\":\"system\",\"attribute\":\"dns_hostname\",\"operator\":\"li\",\"value\":\"playstation\"}]','[{\"table\":\"system\",\"attribute\":\"type\",\"value\":\"game console\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (53,'Form Factor based on Manufacturer (like innotek GmbH)',1,'Set the form factor based on the manufacturer.',100,'[{\"table\":\"system\",\"attribute\":\"manufacturer\",\"value\":\"innotek GmbH\",\"operator\":\"li\"}]','[{\"value\":\"Virtual\",\"table\":\"system\",\"attribute\":\"form_factor\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (54,'Manufacturer change HP to Hewlett Packard',1,'Set manufacturer to Hewlett Packard if we receive HP.',100,'[{\"attribute\":\"manufacturer\",\"operator\":\"eq\",\"table\":\"system\",\"value\":\"HP\"}]','[{\"attribute\":\"manufacturer\",\"table\":\"system\",\"value\":\"Hewlett Packard\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (55,'Ubiquiti RP-5AC-Gen2 set type',1,'Set type based on model.',100,'[{\"attribute\":\"model\",\"operator\":\"li\",\"table\":\"system\",\"value\":\"RP-5AC-Gen2\"}]','[{\"attribute\":\"type\",\"table\":\"system\",\"value\":\"wireless link\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
INSERT INTO `rules` VALUES (56,'NMIS Manage for SNMP devices',1,'Set nmis_manage to y if we detect an SNMP OID.',100,'[{\"attribute\":\"snmp_oid\",\"operator\":\"gt\",\"table\":\"system\",\"value\":\"\"}]','[{\"attribute\":\"nmis_manage\",\"table\":\"system\",\"value\":\"y\",\"value_type\":\"string\"}]','system','2001-01-01 00:00:00');
...
INSERT INTO `users` VALUES (2,'open-audit_enterprise',1,'','Open-AudIT Enterprise','','[\"admin\",\"org_admin\"]','[1]','en','y','','user',1,'','icontext','[]','system','2000-01-01 00:00:00');
INSERT INTO `users` VALUES (3,'nmis',1,'','NMIS','','[\"admin\",\"org_admin\"]','[1]','en','y','','user',1,'','icontext','[]','system','2000-01-01 00:00:00');
